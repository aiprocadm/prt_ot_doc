...
describe("GasAnalysisEditor", () => {
  it("рендерит строки замеров", () => {
    ...
    expect(
      (screen.getAllByLabelText("Значение")[0] as HTMLInputElement).value,
    ).toBe("20.9");
  });
  ...
  it("изменение значения зовёт onCell(index, 'value', newValue)", () => {
    ...
    fireEvent.change(screen.getAllByLabelText("Значение")[0], {
      target: { value: "19.5" },
    });
  ...
  it("смена параметра зовёт onCell(index, 'parameter', code)", () => {
    ...
    fireEvent.change(screen.getAllByLabelText("Параметр замера")[1], {
      target: { value: "harmful" },
    });
