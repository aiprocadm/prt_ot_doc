...
import type { WorkflowInstance, WorkflowInstanceListItem, WorkflowTask } from "@/features/workflow/types";
  ...
  <div className="space-y-6">
    <Card>
      ...
      <CardContent className="space-y-3">
        {!loading && !hasError && instances.length === 0 ? <EmptyState title="Нет экземпляров процессов" description="После запуска здесь появятся активные и завершённые экземпляры." /> : null}
        {instances.map((instance) => (
          <div key={instance.id} className="rounded border p-3">
            <div className="flex items-center justify-between gap-2">
              ...
              <Button size="sm" variant="outline" onClick={() => onOpenInstance(instance.id)}>Открыть</Button>
            </div>
          </div>
        ))}
      </CardContent>
    ...
    <Card>
      ...
      <CardContent className="space-y-3">
        ...
        {!loading && !hasError && tasks.length === 0 ? <EmptyState title="Нет задач процесса" description="Когда процесс дойдёт до узлов-задач, здесь появятся активные поручения." /> : null}
        {tasks.map((task) => (
          <div key={task.id} className="rounded border p-3">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              <Can permission={PERMISSIONS.WORKFLOW_MANAGE}>
                <Button size="sm" onClick={() => onCompleteTask(task.id)}>Завершить</Button>
                <Button size="sm" variant="outline" onClick={() => onMoveTask(task.id, "reassign")}>Переназначить</Button>
                <Button size="sm" variant="outline" onClick={() => onMoveTask(task.id, "delegate")}>Делегировать</Button>
                <Button size="sm" variant="outline" onClick={() => onMoveTask(task.id, "escalate")}>Эскалировать</Button>
              </Can>
              <Button size="sm" variant="outline" onClick={() => onOpenInstance(task.instance_id)}>Хронология</Button>
            </div>
          </div>
        ))}
      </CardContent>
