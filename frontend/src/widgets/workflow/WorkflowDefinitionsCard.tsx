...
export const WorkflowDefinitionsCard = ({ loading, hasError, definitions, onStart, onPublish, onArchive }: Props) => (
  <Card>
    <CardHeader><CardTitle>Процессы и версии</CardTitle></CardHeader>
    <CardContent className="space-y-4">
      {!loading && !hasError && definitions.length === 0 ? <EmptyState title="Нет описаний процессов" description="Создайте черновик процесса, чтобы опубликовать первую схему." /> : null}
        ...
        <div key={definition.id} className="rounded-lg border p-4 space-y-3">
          <div className="flex items-center justify-between gap-4">
            ...
            <Can permission={PERMISSIONS.WORKFLOW_MANAGE}>
              <Button size="sm" onClick={() => onStart(definition.code)}>Запустить</Button>
            </Can>
          </div>
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {version.status !== "published" ? (
                    <Can permission={PERMISSIONS.WORKFLOW_MANAGE}>
                      <Button size="sm" variant="outline" onClick={() => onPublish(version.id)}>Опубликовать</Button>
                    </Can>
                  ) : null}
                  {version.status !== "archived" ? (
                    <Can permission={PERMISSIONS.WORKFLOW_MANAGE}>
                      <Button size="sm" variant="ghost" onClick={() => onArchive(version.id)}>В архив</Button>
                    </Can>
                  ) : null}
                </div>
              </div>
