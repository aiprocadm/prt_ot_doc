...
const idempotencyKeyForTemplateUpload = async (
  templateId: string,
  file: File,
) => {
...
export const templatesApi = {
  uploadVersion: async (templateId: string, file: File): Promise<void> => {
    ...
    const idempotencyKey = await idempotencyKeyForTemplateUpload(
      templateId,
      file,
    );
  ...
  lintVersion: async (
    ...
    const { data } = await apiClient.post<Record<string, unknown>>(
      `/templates/${templateId}/versions/${versionId}:lint`,
      {
        required_fields: [],
      },
    );
  ...
  ): Promise<{ docx_url?: string }> => {
    const { data } = await apiClient.post<{ docx_url?: string }>(
