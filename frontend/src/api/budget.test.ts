...
describe("budgetApi", () => {
  ...
  it("attaches an expense to a reimbursement", async () => {
    ...
    expect(apiClient.post).toHaveBeenCalledWith(
      "/budget/reimbursements/r1/items",
      { expense_id: "e1" },
    );
  });
  ...
  it("detaches an expense from a reimbursement", async () => {
    ...
    expect(apiClient.delete).toHaveBeenCalledWith(
      "/budget/reimbursements/r1/items/e1",
    );
  });
  ...
  it("posts an FSM action with an empty body by default", async () => {
    ...
    expect(apiClient.post).toHaveBeenCalledWith(
      "/budget/reimbursements/r1/submit",
      {},
    );
  });

  it("posts approved_amount on approve and decision_reason on reject", async () => {
    await budgetApi.reimbursementAction("r1", "approve", {
      approved_amount: 5000,
    });
    expect(apiClient.post).toHaveBeenCalledWith(
      "/budget/reimbursements/r1/approve",
      {
        approved_amount: 5000,
      },
    );

    await budgetApi.reimbursementAction("r1", "reject", {
      decision_reason: "нет подтверждающих",
    });
    expect(apiClient.post).toHaveBeenCalledWith(
      "/budget/reimbursements/r1/reject",
      {
        decision_reason: "нет подтверждающих",
      },
    );
