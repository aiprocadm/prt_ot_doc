...
export const workflowApi = {
  ...
  getInstances: async (): Promise<WorkflowInstanceListItem[]> => {
    const { data } = await apiClient.get<WorkflowInstanceListItem[]>(
      "/workflow/instances",
    );
    return data;
  },
  ...
  ): Promise<{ node_types?: string[] }> => {
    const { data } = await apiClient.post<{ node_types?: string[] }>(
  ...
  completeTask: async (taskId: string): Promise<void> => {
    await apiClient.post(`/workflow/tasks/${taskId}/complete`, {
      decision: "approve",
