...
export const workPermitsApi = {
  ...
  async addMember(
    id: string,
    body: { person_id: string; role: string },
  ): Promise<WorkPermitMemberDto> {
    return (
      await apiClient.post<WorkPermitMemberDto>(`${base}/${id}/members`, body)
    ).data;
  },
  ...
  async readiness(id: string): Promise<ReadinessReportDto> {
    return (await apiClient.get<ReadinessReportDto>(`${base}/${id}/readiness`))
      .data;
  },
  async events(id: string): Promise<WorkPermitEventDto[]> {
    return (await apiClient.get<WorkPermitEventDto[]>(`${base}/${id}/events`))
      .data;
  },
  async action(
    id: string,
    ...
    return (await apiClient.post<WorkPermitDto>(`${base}/${id}/${name}`, body))
      .data;
  },
  async extend(id: string, planned_end: string): Promise<WorkPermitDto> {
    return (
      await apiClient.post<WorkPermitDto>(`${base}/${id}/extend`, {
        planned_end,
      })
    ).data;
  },
  async getBriefings(id: string): Promise<WorkPermitBriefingDto[]> {
    return (
      await apiClient.get<WorkPermitBriefingDto[]>(`${base}/${id}/briefing`)
    ).data;
  },
  async createBriefing(
    ...
    return (
      await apiClient.post<WorkPermitBriefingDto>(
        `${base}/${id}/briefing`,
        body,
      )
    ).data;
  ...
  async updateBriefing(
    ...
    return (
      await apiClient.patch<WorkPermitBriefingDto>(
        `${base}/${id}/briefing/${briefingId}`,
        body,
      )
    ).data;
  },
  async listSignatures(id: string): Promise<WorkPermitSignatureDto[]> {
    return (
      await apiClient.get<WorkPermitSignatureDto[]>(`${base}/${id}/signatures`)
    ).data;
  },
  async createPermitSignature(
    id: string,
    body: { person_id: string; mode: "attested" | "code" },
  ): Promise<WorkPermitSignatureDto> {
    return (
      await apiClient.post<WorkPermitSignatureDto>(
        `${base}/${id}/signatures`,
        body,
      )
    ).data;
  ...
  async createBriefingSignature(
    ...
    return (
      await apiClient.post<WorkPermitSignatureDto>(
        `${base}/${id}/briefing/${briefingId}/signatures`,
        body,
      )
    ).data;
  },
  ...
  async getClosing(id: string): Promise<WorkPermitClosingSummaryDto> {
    return (
      await apiClient.get<WorkPermitClosingSummaryDto>(`${base}/${id}/closing`)
    ).data;
  },
  async recordCompletion(
    id: string,
    completionText: string,
  ): Promise<WorkPermitClosingSummaryDto> {
    return (
      await apiClient.post<WorkPermitClosingSummaryDto>(
        `${base}/${id}/closing`,
  ...
  async listAdmissions(id: string): Promise<WorkPermitDailyAdmissionDto[]> {
    return (
      await apiClient.get<WorkPermitDailyAdmissionDto[]>(
        `${base}/${id}/admissions`,
      )
    ).data;
  },
  async createAdmission(
    ...
    return (
      await apiClient.post<WorkPermitDailyAdmissionDto>(
        `${base}/${id}/admissions`,
        body,
      )
    ).data;
  ...
  async updateAdmission(
    ...
    return (
      await apiClient.patch<WorkPermitDailyAdmissionDto>(
        `${base}/${id}/admissions/${admissionId}`,
        body,
      )
    ).data;
  },
...
export async function fetchAllPersons(): Promise<PersonOption[]> {
  const r = await apiClient.get<{ items: Array<Record<string, unknown>> }>(
    "/persons",
    {
