...
describe("committeesApi срез-4", () => {
  ...
  it("putInvitations шлёт person_ids на канонический путь", async () => {
    mock.onPut("/committees/meetings/m1/invitations").reply((config) => {
      expect(JSON.parse(config.data as string)).toEqual({ person_ids: ["p1", "p2"] });
      return [
        200,
        [{ id: "i1", meeting_id: "m1", person_id: "p1", invited_at: "2026-08-03T10:00:00Z" }]
      ];
    });
  ...
  it("createCommittee передаёт порог кворума", async () => {
    ...
    await committeesApi.createCommittee({ kind: "osms", name: "К", quorum_threshold_pct: 66 });
  });
