...
const kppOrEmpty = trimStr.refine((val) => val === "" || /^\d{9}$/.test(val), "КПП — 9 цифр");
...
const optionalEmail = trimStr.pipe(z.union([z.literal(""), z.string().email("Некорректный email")]));

const optionalWebsite = trimStr.refine(
