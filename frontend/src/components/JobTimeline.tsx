      ...
      >
        <div className="flex items-center justify-between">
          <div className="flex items-start gap-2">
            <div className="mt-0.5 rounded bg-muted px-1.5 py-0.5 text-xs">
              {idx + 1}
            </div>
            <div>
          ...
          <div className="flex items-center gap-2">
            <span
              className={`rounded px-2 py-0.5 text-xs ${badgeClass(step.status)}`}
            >
              {step.status}
            </span>
            <span className="text-xs text-muted-foreground">
              {durationText(step.started_at, step.ended_at)}
            </span>
          </div>
        </div>
        ...
        {step.error_code ? (
          <div className="text-xs text-red-600">{step.error_code}</div>
        ) : null}
        {step.error_payload ? (
          <pre className="overflow-auto rounded bg-red-50 p-2 text-[11px] text-red-800">
            {stringify(step.error_payload)}
          </pre>
        ) : null}
        {(step as { input?: unknown }).input ? (
          <pre className="overflow-auto rounded bg-slate-50 p-2 text-[11px]">
            {stringify((step as { input?: unknown }).input)}
          </pre>
        ) : null}
        {(step as { output?: unknown }).output ? (
          <pre className="overflow-auto rounded bg-muted/40 p-2 text-[11px]">
            {stringify((step as { output?: unknown }).output)}
          </pre>
        ) : null}
        {extractArtifacts((step as { output?: unknown }).output).length ? (
          <div className="space-y-1 text-xs">
            <div className="font-medium">Артефакты</div>
            {extractArtifacts((step as { output?: unknown }).output).map(
              (artifact) => (
                <div
                  key={`${step.step_run_id}-${artifact.key}`}
