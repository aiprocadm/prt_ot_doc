...
const mapPresetToForm = (preset?: LayoutPresetDto | null): FormState => {
  ...
  return {
    ...
    watermark_text: String((preset.watermark?.text as string | undefined) ?? ""),
...
export const LayoutPresetEditor = () => {
  ...
  const [preview, setPreview] = useState<{ warnings: string[]; unresolved: string[] }>({
    warnings: [],
    unresolved: []
  });
  ...
  const runPreview = async () => {
    const unresolved = [
      ...[form.header_first_xml, form.header_odd_xml, form.header_even_xml, form.footer_first_xml, form.footer_odd_xml, form.footer_even_xml]
        .join("\n")
        .match(/\{\{[^}]+\}\}/g) ?? []
    ];
    setPreview({
      warnings: [
        form.different_odd_even ? "even/odd mode enabled" : "single odd/default mode",
        form.watermark_enabled ? `watermark=${form.watermark_text || "enabled"}` : "watermark disabled"
      ],
      unresolved
    });
  };
    ...
    <div className="space-y-4">
      ...
      <div className="grid gap-3 md:grid-cols-4">
        <label className="flex items-center gap-2 text-sm">
          <input type="checkbox" checked={form.different_first} onChange={(e) => setForm((s) => ({ ...s, different_first: e.target.checked }))} />
          different first page
        </label>
        <label className="flex items-center gap-2 text-sm">
          <input type="checkbox" checked={form.different_odd_even} onChange={(e) => setForm((s) => ({ ...s, different_odd_even: e.target.checked }))} />
          different odd/even
        </label>
        <label className="flex items-center gap-2 text-sm">
      ...
      <div className="flex gap-2">
        ...
        <Button variant="outline" onClick={() => void runPreview()}>Проверить токены</Button>
        <Button variant="ghost" onClick={() => { setSelectedId(""); setForm(emptyForm); }}>Новый</Button>
      </div>
