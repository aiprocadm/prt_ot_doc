...
export const ConflictInboxCard = ({ onConflictStateChange }: { onConflictStateChange?: (hasConflict: boolean) => void }) => {
  ...
  const resolve = async (batchId: string, strategy: "server_wins" | "client_retry") => {
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-3">
        {loading ? <p className="text-xs text-muted-foreground">Обновление конфликтов…</p> : null}
        {!loading && items.length === 0 ? <p className="text-xs text-muted-foreground">Конфликтов нет.</p> : null}
        {items.map((item) => (
          <div key={item.id} className="rounded-md border p-3 text-sm space-y-2">
            <div className="font-medium">{item.entity_type}</div>
            <div className="text-xs text-muted-foreground">{item.conflict_code}</div>
            <div className="flex flex-wrap gap-2">
              <Button size="sm" variant="outline" className="min-h-11 px-4" onClick={() => void resolve(item.id, "server_wins")}>
                Server wins
              </Button>
              <Button size="sm" className="min-h-11 px-4" onClick={() => void resolve(item.id, "client_retry")}>
                Retry client
              </Button>
            </div>
          </div>
