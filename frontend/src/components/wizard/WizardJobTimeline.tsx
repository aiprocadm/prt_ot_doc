...
export const WizardJobTimeline = memo(
  ({ steps }: { steps: PipelineStepRun[] }) => (
      ...
      {steps.map((step) => (
        ...
        >
          <div>
            ...
            {step.error_code ? (
              <div className="text-xs text-destructive">{step.error_code}</div>
            ) : null}
          </div>
          <div className="flex items-center gap-2">
            <span
              className={`rounded px-2 py-0.5 text-xs ${statusClassMap[step.status] ?? "bg-zinc-100 text-zinc-700"}`}
            >
              {step.status}
            </span>
