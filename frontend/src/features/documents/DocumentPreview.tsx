...
export const DocumentPreview = ({
  ...
  useEffect(() => {
    setCurrent(document);
    releaseApi
      .documentReleaseStatus(document.current_version_id ?? document.id)
      .then(setRelease)
      .catch(() => undefined);
    approvalsApi
      .listMyTasks("pending")
  ...
  const handleQuickApprove = async () => {
    ...
    try {
      ...
      releaseApi
        .documentReleaseStatus(releaseTargetId)
        .then(setRelease)
        .catch(() => undefined);
    } catch {
      ...
      <CardHeader className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            {current.type} • Версия {current.version} •{" "}
      ...
      <CardContent>
        ...
        >
          <div className="flex flex-wrap items-center justify-between gap-2">
            <div className="text-sm font-semibold">
              Готовность к выпуску (readiness)
            </div>
            {readinessLoading ? (
            ...
            <div className="mt-3 space-y-3 text-sm">
              <div className="h-2 w-full overflow-hidden rounded-full bg-muted">
                <div
                  ...
                  style={{
                    width: `${Math.min(100, Math.max(0, readiness.score))}%`,
                  }}
                />
              </div>
              ...
              {readiness.pipeline_stages &&
              readiness.pipeline_stages.length > 0 ? (
        ...
        <div className="mb-4 grid gap-2 rounded-md border p-3 text-sm sm:grid-cols-3">
          <div>
            ...
            <Badge variant="secondary">{release.approval}</Badge>
          </div>
          <div>
            <span className="text-muted-foreground">Signature:</span>{" "}
            <Badge variant="secondary">{release.signature}</Badge>
          </div>
          <div>
            <span className="text-muted-foreground">EDO:</span>{" "}
            <Badge variant="secondary">{release.edo}</Badge>
          </div>
        </div>
        <div className="mb-4 flex flex-wrap gap-2">
          <ActionButton
            ...
            disabled={!myTaskId}
            title={
              myTaskId ? undefined : "Нет назначенной вам задачи согласования"
            }
