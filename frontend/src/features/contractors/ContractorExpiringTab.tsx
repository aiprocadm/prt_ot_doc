...
export const ContractorExpiringTab = () => {
  const loader = useCallback(
    () => contractorsApi.listExpiringDocuments().then((p) => p.items),
    [],
  );
  ...
  const registry = useLocalRegistry({
    items: data,
    match: (item, query) =>
      [item.title, item.number, item.doc_type]
        .filter(Boolean)
        .join(" ")
        .toLowerCase()
        .includes(query),
  ...
  return (
    <div className="space-y-3">
      ...
      {!loading && !error && registry.total === 0 ? (
        <EmptyState
          title="Нет истекающих документов"
          description="Все документы подрядчиков в порядке."
        />
      ) : null}
      {!loading && !error && registry.total > 0 ? (
        <RegistryTable
