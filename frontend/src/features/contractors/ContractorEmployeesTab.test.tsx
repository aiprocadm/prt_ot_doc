...
describe("ContractorEmployeesTab", () => {
  it("lists employees scoped by contractor", async () => {
    render(
      <ContractorEmployeesTab contractorId="c1" onChanged={() => undefined} />,
    );
    expect(await screen.findByText("Сидоров С.С.")).toBeInTheDocument();
    expect(contractorsApi.listEmployees).toHaveBeenCalledWith({
      contractor_id: "c1",
    });
  });
  ...
  it("creates an employee for the contractor", async () => {
    ...
    fireEvent.click(
      await screen.findByRole("button", { name: "Добавить сотрудника" }),
    );
    fireEvent.change(await screen.findByLabelText("ФИО"), {
      target: { value: "Новый Н.Н." },
    });
