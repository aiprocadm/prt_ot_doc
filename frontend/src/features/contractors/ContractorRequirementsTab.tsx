...
export const ContractorRequirementsTab = () => {
  const loader = useCallback(
    () => contractorsApi.listRequirements().then((p) => p.items),
    [],
  );
  ...
  if (error && isFeatureDisabledError(error)) {
    return (
      <EmptyState
        ...
        description="Требования к документам не включены для этого тенанта."
      />
    );
  }
    ...
    <div className="space-y-3">
      <div className="flex justify-end">
        <Can permission={PERMISSIONS.CONTRACTOR_MANAGE}>
          <DocumentRequirementFormDialog
            trigger={<Button>Новое требование</Button>}
      ...
      {!loading && !error && data.length === 0 ? (
        <EmptyState
          ...
          description="Добавьте требование к документам подрядчиков."
        />
      ) : null}
      {!loading && !error && data.length > 0 ? (
            ...
            >
              <div className="flex items-center gap-2 text-sm">
                <span className="font-medium">
                  {DOC_TYPE_LABELS[req.doc_type] ?? req.doc_type}
                </span>
                <Badge variant="secondary">
                  {SCOPE_LABELS[req.scope] ?? req.scope}
                </Badge>
                {req.mandatory ? (
                  <Badge>Обязательный</Badge>
                ) : (
                  <Badge variant="secondary">Необязательный</Badge>
                )}
              </div>
              <Can permission={PERMISSIONS.CONTRACTOR_MANAGE}>
                <Button
