...
describe("ContractorIncidentsTab", () => {
  it("lists incidents scoped by contractor", async () => {
    ...
    expect(contractorsApi.listIncidents).toHaveBeenCalledWith({
      contractor_id: "c1",
    });
  });
  ...
  it("registers an incident", async () => {
    render(<ContractorIncidentsTab contractorId="c1" />);
    fireEvent.click(
      await screen.findByRole("button", { name: "Зарегистрировать инцидент" }),
    );
    fireEvent.change(await screen.findByLabelText("Тип инцидента"), {
      target: { value: "порез" },
    });
