...
const extractViolations = (err: unknown): string[] => {
  const details = (
    err as { details?: { details?: Array<{ violations?: unknown[] }> } }
  )?.details;
...
export const EmployeeAdmissionDialog = ({
  employee,
  trigger,
  onAdmitted,
}: Props) => {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    Promise.all([
      contractorsApi.getEmployeeReadiness(employee.id),
      contractorsApi.getEmployeeChecklist(employee.id),
    ])
      .then(([r, c]) => {
        setReadiness(r);
  ...
  const onAdmit = async () => {
    ...
    try {
      ...
      if (e.status === 409 || e.code === "requirements_not_met") {
        ...
        setBlockedViolations(
          violations.length ? violations : ["Не выполнены требования допуска"],
        );
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Проверка готовности документов и оформление допуска сотрудника.
          </DialogDescription>
        </DialogHeader>
        ...
        {!loading ? (
          <div className="space-y-2">
            ...
            {checklist.length === 0 ? (
              <p className="text-sm text-muted-foreground">
                Требования к документам не настроены.
              </p>
            ) : (
              <ul className="divide-y rounded-md border text-sm">
                {checklist.map((item) => (
                  <li
                    key={`${item.doc_type}-${item.scope}`}
                  ...
                  >
                    ...
                    <Badge variant={EXPIRY_BADGE_VARIANT[item.status]}>
                      {EXPIRY_LABELS[item.status]}
                    </Badge>
                  </li>
                ))}
              </ul>
        ...
        {verdict ? (
          <p className="text-sm text-green-700">
            {ADMISSION_STATUS_LABELS[verdict.status]}
          </p>
        ) : null}
