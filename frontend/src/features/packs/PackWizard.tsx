...
const presets: { label: string; value: PackPreset; description: string }[] = [
  {
    ...
    value: "site_entry",
    description: "Документы для допуска к объекту",
  },
  {
    label: "Несчастный случай",
    value: "incident_response",
    description: "Пакет при расследовании",
  },
  {
  ...
  },
  {
    label: "Экология",
    value: "environmental",
    description: "Экологическая документация",
  },
];
      ...
      <CardContent className="space-y-6">
        ...
        {step === 1 && (
          <div className="space-y-4">
            ...
            <Button
              disabled={!companyId}
              title={
                !companyId ? "Выберите компанию, чтобы продолжить" : undefined
              }
              onClick={() => setStep(2)}
            >
              Далее
        ...
        {step === 2 && (
          <div className="space-y-4">
            ...
            <div className="flex gap-2">
              ...
              <Button
                disabled={!preset}
                title={!preset ? "Выберите пресет" : undefined}
                onClick={() => setStep(3)}
              >
                Далее
              </Button>
        ...
        {step === 4 && (
          <div className="space-y-2 text-sm" aria-live="polite">
            <p className="rounded-md border border-emerald-500/40 bg-emerald-500/10 px-3 py-2 text-emerald-700">
              Задача создана. Статус можно отслеживать в разделе «Задачи».
            </p>
...
const StepIndicator = ({
  ...
  <div
    className={`flex items-center gap-2 rounded-full border px-3 py-1 ${step <= current ? "border-primary text-primary" : "border-muted-foreground/30"}`}
  >
    {icon}
