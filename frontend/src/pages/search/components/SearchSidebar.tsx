...
export const SearchSidebar = ({ recent, saved, onApplySavedSearch, onDeleteSavedSearch }: Props) => (
  <div className="space-y-4">
    <Card>
      ...
      <CardContent className="space-y-2">
        {recent.map((item) => (
          <button key={item.id} type="button" className="flex w-full items-start gap-2 rounded border p-3 text-left hover:bg-muted" onClick={() => onApplySavedSearch(item)}>
        ...
        {saved.map((item) => (
          <div key={item.id} className="rounded border p-3">
            <div className="flex items-start justify-between gap-2">
              <button type="button" className="text-left" onClick={() => onApplySavedSearch(item)}>
              ...
              <Button variant="ghost" size="icon" onClick={() => void onDeleteSavedSearch(item.id)}>
