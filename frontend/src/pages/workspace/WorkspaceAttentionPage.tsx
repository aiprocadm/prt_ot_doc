...
const quickLinks = [
  {
    to: "/tasks",
    label: "Задачи и сроки",
    icon: ClipboardList,
    description: "Единый inbox обязательств и напоминаний.",
  },
  {
    to: "/notifications",
    label: "Уведомления",
    icon: Bell,
    description: "События и эскалации по ролям.",
  },
  {
    to: "/calendar",
    label: "Календарь",
    icon: CalendarDays,
    description: "Дедлайны и контрольные точки.",
  },
  {
    to: "/search",
    label: "Поиск",
    icon: Search,
    description: "Быстрый доступ к сущностям и файлам.",
  },
];
...
const attentionWidgets: Array<{
...
}> = [
  {
    ...
    description:
      "Сотрудники с истекшими сроками обучения и назначениями к обновлению.",
  ...
  },
  {
    ...
    description:
      "Персонал с просроченными или истекающими медицинскими допусками.",
  ...
  },
  {
    ...
    description:
      "Ближайшие инспекции, открытые предписания и незакрытые корректирующие действия.",
...
export default function WorkspaceAttentionPage() {
  ...
  const availableWidgets = attentionWidgets.filter((widget) =>
    widget.permissions.every((permission) => can(permission)),
  );
    ...
    <div className="space-y-6 p-6">
      ...
      <div>
        ...
        <p className="text-muted-foreground mt-1 max-w-2xl text-sm">
          Сводка рисков, блокеров готовности и задач — те же данные, что на
          дашборде. Ниже — быстрые переходы в смежные разделы (доступ по ролям).
        </p>
      </div>
        ...
        <div>
          ...
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            {availableWidgets.map(
              ({ title, description, to, cta, icon: Icon }) => (
                <Card
                  key={title}
