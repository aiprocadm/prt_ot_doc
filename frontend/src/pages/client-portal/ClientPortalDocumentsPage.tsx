  ...
  return (
    <div className="space-y-6">
      ...
      {!loading && !error && !items.length ? (
        <EmptyState title="Пакеты отсутствуют" description="Список файлов станет доступен после публикации пакета." />
      ) : null}
      <div className="grid gap-6 lg:grid-cols-[0.9fr_1.4fr]">
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            {!loading && !error && selected?.files.length ? selected.files.map((file) => (
            ...
            {!loading && !error && items.length > 0 && !selected?.files.length ? (
