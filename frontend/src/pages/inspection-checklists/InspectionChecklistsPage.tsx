...
const InspectionChecklistsPage = () => {
  ...
  const items = useMemo(() => {
    const grouped = new Map<
      string,
      { code: string; inspections: number; prescriptions: number }
    >();
    ...
    data.prescriptions.forEach(() => {
      const current = grouped.get("general") ?? {
        code: "general",
        inspections: 0,
        prescriptions: 0,
      };
  ...
  const registry = useLocalRegistry({
    items,
    match: (item, query) => item.code.toLowerCase().includes(query),
  });
    ...
    <div className="space-y-4">
      ...
      {!loading && !error && hasChecklistGap ? (
        <Card className="border-orange-200 bg-orange-50/40">
          ...
          <CardContent className="space-y-3 text-sm">
            <p>
              Открытых предписаний: {openPrescriptions}. Шаблонов документов для
              покрытия чек-листов: {data.templates.length}.
            </p>
      ...
      {!loading && !error && registry.total === 0 ? (
        <EmptyState
          ...
          description="В тенанте ещё не было проверок."
        />
      ) : null}
      {!loading && !error && registry.total > 0 ? (
        <RegistryTable
