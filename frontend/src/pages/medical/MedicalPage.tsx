...
const MedicalPage = () => {
  ...
  const [contingentView, setContingentView] = useState<"register" | "named">(
    "register",
  );
  ...
  const onPrint = useCallback(
    async (fmt: "docx" | "pdf") => {
      ...
      try {
        ...
        setPrintError(
          mutationErrorText(err, "Не удалось сформировать печатную форму"),
        );
      } finally {
        setPrinting(false);
      }
  ...
  const referrals = useAsyncResource({
    loader: useCallback(
      () =>
        operationsApi.listMedicalReferrals(
          referralStatusFilter ? { status: referralStatusFilter } : undefined,
        ),
  ...
  const [transitioning, setTransitioning] = useState<ReadonlySet<string>>(
    new Set(),
  );
  ...
  const onGenerateReferrals = useCallback(async () => {
    ...
    try {
      ...
      setReferralError(
        mutationErrorText(err, "Не удалось сформировать направления"),
      );
    } finally {
      setGenerating(false);
    }
  ...
  const onCreateReferral = useCallback(async () => {
    ...
    try {
      await operationsApi.createMedicalReferral({
        ...
        ...(newReferral.medical_org_name.trim()
          ? { medical_org_name: newReferral.medical_org_name.trim() }
          : {}),
      });
      setNewReferral({
        person_id: "",
      ...
      setReferralError(
        mutationErrorText(err, "Не удалось создать направление"),
      );
    } finally {
  ...
  const onTransitionReferral = useCallback(
    async (
      ...
      try {
        ...
        setReferralError(
          mutationErrorText(err, "Не удалось изменить статус направления"),
        );
      } finally {
        setTransitioning((prev) => {
          const next = new Set(prev);
  ...
  const [printingReferralId, setPrintingReferralId] = useState<string | null>(
    null,
  );
  const onPrintReferral = useCallback(
    async (referralId: string, fmt: "docx" | "pdf") => {
      ...
      try {
        ...
        setReferralError(
          mutationErrorText(err, "Не удалось сформировать направление"),
        );
      } finally {
  ...
  const suspensions = useAsyncResource({
    loader: useCallback(
      () =>
        operationsApi.listMedicalSuspensions(
          suspensionsActiveOnly ? { status: "active" } : undefined,
        ),
  ...
  const onLiftSuspension = useCallback(
    async (suspensionId: string) => {
      if (
        !window.confirm("Снять отстранение? Работник будет допущен к работе.")
      )
        return;
      ...
      try {
        ...
        setSuspensionError(
          mutationErrorText(err, "Не удалось снять отстранение"),
        );
      } finally {
        setLiftingId(null);
      }
  ...
  const items = useMemo(
    () =>
      data.exams.map((exam) => ({
        ...exam,
        person: data.persons.find((person) => person.id === exam.person_id),
      })),
    [data],
  );
  const registry = useLocalRegistry({
    items,
    match: (item, query) =>
      [item.person?.full_name, item.exam_type, item.conclusion]
        .filter(Boolean)
        .join(" ")
        .toLowerCase()
        .includes(query),
  ...
  return (
    <div className="space-y-4">
      <RegistryPageHeader
        ...
        stats={[
          {
            label: "Позиций контингента",
            value: oversight.data.summary?.total ?? "—",
          },
          {
            label: "Просрочено/отсутствует",
            value: oversight.data.summary?.overdue_count ?? "—",
          },
          {
            label: "Активных отстранений",
            value: oversight.data.summary?.suspended_count ?? "—",
          },
        ]}
      />
      ...
      {!loading && !error && registry.total === 0 ? (
        <EmptyState
          title="Медосмотры не найдены"
          description="Добавьте записи медосмотров или создайте требования."
        />
      ) : null}
      {!loading && !error && registry.total > 0 ? (
        <RegistryTable
          columns={[
            {
              ...
              header: "Сотрудник",
              cell: ({ row }) =>
                row.original.person?.full_name || row.original.person_id,
            },
      ...
      <section className="rounded-lg border border-border p-4 space-y-3">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Реестр по должностям (29н/342н) и поимённый список; печатные формы
              DOCX/PDF.
            </p>
          </div>
        ...
        {contingentView === "register" ? (
          oversight.data.register.length > 0 ? (
              ...
              <TableBody>
                {oversight.data.register.map((row) => (
                  <TableRow key={row.position_id}>
                    ...
                    <TableCell>
                      {row.exam_kinds.map(examKindLabel).join(", ")}
                    </TableCell>
            ...
            <p className="text-sm text-muted-foreground">
              Контингент пуст — настройте нормы и факторы.
            </p>
          )
            ...
            <TableBody>
              {oversight.data.namedList.map((row) => (
                <TableRow key={row.person_id}>
                  ...
                  <TableCell>
                    {row.required_kinds.map(examKindLabel).join(", ")}
                  </TableCell>
                  <TableCell>
                    {row.last_exam_date ? formatDate(row.last_exam_date) : "—"}
                  </TableCell>
                  <TableCell>
      ...
      <section className="rounded-lg border border-border p-4 space-y-3">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Выдача направлений вручную или по контингенту; статусы: выдан →
              запланирован → завершён/отменён.
            </p>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <select
        ...
        <ErrorState
          error={referrals.error ?? undefined}
          onRetry={() => void referrals.reload()}
        />
        {referralError ? (
          <p role="alert" className="text-sm text-destructive">
            {referralError}
        ...
        {generatedCount !== null ? (
          <p className="text-sm">Создано направлений: {generatedCount}</p>
        ) : null}
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <select
            ...
            onChange={(e) =>
              setNewReferral((f) => ({ ...f, person_id: e.target.value }))
            }
          >
          ...
          <select
            ...
            onChange={(e) =>
              setNewReferral((f) => ({ ...f, exam_kind: e.target.value }))
            }
          >
          ...
          <input
            ...
            onChange={(e) =>
              setNewReferral((f) => ({ ...f, due_at: e.target.value }))
            }
          />
          <input
            ...
            onChange={(e) =>
              setNewReferral((f) => ({
                ...f,
                medical_org_name: e.target.value,
              }))
            }
          />
        </div>
            ...
            <TableBody>
              {referrals.data.map((referral) => (
                <TableRow key={referral.id}>
                  ...
                  <TableCell>
                    {referral.due_at ? formatDate(referral.due_at) : "—"}
                  </TableCell>
                  ...
                  <TableCell>
                    <div className="flex flex-wrap items-center gap-2">
                      {referral.status === "issued" ? (
                        <button
                          ...
                          onClick={() =>
                            void onTransitionReferral(referral.id, "scheduled")
                          }
                      ...
                      referral.status === "scheduled" ? (
                        <button
                          ...
                          className="rounded-md border border-border px-2 py-1 text-xs"
                          onClick={() =>
                            void onTransitionReferral(referral.id, "cancelled")
                          }
                          disabled={transitioning.has(referral.id)}
                        >
                          Отменить
                      ...
                      <button
                        ...
                        className="rounded-md border border-border px-2 py-1 text-xs"
                        onClick={() =>
                          void onPrintReferral(referral.id, "docx")
                        }
                    ...
                    {completingId === referral.id
                      ? (() => {
                          const personExams = data.exams.filter(
                            (exam) => exam.person_id === referral.person_id,
                          );
                          if (personExams.length === 0) {
                            return (
                              <p className="mt-2 text-xs text-muted-foreground">
                                Сначала зафиксируйте осмотр в реестре выше.
                              </p>
                            );
                          }
                          return (
      ...
      <section className="rounded-lg border border-border p-4 space-y-3">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Автоматические отстранения по результатам осмотров; снятие —
              только admin/owner.
            </p>
          </div>
        ...
        <ErrorState
          error={suspensions.error ?? undefined}
          onRetry={() => void suspensions.reload()}
        />
        {suspensionError ? (
          <p role="alert" className="text-sm text-destructive">
            {suspensionError}
            ...
            <TableBody>
              {suspensions.data.map((suspension) => (
                <TableRow key={suspension.id}>
                  ...
                  <TableCell>
                    {SUSPENSION_REASON_LABELS[suspension.reason] ??
                      suspension.reason}
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell>
                    {examLabelById(suspension.source_exam_id)}
                  </TableCell>
                  <TableCell>
      ...
      <section className="rounded-lg border border-border p-4 space-y-3">
        <div className="flex items-center justify-between gap-2">
          <div>
            <h2 className="text-base font-semibold">
              Психиатрическое освидетельствование (342н)
            </h2>
            <p className="text-sm text-muted-foreground">
              Виды деятельности по перечню ПП РФ № 695 и подлежащий контингент.
            </p>
          </div>
        ...
        <ErrorState
          error={psych.error ?? undefined}
          onRetry={() => void psych.reload()}
        />
        {psych.data.activityTypes.length > 0 ? (
            ...
            {psych.data.activityTypes.map((a) => (
              ...
              >
                ...
                <span className="text-muted-foreground">
                  {Math.round(a.interval_days / 365)} лет
                </span>
              </li>
            ))}
          </ul>
          ...
          <p className="text-sm text-muted-foreground">
            Каталог видов деятельности пуст.
          </p>
        )}
        <div className="text-sm">
          Подлежит освидетельствованию (контингент):{" "}
          <strong>{psych.data.contingent.length}</strong>
        </div>
      </section>
    </div>
  );
