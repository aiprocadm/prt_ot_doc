...
const NotificationsPage = () => {
  ...
  const [statusFilter, setStatusFilter] = useState<(typeof statuses)[number]>(
    (statuses.find((item) => item === searchParams.get("status")) ??
      "unread") as (typeof statuses)[number],
  );
  const [channel, setChannel] = useState<(typeof channels)[number]>(
    (channels.find((item) => item === searchParams.get("channel")) ??
      "all") as (typeof channels)[number],
  );
  const [priority, setPriority] = useState<(typeof priorities)[number]>(
    (priorities.find((item) => item === searchParams.get("priority")) ??
      "all") as (typeof priorities)[number],
  );
  ...
  const patchQuery = useCallback(
    (patch: {
      ...
      if ("channel" in patch) {
        if (patch.channel && patch.channel !== "all")
          next.set("channel", patch.channel);
        else next.delete("channel");
      }
      if ("priority" in patch) {
        if (patch.priority && patch.priority !== "all")
          next.set("priority", patch.priority);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError({
        ...
        message: apiError.message ?? "Не удалось загрузить уведомления",
      });
    } finally {
      setLoading(false);
    }
  ...
  const loadSettings = useCallback(async () => {
    try {
      const response =
        await notificationsApi.getMySettings<NotificationSettings>();
  ...
  const loadTemplates = useCallback(async () => {
    try {
      const response =
        await notificationsApi.listTemplates<NotificationTemplate>();
  ...
  const selectedUnreadIds = useMemo(
    () =>
      items
        .filter((item) => selectedIds.includes(item.id) && !item.is_read)
        .map((item) => item.id),
    [items, selectedIds],
  );
  ...
  const markRead = async (ids: string[]) => {
    ...
    try {
      ...
      setError({
        status: apiError.status ?? 500,
        message:
          apiError.message ?? "Не удалось отметить уведомления как прочитанные",
      });
    }
  };
  ...
  const saveSettings = async () => {
    ...
    try {
      ...
      setError({
        status: apiError.status ?? 500,
        message:
          apiError.message ?? "Не удалось сохранить настройки уведомлений",
      });
    }
  };
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Button
              variant="outline"
              onClick={() => void markRead(selectedUnreadIds)}
        ...
        <CardContent className="space-y-4">
          ...
          {!loading
            ? items.map((item) => (
                <div
                  key={item.id}
                  className={`rounded-md border p-3 ${item.is_read ? "bg-muted/30" : "border-primary/40"}`}
                >
                  <div className="flex items-start gap-3">
                    <input
                      ...
                      aria-label={`Выбрать уведомление ${item.id}`}
                    />
                      ...
                      <div className="mt-2 flex flex-wrap items-center gap-2 text-xs">
                        ...
                        {item.deeplink || item.payload?.deeplink ? (
                          <Link
                            ...
                            to={item.deeplink ?? item.payload?.deeplink ?? "#"}
                          >
        ...
        <CardContent className="grid gap-3 md:grid-cols-2">
          <label className="flex items-center gap-2 text-sm">
            <input
              ...
              onChange={(e) =>
                setSettings((prev) =>
                  prev ? { ...prev, email_enabled: e.target.checked } : prev,
                )
              }
            />{" "}
            Электронная почта
          ...
          <label className="flex items-center gap-2 text-sm">
            <input
              ...
              onChange={(e) =>
                setSettings((prev) =>
                  prev ? { ...prev, telegram_enabled: e.target.checked } : prev,
                )
              }
            />{" "}
            Телеграм
          ...
          <label className="flex items-center gap-2 text-sm">
            <input
              type="checkbox"
              checked={Boolean(settings?.inapp_enabled)}
              onChange={(e) =>
                setSettings((prev) =>
                  prev ? { ...prev, inapp_enabled: e.target.checked } : prev,
                )
              }
            />{" "}
            В приложении
          ...
          <Input
            placeholder="Режим дайджеста: off / daily / weekly"
            value={settings?.digest_mode ?? ""}
            onChange={(e) =>
              setSettings((prev) =>
                prev ? { ...prev, digest_mode: e.target.value } : prev,
              )
            }
          />
          <Input
          ...
          />
          <Input
            placeholder="Тихие часы: с (например 22:00)"
            value={settings?.quiet_hours?.from ?? ""}
            onChange={(e) =>
              setSettings((prev) =>
                prev
                  ? {
          ...
          />
          <Input
            placeholder="Тихие часы: до (например 08:00)"
            value={settings?.quiet_hours?.to ?? ""}
            onChange={(e) =>
              setSettings((prev) =>
                prev
                  ? {
          ...
          />
          <Input
            placeholder="Часовой пояс (Europe/Moscow)"
            value={settings?.quiet_hours?.tz ?? ""}
            onChange={(e) =>
              setSettings((prev) =>
                prev
                  ? {
          ...
          />
          <Button className="md:col-span-2" onClick={() => void saveSettings()}>
            Сохранить настройки
          </Button>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          {templates.length ? (
            ...
            <div className="text-sm text-muted-foreground">
              Шаблоны пока не настроены — база для шаблонов в области тенанта
              готова.
            </div>
          )}
        </CardContent>
