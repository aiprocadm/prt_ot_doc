...
const FireInspectionsPage = () => {
  ...
  const registry = useLocalRegistry({
    ...
    match: (item, query) =>
      [item.authority, item.purpose, item.status, item.inspection_type]
        .filter(Boolean)
        .join(" ")
        .toLowerCase()
        .includes(query),
  });
  const overdueInspections = data.inspections.filter(
    (item) =>
      item.status !== "done" &&
      item.status !== "closed" &&
      Boolean(
  ...
  const openPrescriptions = data.prescriptions.filter(
    (item) => item.status !== "closed",
  ).length;
  const overdueTasks = data.tasks.filter((item) => item.overdue).length;
  const hasBlockers =
    overdueInspections > 0 || openPrescriptions > 0 || overdueTasks > 0;
    ...
    <div className="space-y-4">
      <RegistryPageHeader
        ...
        stats={[
          ...
          { label: "Предписаний", value: data.prescriptions.length },
          {
            ...
            value: data.tasks.filter((item) => item.status !== "done").length,
          },
        ]}
      />
      ...
      {!loading && !error && hasBlockers ? (
        <Card className="border-orange-200 bg-orange-50/40">
          ...
          <CardContent className="space-y-3 text-sm">
            <p>
              Открытые узкие места: предписания {openPrescriptions},
              просроченные задачи {overdueTasks}, просроченные проверки{" "}
              {overdueInspections}.
            </p>
            <div className="flex flex-wrap gap-2">
              <Button asChild size="sm" variant="outline">
                <Link to="/inspections">Проверки</Link>
              </Button>
              <Button asChild size="sm" variant="outline">
                <Link to="/tasks?type=inspection">Задачи проверок</Link>
              </Button>
              <Button asChild size="sm" variant="outline">
      ...
      {!loading && !error && registry.total === 0 ? (
        <EmptyState
          ...
          description="Создайте записи проверок или загрузите данные."
        />
      ) : null}
      {!loading && !error && registry.total > 0 ? (
        <RegistryTable
