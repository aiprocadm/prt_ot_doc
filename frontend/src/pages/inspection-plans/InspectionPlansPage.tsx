...
const InspectionPlansPage = () => {
  ...
  const overduePlanned = data.inspections.filter((item) => item.status !== "done" && item.status !== "closed" && Boolean(item.scheduled_at && new Date(item.scheduled_at).getTime() < Date.now())).length;
  const openPlanTasks = data.tasks.filter((item) => item.status !== "done").length;
        ...
        <Card className="border-orange-200 bg-orange-50/40">
          ...
          <CardContent className="space-y-3 text-sm">
            ...
            <div className="flex flex-wrap gap-2">
              <Button asChild size="sm" variant="outline"><Link to="/tasks?type=inspection">Открыть задачи</Link></Button>
              <Button asChild size="sm" variant="outline"><Link to="/inspections">Открыть реестр проверок</Link></Button>
            </div>
          </CardContent>
        </Card>
      ) : null}
