...
import type { AutomationRuleRead, EventTypeMeta, TriggerRead } from "@/types/dto/rules";
...
const RULE: AutomationRuleRead = {
  ...
  description: null,
  event_type: "IncidentCreated",
  conditions_json: { match: "all", conditions: [{ field: "severity", op: "eq", value: "critical" }] },
  actions_json: [
    {
      type: "notify",
      recipient_mode: "role",
...
beforeEach(() => {
  vi.clearAllMocks();
  (rulesApi.list as any).mockResolvedValue({ items: [RULE], total: 1, limit: 100, offset: 0 });
  (rulesApi.eventTypes as any).mockResolvedValue({ items: EVENT_TYPES, total: EVENT_TYPES.length });
  (rulesApi.triggers as any).mockResolvedValue({ items: [TRIGGER], total: 1, limit: 50, offset: 0 });
});
...
describe("RulesPage", () => {
  it("renders the rules registry with RU event labels and action badges", async () => {
    renderPage();
    expect((await screen.findAllByText("Критичные инциденты")).length).toBeGreaterThan(0);
  ...
  it("creates a rule through the form dialog", async () => {
    (rulesApi.create as any).mockResolvedValue({ ...RULE, id: "r2", name: "Тестовое правило" });
    renderPage();
    ...
    fireEvent.change(within(dialog).getByLabelText("Имя"), { target: { value: "Тестовое правило" } });
    fireEvent.change(within(dialog).getByLabelText("Событие"), { target: { value: "IncidentCreated" } });
    ...
    fireEvent.change(within(conditionRow).getByLabelText("Поле"), { target: { value: "severity" } });
    fireEvent.change(within(conditionRow).getByLabelText("Оператор"), { target: { value: "eq" } });
    fireEvent.change(within(conditionRow).getByLabelText("Значение"), { target: { value: "critical" } });
    ...
    fireEvent.change(within(actionCard).getByLabelText("Тип действия"), { target: { value: "notify" } });
    fireEvent.change(within(actionCard).getByLabelText("Получатель"), { target: { value: "role" } });
    fireEvent.click(within(actionCard).getByLabelText("Администратор"));
    fireEvent.change(within(actionCard).getByLabelText("Заголовок уведомления"), { target: { value: "Заголовок" } });
    fireEvent.change(within(actionCard).getByLabelText("Текст уведомления"), { target: { value: "Текст" } });
  ...
  it("runs a dry-run and shows the matched badge", async () => {
    (rulesApi.dryRun as any).mockResolvedValue({
      matched: true,
      condition_results: [{ field: "severity", op: "eq", value: "critical", actual: "critical", matched: true }],
      would_actions: ["уведомление (role) «Инцидент»"]
    });
    renderPage();
    await screen.findAllByText("Критичные инциденты");

    fireEvent.change(screen.getByLabelText("Правило"), { target: { value: "r1" } });
    ...
    expect(rulesApi.dryRun).toHaveBeenCalledWith(
      expect.objectContaining({
        rule: expect.objectContaining({ name: "Критичные инциденты", event_type: "IncidentCreated" }),
        event: { event_type: "IncidentCreated", payload: { severity: "critical" } }
      })
    );
    expect(screen.getByText("уведомление (role) «Инцидент»")).toBeInTheDocument();
  });
