    ...
    <section className="space-y-4">
      ...
      {!loading && !error && runs.length === 0 ? (
        <EmptyState
          ...
          description="После первых запусков пайплайна здесь появится история задач."
        />
      ) : null}
      <div className="flex items-center gap-2 text-sm">
        <button
          className="rounded border px-3 py-1"
          onClick={() => doBulk("retry").catch(() => undefined)}
        ...
        <button
          className="rounded border px-3 py-1"
          onClick={() => doBulk("cancel").catch(() => undefined)}
          disabled={!selectedIds.length}
        >
          Отменить выбранные
        </button>
            ...
            <label className="flex items-center gap-2">
              <input
                ...
                onChange={(e) =>
                  setSelected((prev) => ({
                    ...prev,
                    [run.run_id]: e.target.checked,
                  }))
                }
              />
              <Link
                to={`/pipelines/runs/${run.run_id}`}
