...
const BriefingsPage = () => {
  ...
  const [entryForm, setEntryForm] = useState({
    ...
    instructor_user_id: "",
    briefing_type: "introductory",
    briefing_date: new Date().toISOString().slice(0, 16),
    valid_until: "",
    reason: "",
    status: "draft",
    notes: "",
  ...
  const load = async () => {
    ...
    setLoadError(null);
    try {
      const [templateItems, journalItems, entryItems, overdueItems] =
        await Promise.all([
      ...
      setLoadError(
        (err as ApiError) ?? {
          status: 0,
          message: "Не удалось загрузить данные по инструктажам",
        },
      );
  ...
  return (
    <div className="space-y-6">
      ...
      {loading && templates.length === 0 && journals.length === 0 ? (
        <LoadingScreen label="Загрузка инструктажей" />
      ) : null}
        ...
        <div className="flex items-center gap-2">
          ...
          <Button
            ...
            title={
              canManageBriefings
                ? undefined
                : "Недостаточно прав для управления инструктажами"
            }
            onClick={() =>
              void briefingsApi
                .remindOverdue()
                .then((result) => {
                  toast.success(`Отправлено напоминаний: ${result.count}`);
                  return load();
                })
                .catch(() => toast.error("Не удалось отправить напоминания"))
            }
          >
      ...
      <Card>
        ...
        <CardContent className="grid gap-3 md:grid-cols-4">
          ...
          <div className="rounded-lg border p-3">
            <div className="text-sm text-muted-foreground">Журналы</div>
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            <Input
              ...
              onChange={(e) =>
                setTemplateForm((p) => ({ ...p, code: e.target.value }))
              }
            />
            <Input
            ...
            />
            <Input
              placeholder="Тип инструктажа"
              value={templateForm.briefing_type}
              onChange={(e) =>
                setTemplateForm((p) => ({
                  ...p,
                  briefing_type: e.target.value,
                }))
              }
            />
            <Input
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            <Input
              ...
              onChange={(e) =>
                setJournalForm((p) => ({ ...p, code: e.target.value }))
              }
            />
            <Input
              placeholder="Название"
            ...
            <Input
              ...
              onChange={(e) =>
                setJournalForm((p) => ({ ...p, journal_type: e.target.value }))
              }
            />
            <Button
              className="w-full"
              ...
              onClick={() =>
                void briefingsApi
                  ...
                  .catch(() => toast.error("Не удалось создать журнал"))
              }
            >
              Создать журнал
            </Button>
          ...
          <CardContent className="space-y-3">
            <div className="space-y-1">
              ...
              <select
                ...
                value={entryForm.briefing_journal_id}
                onChange={(e) =>
                  setEntryForm((p) => ({
                    ...p,
                    briefing_journal_id: e.target.value,
                  }))
                }
              >
            ...
            <div className="space-y-1">
              ...
              <select
                className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm"
                value={entryForm.briefing_template_id}
                onChange={(e) =>
                  setEntryForm((p) => ({
                    ...p,
                    briefing_template_id: e.target.value,
                  }))
                }
              >
            ...
            <div className="space-y-1">
              ...
              <select
                ...
                onChange={(e) =>
                  setEntryForm((p) => ({ ...p, person_id: e.target.value }))
                }
              >
            ...
            <Input
              type="datetime-local"
              value={entryForm.briefing_date}
              onChange={(e) =>
                setEntryForm((p) => ({ ...p, briefing_date: e.target.value }))
              }
            />
            <Button
              className="w-full"
              ...
              onClick={() =>
                void briefingsApi
                  .createEntry({
                    ...entryForm,
                    briefing_template_id:
                      entryForm.briefing_template_id || null,
                    person_id: entryForm.person_id || null,
                    instructor_user_id: entryForm.instructor_user_id || null,
                    valid_until: entryForm.valid_until || null,
                  })
                  .then(() => {
                    toast.success("Инструктаж назначен");
                    return load();
                  })
                  .catch(() => toast.error("Не удалось назначить инструктаж"))
              }
            >
              Назначить
            </Button>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Журнал инструктажей</CardTitle>
        </CardHeader>
        <CardContent className="space-y-3">
          {entries.map((entry) => (
            <div key={entry.id} className="rounded-lg border p-4">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  ...
                  <div className="text-sm text-muted-foreground">
                    Дата: {formatDate(entry.briefing_date)} · Срок:{" "}
                    {entry.valid_until
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                <Button
                  ...
                  onClick={() =>
                    void briefingsApi
                      .sign(entry.id, "employee")
                      .then(() => load())
                      .catch(() =>
                        toast.error(
                          "Не удалось зафиксировать подпись сотрудника",
                        ),
                      )
                  }
                >
                ...
                <Button
                  ...
                  onClick={() =>
                    void briefingsApi
                      .sign(entry.id, "instructor")
                      .then(() => load())
                      .catch(() =>
                        toast.error(
                          "Не удалось зафиксировать подпись инструктора",
                        ),
                      )
                  }
                >
                ...
                <Button
                  ...
                  onClick={() =>
                    void briefingsApi
                      .complete(entry.id)
                      .then(() => {
                        toast.success("Инструктаж завершен");
                        return load();
                      })
                      .catch((err) =>
                        toast.error(
                          (err as { message?: string })?.message ||
                            "Не удалось завершить инструктаж (нужны обе подписи)",
                        ),
                      )
                  }
                >
              ...
              <div className="mt-3 text-sm text-muted-foreground">
                Подписей: {entry.signatures.length}
              </div>
            </div>
          ))}
          {!entries.length ? (
