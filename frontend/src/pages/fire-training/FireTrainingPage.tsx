...
const FireTrainingPage = () => {
  const { data, loading, error, reload } = useAsyncResource({
    ...
    initialData: { templates: [], journals: [], overdueEntries: [], programs: [] },
    ...
    <div className="space-y-4">
      ...
      {!loading && !error ? (
        <div className="grid gap-4 lg:grid-cols-3">
          <Card>
            <CardHeader><CardTitle className="text-base">Шаблоны</CardTitle></CardHeader>
            <CardContent className="space-y-2 text-sm">{data.templates.slice(0, 5).map((item) => <p key={item.id}>{item.title} · {item.status}</p>)}</CardContent>
          </Card>
          <Card>
            <CardHeader><CardTitle className="text-base">Журналы</CardTitle></CardHeader>
            <CardContent className="space-y-2 text-sm">{data.journals.slice(0, 5).map((item) => <p key={item.id}>{item.title} · {item.status}</p>)}</CardContent>
          </Card>
          <Card>
            <CardHeader><CardTitle className="text-base">Просроченные записи</CardTitle></CardHeader>
            <CardContent className="space-y-2 text-sm">{data.overdueEntries.slice(0, 5).map((item) => <p key={item.id}>{item.briefing_type} · {item.status}</p>)}</CardContent>
          </Card>
        </div>
      ) : null}
    </div>
