...
const AuditPrepPage = () => {
  ...
  const [snapshot, setSnapshot] = useState<AuditPrepSnapshot>({
    inspections: [],
    prescriptions: [],
    overdueTasks: [],
    denied: [],
  ...
  const load = async () => {
    ...
    try {
      ...
      setError(
        (err as ApiError) ?? {
          message: "Не удалось загрузить подготовку к проверкам",
        },
      );
    } finally {
      setLoading(false);
  ...
  const packages = useMemo(() => {
    ...
    snapshot.inspections.forEach((inspection) => {
      ...
      if (
        nextScheduledAt &&
        (!existing.next || nextScheduledAt < existing.next)
      )
        existing.next = nextScheduledAt;
    ...
    snapshot.prescriptions.forEach((prescription) => {
      ...
      if (
        !["closed", "resolved", "done"].includes(
          String(prescription.status).toLowerCase(),
        )
      )
        existing.openPrescriptions += 1;
    ...
    return Array.from(byAuthority.values()).map((entry) => {
      const denominator = Math.max(
        1,
        entry.inspections + entry.openPrescriptions + entry.blockers,
      );
      const readiness = Math.max(
        0,
        Math.round((entry.inspections / denominator) * 100),
      );
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          ...
          {!loading && !error && packages.length === 0 ? (
            <EmptyState
              title="Нет контуров проверки"
              description="Инспекции и связанные блокеры ещё не зарегистрированы."
            />
          ) : null}
          {!loading && !error && packages.length > 0 ? (
            <Table>
              ...
              <TableBody>
                {packages.map((pkg) => (
                  <TableRow key={pkg.name}>
                    ...
                    <TableCell>
                      {pkg.openPrescriptions} предписаний · {pkg.blockers}{" "}
                      blockers
                    </TableCell>
