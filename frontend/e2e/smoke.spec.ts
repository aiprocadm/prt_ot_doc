...
test.describe("smoke", () => {
  ...
  test.describe("mandatory (no external creds)", () => {
    test("login page renders", async ({ page }) => {
      ...
      await expect(
        page.getByRole("heading", { name: "Вход в платформу" }),
      ).toBeVisible();
    ...
    test("attention hub route redirects to login when logged out", async ({
      page,
    }) => {
    ...
    test("unauthorized/denied route shows access denied page", async ({
      page,
    }) => {
  ...
  test.describe("credential-based flows", () => {
    ...
    test("login wrong password shows inline error", async ({ page }) => {
      test.skip(
        ...
        "Set E2E_USER_EMAIL (password may be wrong on purpose)",
      );
    ...
    test("R11 documents list after login", async ({ page }) => {
      ...
      if ((await nameCellButton.count()) > 0) {
        ...
        await expect(
          page.getByRole("tab", { name: "Предпросмотр" }),
    ...
    test("R11b documents loading screen settles", async ({ page }) => {
      ...
      await expect(page.getByText("Загрузка документов")).toBeHidden({
        timeout: 35_000,
      });
    });

    test("navigation regression: command bar, top nav links and breadcrumb", async ({
      page,
    }) => {
      test.skip(!hasDefaultCreds, "Set E2E_USER_EMAIL and E2E_USER_PASSWORD");
      await loginDefaultUser(page);
      ...
      await page
        ...
        .click();
      await page
        .getByPlaceholder("Найти раздел по названию или группе")
        .fill("Документы");
    ...
    test("navigation regression: mobile menu opens and routes", async ({
      page,
    }) => {
      test.skip(!hasDefaultCreds, "Set E2E_USER_EMAIL and E2E_USER_PASSWORD");
      await loginDefaultUser(page);
    ...
    test("limited user denied on attention hub when dashboard permission is missing", async ({
      page,
    }) => {
      test.skip(
        !hasLimitedCreds,
        "Set E2E_LIMITED_USER_EMAIL and E2E_LIMITED_USER_PASSWORD",
      );
      await loginLimitedUser(page);
      await waitMainShellReady(page);
