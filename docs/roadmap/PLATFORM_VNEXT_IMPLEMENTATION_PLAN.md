...
| **P10-07** | Analytics frontend | 🟡 **partial** | Дашборды есть: `pages/DashboardPage`, `components/analytics/JsonKpiGrid.tsx`, `api/dashboard*.ts`, Command Center (#658). Report-builder MVP (ветка `feat/p10-07-report-builder-mvp`, §24.3): сущность `ReportDefinition` (миграция `rb01`) + декларативный реестр 4 датасетов (обучение/инциденты/риски/СИЗ-остатки) + SQL-engine (фильтры/группировки count-sum/сортировка, preview ≤100, экспорт-кап 50k) + renderers CSV/XLSX/PDF (formula-injection нейтрализована, PDF-кап 2000) + **первый работающий материализатор ExportJob** (celery `report_export_job`: файл в storage + `File`-строка + typed-ошибки + broad-except) + API `/report-builder/*` (RBAC, ETag, audit, выделенное скачивание — legacy `/files/{id}/download` требует MinIO) за default-off флагом `report_builder` + demo-сид (флаг + 4 системных шаблона) + `ReportBuilderPage` (`/reports/builder`: конструктор, предпросмотр, экспорт с поллингом `usePolling`). **Остаётся:** управленческие дашборды (§24.2 — chart-слой, фильтры company/site у `/analytics/dashboard/*` фронт не передаёт); остальные 4 датасета каталога; cron-исполнение `ExportSchedule` + delivery; графики/heatmap в конструкторе; anonymization; старая кнопка ReportsPage (`reports:xlsx`) всё ещё без материализатора |
