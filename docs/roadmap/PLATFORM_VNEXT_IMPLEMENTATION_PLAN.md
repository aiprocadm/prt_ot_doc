...
| **P10-10** | Workflow / Rules / Smart Recommendations engines | 🟡 **partial** | Workflow engine влит и смонтирован: `modules/workflow/` (~1.4k LOC — граф-определения start/task/approval/condition/timer/notification/webhook/end, версии draft/published/archived, инстансы, задачи, таймлайн, SLA, эскалация/делегирование), роутер в `api/v1/route_groups.py`, потребители `analytics`/`projections`/`domain_ticks`. **Остаётся:** визуальный редактор, rules-engine с dry-run/логом срабатываний, smart recommendations (обнаружено сверкой 2026-07-04, см. `docs/audit/SPEC_CODE_SYNC_2026-07-04.md`) |
