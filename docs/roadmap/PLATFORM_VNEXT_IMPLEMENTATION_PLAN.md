...
| Phase 4: Calendar & Search | P2 | Smart Calendar improvements, universal search + command bar | 2-3 sessions | ✅ COMPLETE (4.1 Smart Calendar 100% done; 4.2 backend FTS + CMD+K palette with entity results + 7 type-to-execute commands + ARIA listbox keyboard nav + backend SearchService accuracy tests (8 cases) + saved-search shortcuts inside palette + recent clicked entities tracking via localStorage done) |
...
- [x] Tests: 30+ template test cases covering edge cases (null values, loops, conditions, formatting) — Session 36 added 53 cases across two new files: `tests/test_templates_linter_hardening.py` (28 — empty/duplicate/filter/safety/loops/conditions/headers/footers/backcompat/param table) + `tests/test_templates_inspector.py` (16 — shape/flatten/undefined/unused/required-missing/loops/filters/service-wiring/summary parity); pre-existing 11 linter tests still green; production render-path parity lives in `tests/test_templates_preview_parity.py` (filters/split-runs/missing-var/nested context); the legacy sandboxed preview renderer (`app.modules.templates.render`) is covered by `tests/test_templates_render_edge_cases.py` (40 cases incl. loops/conditions/sandbox/None-vs-0-vs-False rendering), restored from the pre-#538 stash in PR #786 (split-runs + passport marker already pinned by `tests/test_templates_next18_unit.py`)
...
> **Sync note (2026-07-04, `docs/audit/SPEC_CODE_SYNC_2026-07-04.md`):** прямая сверка кода нашла, что несколько строк ниже **занижают** готовность. Уточнения: **P10-10** — workflow-движок влит (не «not started»; строка обновлена). **P10-03 медосмотры** — «остаётся контингент/психиатрия/направления/отстранение» неточно: контингент-реестр, `MedicalExamKind.PSYCHIATRIC`, `MedicalReferral`-FSM и `MedicalSuspension`-FSM уже в коде (остаётся полнота автоматизации/данных, не отсутствие). **НПА/Compliance** — реестр + impact-analysis с bindings + генерация задач-обязательств + дедлайны реализованы (substantial). **P10-02** — клиентский кабинет уже есть (5 страниц `client-portal/`); реально отсутствуют reseller-портал и trial/sandbox-изоляция.

> **Sync note (2026-07-29, сверка §0.2 в волне OPS-71):** все срезы, помеченные ниже как «ветка `…`», **уже в `main`** — проверено по наличию миграций `med03` / `bg01` / `bg02` / `rb01` / `cmt02` / `re01` и кода (`domains/committees/kpi.py`, `modules/rules_engine/`, `modules/report_builder/`, страницы `analytics` / `rules` / `budget`). Пометка «ветка» осталась от волн, которые писали строку до мержа своего же PR: читать её как «влито», а не как «работа лежит в стороне». Тесты комитетов срезов 2–3 прогнаны в этой волне — 13/13. Слова «ветка `…`» в строках ниже сохранены как след происхождения кода.
...
| **P10-03** | Медосмотры (полный контур) | 🟡 **substantial** | Печатные формы 29н + runtime hazard→factor CRUD (PR #691), `domains/medical/print_form.py`, exams API + ETag (S53). Психиатрическое освидетельствование 342н (ветка `feat/p10-03-psychiatric-342n`): каталог видов деятельности ПП РФ 695 (`PsychiatricActivityType`, миграция `med03`) + маппинг должность→вид (`PsychiatricPositionActivity`) как **вторая ось контингента** (`compute_contingent` +PSYCHIATRIC, батч-загрузка без N+1); расширенный `MedicalExam` (`psychiatric_protocol_no`/`psychiatric_activity_codes`); периодичность 5 лет переопределяемо per-вид; направление/авто-отстранение/блок допуска переиспользуют готовую машинерию (0 нового кода); API `/medical/psychiatric/*` (каталог CRUD + `seed-defaults` + маппинг) за флагом `medical`; demo-сид; тонкая секция «Психиатрическое освидетельствование (342н)» на `MedicalPage`. Фронт-контур на MedicalPage (ветка `feat/p10-03-medical-oversight-ui`): сводка `/medical/summary` в шапке; секция «Контингент медосмотров» (реестр по должностям + поимённый список + печать DOCX/PDF); секция «Направления на медосмотры» (server-side фильтр по статусу, bulk-генерация по контингенту, ручное создание, FSM-переходы запланировать/отменить/завершить с привязкой осмотра-результата); секция «Отстранения от работы» (фильтр «только активные», снятие с подтверждением, 403 для не-admin/owner — ошибкой секции); попутно latest-wins фикс shared-хука `useAsyncResource` (дроп reload при смене фильтра mid-flight). **Остаётся:** per-activity частичное ограничение; печатная форма направления/решения комиссии; редактор маппинга должность→вид 342н на UI; серверный typeahead для пикеров (сейчас первые 100) |
| **P10-06** | СИЗ склад (полный) | 🟡 **движения + мин-остаток + инвентаризация + перемещения + поставщики + мобильная выдача + бюджет** | `PPEStockBatch` + `PPEStockMovement` (append-only журнал движений, migration `wa04`); эндпоинты `/ppe/stock/movements` (receipt/writeoff/adjustment, ETag+tenant-iso) + FIFO-списание при выдаче СИЗ (`deplete_for_issue`, за warehouse-флагом); `batch.quantity` теперь честный кэш-баланс (мутируется только проводкой), `/stock/levels` — живой остаток; фронт-секция «Движения»; мин-остаток `PPEItem.min_stock` (миграция `wa05`) + прогноз дефицита `GET /ppe/stock/shortages` (скорость расхода по issue-движениям за окно → дефицит к дозаказу + дни до исчерпания + дата пробоя) + фронт-секция «Дефицит / мин-остаток» на WarehousePage. Инвентаризация: двухфазная сессия `ppe_inventory_count`(+`_line`, миграция `wa06`) draft→applied/cancelled — снимок партий → ввод факта → `apply` выпускает `adjustment`-проводки через `record_movement` (live-дельта от текущего остатка, чокпоинт цел); эндпоинты `POST/GET /ppe/stock/inventory/counts`, `GET /…/{id}`, `PATCH /…/{id}/lines`, `POST /…/{id}/{apply,cancel}` (за warehouse-флагом, optimistic-lock→409) + фронт-секция «Инвентаризация» на WarehousePage. Перемещения между локациями: частичный перенос запаса из локации A в B парой `transfer`-проводок (`−q` источник / `+q` приёмник, общий `ref_id`) через `_write_movement`, find-or-create партии-приёмника с копией провенанса, миграция `wa07` расширяет уникальный ключ партии локацией (частичный unique-индекс `WHERE deleted_at IS NULL` + `NULLS NOT DISTINCT`, PG16); эндпоинты `POST/GET /ppe/stock/transfers` (история группируется в пары по `ref_id`, ETag) + `GET /ppe/stock/levels/by-location` (за warehouse-флагом; `IntegrityError`/optimistic-lock→409) + фронт-секция «Перемещения между локациями». Поставщики: справочник `PPESupplier` (`ppe_supplier`, миграция `wa08` аддитивная) + nullable FK `batch.supplier_id` (провенанс лота) / `item.preferred_supplier_id` (явный вендор); CRUD `/ppe/suppliers` (за warehouse-флагом, дубль имени→409); `transfer_stock` копирует поставщика в партию-приёмник; `compute_shortages` резолвит поставщика позиции (явный-если-жив → fallback на последнего из истории партий, soft-deleted→нет, без N+1) → строки `/stock/shortages` с `supplier_*`; `GET /ppe/stock/reorder` — вычисляемый черновик заявки (`build_reorder_draft`: below-threshold дефицит по поставщику, `unassigned` последней); фронт-секции «Поставщики»/«Дозаказ» (CSV-копирование) + пикер поставщика в приёмке + инлайн `preferred_supplier` в дефиците. Мобильная выдача (PR #726): online-first touch-first экран `/ppe/issue` (право `ppe.issue`) поверх готового `POST /ppe/issues` (FIFO) — поиск сотрудника (typeahead, только active) → корзина позиций (merge дублей, qty-степпер) → «выдать всё» (N вызовов, частичный отказ оставляет только неудачные строки, guard от двойного тапа); stock-aware бейджи при флаге `warehouse`, тихая деградация при выключенном; кнопка входа на `PpePage`; чисто фронтовый срез. Бюджет безопасности (СИЗ-контур §12.4): справочник `PPESafetyBudget` (`ppe_safety_budget`, миграция `wa09` аддитивная) — план расходов на период (диапазон дат, tenant-wide) + `batch.unit_cost` (цена лота); факт = **вычисляемый** агрегат закупок (`compute_budget_actual`: Σ приход×`unit_cost` за период, только `kind=receipt`, разбивка по категориям `item.category`, счётчик приходов без цены — не персистится, зеркало `compute_shortages`); CRUD `/ppe/budgets` (за warehouse-флагом) + `GET /ppe/budgets/{id}` (план/факт/остаток/разбивка/unpriced); фронт-секция «Бюджет безопасности» + поле «Цена за единицу» в приёмке. Инвариант честного остатка не тронут (цена — метаданные, факт вычисляемый). **Кросс-доменный §12.4 срез-1 «ядро» реализован отдельным под-проектом** (ветка `claude/tz-continuation-d43adc`, миграция `bg01`): бюджеты обучения/медосмотров/мероприятий (`safety_budget`), справочник статей расходов (`budget_expense_article` + seed-defaults), журнал фактических расходов (`budget_expense` с измерениями компания/филиал/объект и опциональной ссылкой на доменную запись), сводка план↔факт `GET /budget/overview` по 4 доменам (СИЗ-план из `ppe_safety_budget` + СИЗ-факт из складского леджера — read-only, склад не тронут), разрез `GET /budget/breakdown` по статье/домену/компании/филиалу/объекту (первый branch-разрез в проекте), 16 роутов `/api/v1/budget/*` за флагом `budget` default-off, страница `/budget` с 4 вкладками, demo-сид. Факт нигде не персистится (инвариант эталона). **§12.4 срез-2 «заявки на возмещение СФР» реализован** (ветка `feat/budget-srez2-reimbursements`, миграция `bg02`, спека `2026-07-19-budget-reimbursements-srez2-design.md`): `budget_reimbursement` + `budget_reimbursement_item` (сумма состава **вычисляемая**, join к `budget_expense` — инвариант эталона сохранён), чистый FSM `draft→submitted→approved|rejected→paid` (rejected/paid терминальны, правка только в draft), 9 роутов `/budget/reimbursements/*` за тем же флагом `budget` и RBAC admin/owner/accountant/ot_pb_lead, ETag списка со скаляром состава, вкладка «Возмещения СФР» на `/budget`, demo-сид из 3 заявок. **Остаётся (срез-3+):** печатная форма заявки и пакет документов в СФР; автоподбор расходов в заявку; частичная выплата/график платежей; вход СИЗ-закупок в breakdown (нужна связка batch→branch); автосбор факта из доменов; бюджеты с привязкой к филиалу; мобильная выдача follow-up (офлайн-очередь+серверная идемпотентность, QR-скан бейджа, захват подписи, norm-driven 766н-комплект, per-line партия); location-фильтр истории переносов — follow-up |
| **P10-07** | Analytics frontend | 🟡 **partial** | Дашборды есть: `pages/DashboardPage`, `components/analytics/JsonKpiGrid.tsx`, `api/dashboard*.ts`, Command Center (#658). Управленческие дашборды §24.2 (ветка `feat/p10-07-management-dashboards`): хаб `/analytics` за новым правом `ANALYTICS_VIEW` — фильтры company/site/contractor/период (фронт впервые передаёт их в `/analytics/dashboard/*`), KPI-карты (executive/overdue/sla-load), графики 6 трендов (recharts 2.x + `TrendLineChart` — первая chart-библиотека проекта), breakdown-эндпоинт `GET /analytics/dashboard/breakdown` (company/site/contractor, GROUP BY на метрику, строка «— без объекта», cap 200) с drill-down кликом, ссылки на 5 быв. «сиротских» суб-дашбордов; попутно закрыта RBAC-дыра analytics/export_center-роутеров + дедуп operational_dashboard-регистрации. Report-builder MVP §24.3 — **влит PR #732** (сущность `ReportDefinition`+`rb01`, engine/renderers CSV-XLSX-PDF, первый материализатор ExportJob, `/reports/builder`). **Остаётся:** «активность пользователей» и «состояние системы» из §24.2; остальные 4 датасета report-builder; cron-исполнение `ExportSchedule`; графики в конструкторе; пере-вёрстка суб-дашбордов с JsonKpiGrid |
...
| **P10-01** | Комитеты / комиссии / заседания | ✅ **done (срезы 1–4 в main)** | срез-1 skeleton merged (PR #695): models/committees.py, migration cmt01, domains/committees/{lifecycle,service}.py, api/routes/committees.py (15 endpoints, ETag+tenant-iso), default-off committees flag, thin CommitteesPage UI. **срез-2 ядро заседаний** (ветка `feat/p10-01-committees-srez2-proceedings`, **влита — см. sync-note 2026-07-29**): кворум+голосование+нумерация/журнал протоколов+write-UI — миграция `cmt02` (таблицы attendance/votes, enum votechoice, 6 колонок meeting + partial unique index номера); кворум = простое большинство (>½ DISTINCT членов), решение carried если за>против; авто-номер `N/ГГГГ` при проведении; API attendance GET/PUT + hold-with-quorum + votes POST/GET + журнал `GET /protocols` + `GET /{cid}/members`; полный write-UI на CommitteesPage (создание/присутствие/проведение/голосование/журнал); adversarial-review-фиксы (дедуп attendance, error-ordering, outcome-consistency). **срез-3 KPI + Command Center** (ветка `feat/p10-01-committees-srez3-kpi-command-center`, спека `2026-07-22-...-design.md`, БЕЗ миграции): KPI-сервис `domains/committees/kpi.py` + роут `GET /committees/kpi` (management-RBAC `_KPI_ROLES`, флаг-гейт, ETag/304) + `CommitteeKpiDto`; проекция задач комитетов в Command Center (категория `committee_task`, источник overdue→HIGH/unassigned→MEDIUM); фронт-страница `/committees/kpi` под `ANALYTICS_VIEW` + nav + метка в CommandCenterPanel. **срез-4 (2026-08-03, влит): контур ЗАКРЫТ** — приглашения (миграция `cmt03`: `committee_meeting_invitation` + `committee.quorum_threshold_pct`; `GET/PUT /meetings/{mid}/invitations`, только planned, PUT-замена набора), настраиваемый порог кворума (NULL = строгое большинство; порог включительно, применяется в hold-with-quorum и в клиентском индикаторе), печатная форма протокола (`GET /meetings/{mid}/protocol/print` DOCX/PDF, чистый `domains/committees/print_form.py` + `services/committee_protocol_print.py`, 503 при недоступном LibreOffice), серверный person-typeahead (`GET /persons?q=`, компонент `PersonTypeahead`, ручной ввод person_id из UI комитетов убран). Матрица TZ-B17 переведена в `done`. |
...
| **P10-10** | Workflow / Rules / Smart Recommendations engines | 🟡 **partial (workflow merged + rules-engine срез-1 на ветке)** | Workflow engine влит и смонтирован: `modules/workflow/` (~1.4k LOC — граф-определения start/task/approval/condition/timer/notification/webhook/end, версии draft/published/archived, инстансы, задачи, таймлайн, SLA, эскалация/делегирование), роутер в `api/v1/route_groups.py`, потребители `analytics`/`projections`/`domain_ticks`. **Rules-engine срез-1** (vNext §25.2 ядро, ветка `feat/p10-10-rules-engine-srez1`): `modules/rules_engine/` (условия/действия/движок/каталог/API) + модели `automation_rule`/`automation_rule_trigger` (миграция `re01`) — событийные правила поверх синхронного hook'а в `OutboxService.enqueue` (SAVEPOINT-изоляция, guard от каскада `rule.triggered`), действия create_task/notify/webhook, dry-run + тест по истории, лог срабатываний, admin-API `/rules/*` за флагом `rules_engine` + страница `/rules`, demo-сид. **Остаётся:** версии правил + tenant overrides (срез-2), визуальный редактор workflow, smart recommendations (§25.3) |
...
**Summary (updated 2026-07-04 sync):** **3 subprojects fully merged** (P10-05 Подрядчики, P10-08 Наряды-допуски, P10-04 СОУТ срезы 1-6) · **9 partial** (P10-01/02/03/06/07/09/10/12/14 — P10-10 workflow-движок перемещён из «not started» в «partial» по сверке кода 2026-07-04) · **3 not started** (P10-11/13/15). P10-04 СОУТ закрыт по всем 6 срезам (PR #696–#710). P10-06 СИЗ-склад дополнен журналом движений + FIFO (PR #720), затем инвентаризацией/перемещениями/поставщиками (PR #722/#724/#725), мобильной выдачей (online-first `/ppe/issue`, PR #726) и бюджетом безопасности (СИЗ-контур §12.4: `wa09`, план↔факт закупок). Next-up by version tag `[v1.1]`: finish partials — **P10-03 медосмотры** (контингент/психиатрия/направления/отстранение), **P10-07 analytics** (управленческие дашборды + report-builder UI), кросс-доменный §12.4 бюджетный контур (обучение/медосмотры/мероприятия/возмещения/branch-аналитика); **P10-10** (rules-engine + визуальный редактор + smart recommendations поверх готового workflow-движка); then not-started **P10-13 SSO**, P10-11 вертикали, P10-15 терминалы/CV/AI.
...
**Срез-4 влит 2026-08-04:** единый календарь дедлайнов
(`GET /managed-clients/calendar` + блок в кокпите) — медосмотры, СИЗ, обучение
и договоры по всем клиентам по дням, фильтры по клиенту/специалисту/типу;
просроченное первым и не исчезает, «сегодня» ещё не просрочка.

**Срез-5 влит 2026-08-04 — разд. 49.2 закрыт целиком:** загрузка специалистов
(`GET /managed-clients/workload` + панель в кокпите) — клиенты/сигналы/просрочки
на каждом ответственном, перегруз с причинами и видимыми порогами; клиенты без
ответственного отдельной строкой.

**Срез-6 влит 2026-08-04:** матрица делегированного доступа (разд. 49.3,
фундамент) — `managed_client_access` (mc02 с RLS), гранты с модульным охватом,
отзыв как след (не удаление), выдача/отзыв в аудите, `GET /managed-clients/my`
как основа переключателя клиентов. Умолчание — запрет.

**Срез-7 влит 2026-08-04:** работа «в контексте клиента» — `POST /managed-clients/{id}/context`
+ зависимость `ClientContextDep` (заголовок `X-Managed-Client`) для любого роута
данных; отказ 403 вместо тихого игнорирования, обязательная пометка в аудите.

**Срез-8 влит 2026-08-05:** переключатель клиентов без перелогина —
`ClientContextSwitcher` в кокпите, постоянный заметный индикатор «работаете
от имени X» с выходом в один клик, заголовок `X-Managed-Client` в клиенте API,
сброс контекста при смене контура.

**Срез-9 влит 2026-08-05:** контекст стал ФИЛЬТРОМ данных. В разделах «Люди»
и «Медосмотры» видны данные только клиента из контекста; правила fail-closed
(нет организации или свой контур → пусто с объяснением, а не весь арендатор);
чужая запись — 404, а не 403; идентификатор клиента входит в ETag; запись в
чужую организацию и перенос наружу контекста запрещены; удаление в контексте
запрещено (Доп. №3 63.2); индикатор честно называет разделы, где фильтр
действует.

**Срез-10 влит 2026-08-05:** у работы «от имени» появились границы —
сессия на 60 минут (`managed_client_context_session`, миграция mc03),
отдельный код `MANAGED_CLIENT_CONTEXT_EXPIRED`, выход закрывает сессию на
сервере и пишется в аудит, переключение на другого клиента закрывает
прошлую сессию, список запретов (пароли/2FA, платёжные данные, массовый
экспорт ПДн, удаление, настройки безопасности) применяется middleware,
в баннере идёт счётчик остатка.

**Срез-11 влит 2026-08-05:** фильтр по клиенту в СИЗ, обучении и документах —
реестр разделов вырос с 2 до 5, общие помощники `apply_person_scope` /
`apply_company_scope`, фильтр стоит перед параметрами запроса (`?company_id`
не расширяет область), тот же фильтр на подсчёте итога, скачивание и статус
документа закрыты отдельно.

**Срез-12 влит 2026-08-07:** согласие клиента на делегированный доступ (разд. 49.3 + 66.3) — `managed_client_consent` (mc04 с RLS), `GET/POST/DELETE /managed-clients/{id}/consents`; без действующего согласия грант не выдаётся и вход «от имени» закрыт (`MANAGED_CLIENT_CONSENT_REQUIRED`); истёкшее согласие равно отозванному; отзыв — след и немедленно закрывает открытые сессии всех специалистов по клиенту; фиксация/отзыв в аудите.

**Срез-13 влит 2026-08-07:** перевод Lightweight → Dedicated (разд. 49.1, операция) — `POST /managed-clients/{id}/convert-to-dedicated`: собственный арендатор через BootstrapTenantService, история ядра на той же строке, организация — ссылка на историю; слаг обязан быть свободен; расторгнутый договор блокирует; аудит. Перенос доменных данных — следующий срез.

**Срез-14 влит 2026-08-08:** перенос данных клиента в его арендатор — `POST /managed-clients/{id}/transfer` + журнал `managed_client_transfer` (mc05, RLS, id_map): копия организации и сотрудников (не переезд), ровно один раз, обезличенные/удалённые остаются (разд. 66.2), каталожные ссылки обнуляются, доверенная сессия + PG-тест.

**Срез-15 влит 2026-08-08:** доменная история едет вместе с людьми — медосмотры, выдачи СИЗ, завершённое обучение; карточки использованных курсов/программ/номенклатуры копируются (там FK NOT NULL либо копия иначе мертва), методика остаётся у аутсорсера; служебные даты копий = даты оригинала; непереносимое показано числами, включая отчёт «что осталось у аутсорсера».

**Срез-16 влит 2026-08-08:** структура организации (площадки, должности, рабочие места) и оставшаяся история (допуски, журналы инструктажей, происшествия). Исправлен срез-14: ссылки человека на должность и рабочее место перевешиваются, а не обнуляются — это данные организации клиента, а не каталог аутсорсера.

**Срез-17 влит 2026-08-08:** нормы медосмотров и СИЗ переезжают за должностью вместе с карточками использованных опасностей — у клиента снова считается периодичность медосмотров и положенные СИЗ.

Дальше по контуру: согласие клиента на делегированный доступ (разд. 66),
делегированный доступ к своему контуру клиента (только он снимет
`not_aggregated`), перевод Lightweight→Dedicated без потери истории (49.1).
...
## Решения владельца по внешним зависимостям (2026-09-14)

Одиннадцать строк матрицы висели в `partial` с пометкой «остаток требует
решения владельца / внешней системы». Владелец делегировал эти решения
исполнителю с требованием **«желательно, чтобы всё было реализовано в коде»**.

### Принцип, общий для всех одиннадцати

Внешнюю зависимость нельзя купить кодом. Но у каждой из них есть код-часть, и
она решающая:

1. **Шов вместо заглушки.** Если не хватает внешнего поставщика (KMS, SMS, ЭДО),
   в коде заводится точка подключения с двумя реализациями: рабочей «без
   поставщика» и той, что вызывает внешнюю систему по настройке. Подключение
   боевого поставщика становится **настройкой, а не разработкой**.
2. **Правило — это данные плюс сторож.** Если «внешнее» на самом деле правило
   работы (сроки хранения, удаление из резервных копий, порядок раскатки), оно
   записывается данными и проверяется тестом. Правило, живущее только во фразе
   в документе, протухает молча — так уже случалось (срез-171, снимок задач).
3. **Честный отказ вместо симуляции.** Там, где без внешней стороны результата
   действительно нет (пентест, домен партнёра), код доводит дело до самой
   границы: готовит задание, проверяет подтверждение, называет причину кодом
   ошибки. Симулировать успех запрещено (известный случай: пустые адаптеры
   уверяли, что данные приняты ведомством).

### Таблица решений

| Строка | Решение | Код-часть |
|---|---|---|
| SEC-66 | Локализация ПДн объявляется данными и проверяется при старте; DPA — вид юридического текста поверх готового механизма `legal_documents`; роли Оператор/Обработчик — поле арендатора | реестр обработки обязан называть регион; сторож соответствия объявленного региона фактической настройке хранилища |
| SEC-67 | KMS/Vault не выбирается, а подключается: провайдер ключей с реализациями `env` и `command` (агент Vault, CLI облачного KMS) | `core/key_provider.py` + настройка + громкая ошибка вместо молчаливого отката на env |
| SEC-68 | SMS — второй канал доставки через общий шов каналов; поставщик задаётся настройкой | протокол канала + реализации `log` и `command`, выбор канала на отправке кода |
| SEC-69 | Пентест заказывается по готовому заданию; «седьмой вопрос» становится настоящим гейтом | `scripts/ci/check_security_dod.py` + задание на пентест с областью и границами |
| BIZ-50 | ЭДО подключается провайдером; пока его нет — рабочий провайдер `file` (выгрузка подписанного пакета + журнал отправки), а не тупик | реестр провайдеров ЭДО, `POST /edo/send` перестаёт быть заведомым отказом |
| BIZ-51 | Отчёт клиенту уходит по существующему контуру доставки; согласие и адрес — поля клиента | согласие на рассылку, адрес доставки, отправка через outbox |
| BIZ-52 | DNS остаётся снаружи, но подтверждение владения доменом автоматизируется (TXT-запись); цена партнёра хранится прайс-листом | модель домена с токеном и статусом; прайс-лист, отчёт выручки считается по нему |
| OPS-71 | Образцы выгрузок конкурентов НЕ нужны: профиль сопоставления колонок заводится из файла самого клиента и переиспользуется | сохраняемый профиль маппинга + подбор по заголовкам загруженного файла |
| OPS-72 | Удаление из резервных копий — реестр копий с политикой хранения и обязательствами удаления; исполнение подтверждается актом | реестр копий, обязательства при офбординге, акт выполнения |
| OPS-73 | Календарный запрет на снятие legacy-ручек СОХРАНЯЕТСЯ (до 2027-07-30); берётся вторая половина строки — единый конверт событий v2 | `schema_version` v2, единицы подписи, совместимость v1 |
| OPS-74 | Способ раскатки записывается данными и проверяется сторожем: разрушающая миграция обязана объявить фазу expand/contract и релиз | сторож безопасности миграций + объявленный SLA отката |

Порядок исполнения — по убыванию цены пропуска: SEC-67 → SEC-69 → OPS-74 →
SEC-66 → SEC-68 → BIZ-50 → OPS-72 → OPS-73 → OPS-71 → BIZ-52 → BIZ-51.
