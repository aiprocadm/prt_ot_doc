...
- [x] Recent items: Recently viewed entities — recent navigation paths already in localStorage (`ux.commandbar.recentPaths.v1`); recent **clicked entities** now persisted in `ux.commandbar.recentEntities.v1` (dedup by `entity_type+entity_id`, cap 6) and rendered as «Недавно открытые» section on empty query in CMD+K palette; clicks from entity-search results and from the recent section both update the list; integrated with ↑↓/Enter keyboard nav; 3 new `CommandBar.test.tsx` cases (render-from-storage, hide-on-query, write-on-click)
- [x] Saved searches: User-defined filters (e.g., "my overdue items") — backend `/search/saved` CRUD + UI on `/search` page (long-standing) + CMD+K palette shortcuts (Phase 4.2 tail) — `CommandBar.tsx` loads `fetchSavedSearches()` on open, renders «Сохранённые запросы» section above entity results (cap 5), filters by name/q substring as the user types, links to `/search?q=…&types=…`, integrated with ↑↓/Enter/Home/End keyboard nav; 3 new `CommandBar.test.tsx` cases (render+activate, filter, empty)
- [x] Tests: Search index accuracy, command parsing — frontend command parsing covered (4 CommandBar.test.tsx cases); backend `tests/test_search_service_accuracy.py` (8 cases — tenant isolation, type-alias resolution, ranking exact/prefix/contains, status+tags filters, facet counts, pagination cursor, snippet windowing, empty-query fallback)
...
**Acceptance criteria:**
- [~] Template lint: Detect broken placeholders, undefined variables, missing conditions — base linter (`backend/app/modules/templates/linter.py`) covers balanced if/for blocks + safe-expr; Phase 5.1 extension adds `{% else %}`/`{% elif … %}` recognition with placement check, unbalanced `{{…}}` and `{%…%}` delimiter detection, and optional `available_fields` warning for used-but-undeclared roots (loop-local vars excluded); 18 new cases in `tests/test_templates_linter_phase5.py`
- [ ] Preview engine: 100% accuracy (preview matches final PDF)
- [x] Variable inspector: Show all available variables in template context — pure function `inspect_template_variables()` in `app/modules/templates/service.py` builds used vs declared coverage from the persisted `placeholder_index` and `required_fields_schema` (declared_required, declared_available, used_undeclared, declared_unused, loop-vars excluded); exposed via `GET /api/v1/templates/{template_id}/versions/{version_id}/variables` returning `TemplateVariableInspectorDTO`; 8 cases in `tests/test_templates_variable_inspector.py`
- [x] Tests: 30+ template test cases covering edge cases (null values, loops, conditions, formatting) — `tests/test_templates_render_edge_cases.py` (42 cases): filters (`_f_upper`/`_f_lower`/`_f_date` with datetime/ISO-string/None/non-string/invalid string + custom fmt), placeholders (split-runs, nested dotted paths, missing root/attr raise `UndefinedError`, None/0/False/empty/float/list rendering, XML-special chars, Cyrillic), filters via template (`| upper`/`| lower`/`| date('%d.%m.%Y')`), conditions (`if`/`else`/`elif` chain), loops (empty/single/dict-items/inner-if/`loop.index`), multi-paragraph + header.xml rendering, sandbox blocks `__class__` access, undefined globals raise, passport marker present in output
