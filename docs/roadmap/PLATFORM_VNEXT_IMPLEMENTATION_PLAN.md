...
| Phase 4: Calendar & Search | P2 | Smart Calendar improvements, universal search + command bar | 2-3 sessions | 🟡 IN PROGRESS (4.1 Smart Calendar 100% done; 4.2 backend FTS + CMD+K palette with entity results + 7 type-to-execute commands + ARIA listbox keyboard nav (↑↓/Enter/Home/End) done; backend search index accuracy tests + recent entities + saved-search shortcuts in palette pending) |
...
- [~] Recent items: Recently viewed entities — recent navigation paths already in localStorage (`ux.commandbar.recentPaths.v1`); recent **entities** (clicked from search) backend exists (`/search/recent` for queries) but recent **entities** (clicked items) not tracked
- [~] Saved searches: User-defined filters (e.g., "my overdue items") — backend `/search/saved` CRUD + UI on `/search` page exists; saved-search shortcuts not exposed inside the CMD+K palette yet
- [~] Tests: Search index accuracy, command parsing — frontend command parsing covered (4 new CommandBar.test.tsx cases); backend search index accuracy tests for `SearchService.search()` open
