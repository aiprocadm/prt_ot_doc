...
Дальше по контуру: impersonation «работа в контексте клиента» с пометкой
в аудите + применение `managed_client_id` как ABAC-атрибута к данным клиента
(49.3 — он же снимет `not_aggregated`), переключатель в UI, перевод
