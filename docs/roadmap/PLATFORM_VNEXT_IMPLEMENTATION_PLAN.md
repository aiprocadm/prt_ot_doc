...
| **P10-04** | СОУТ (спец. оценка условий труда) | 🟡 **partial (срез-1)** | срез-1 skeleton on branch `feat/sout-srez1-p10-04`: `models/sout.py` (4 tables), migration `so01` (chain off cmt01, shared `soutclass` enum), `domains/sout/{lifecycle,service}.py`, `api/routes/sout.py` (14 endpoints, ETag+tenant-iso, campaign lifecycle + reassessment-due projection), default-off `sout` flag + demo seed, thin `SoutPage` UI. 37 backend unit tests green. Остаётся (срез-2+): импорт файла отчёта СОУТ + валидация, авто-каскад класса в нормы СИЗ/медосмотры, версионирование класса (history+diff), декларация соответствия, печатные формы (карта СОУТ / сводная ведомость). |
