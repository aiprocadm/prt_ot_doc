...
- ~~Путь OutboxProcessor → WebhookDispatcher не подписывает доставку~~ —
  **исправлено** (строгий tenant-безопасный резолв секрета): секрет
  восстанавливается в момент доставки по id подписки из заголовков записи либо по
  URL, но ТОЛЬКО если подписка своя (или глобальная), включена и совпадает по типу
  события (с учётом алиасов). Чужой арендатор секрет не получает; расшифровка при
  использовании (SEC-67); в Outbox секрет не персистится. Отозванный ключ = доставка
  уходит в ретрай (fail-closed), а не тихо без подписи. Ошибки БД в резолве
  изолированы savepoint'ом — одна сбойная запись не роняет весь батч доставки.
  env-URL без секрета — без подписи, как и раньше.
