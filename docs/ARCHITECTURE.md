...
## Краткое описание
- **Назначение**: мультиарендный backend на FastAPI (`backend/app/`) для управления компаниями, сотрудниками, документами, пакетами нормативных актов, оценками рисков и аудиторскими событиями.
- **Технологии**: FastAPI + SQLAlchemy 2.0 + PostgreSQL, Celery + Redis, MinIO/S3, DocxTPL + LibreOffice, Prometheus/OTel, ClamAV.
- **Слои**: API (`backend/app/api`), бизнес-сервисы (`backend/app/services`, `backend/app/domains`), модели и БД (`backend/app/models`, `backend/app/db`), инфраструктура (`backend/app/core`, `backend/app/middleware`, `config/`).

## Доменные модули
| Домен | Модели (`backend/app/models`) | Схемы (`backend/app/schemas`) | API (`backend/app/api/...`) | Сервисы/домены |
| --- | --- | --- | --- | --- |
...
| **Подразделения и финансы** | `finance.py::Department`, `Contract`, `Order`, `Invoice` | `department.py`, `contract.py`, `order.py`, `invoice.py` | `api/routes/departments.py`, `contracts.py`, `orders.py`, `invoices.py` | Доступ через `core/security.py` (RBAC+ABAC), связки с документами |
...
| **Дедлайны/обязательства** | `obligations.py::Task` | `task.py::TaskRead/TaskListResponse` | `api/routes/tasks.py`, `api/routes/medical.py` | `services/obligations.py`, `tasks.reminders.dispatch` |
...
### Детализация доменов
- **Tenant/Auth**: зависимости `backend/app/api/dependencies.py` инжектируют `AsyncSession` и выбранный `Tenant`. RBAC/ABAC (`backend/app/core/security.py`) проверяет роли (`owner`, `admin`, `ot_pb_lead`, `ot_specialist`, `pb_engineer`, `ecologist`, `hr`, `lawyer`, `accountant`, `line_manager`, `worker`, `contractor_inspector`, `employee`, `client_admin`, `client_user`) и объектные атрибуты (tenant/company/site/document/risk). `services/auth.py` управляет паролями, JWT (`issue_access_token/refresh_token`).
...
- **Дедлайны/обязательства**: `services/obligations.py` создаёт задачи по событию назначения обучения/медтребования, планирует напоминания и отправляет `TaskDueSoon`/`TaskOverdue` через outbox. Ежедневный Celery beat (`tasks.reminders.dispatch`) агрегирует напоминания по арендам.
...
9. **Обязательства**: `Task` хранит дедлайны по обучению/медосмотрам; worker ежедневно проверяет просрочки и шлёт события outbox.

## RBAC/ABAC enforcement points
- `core/security.py::rbac` проверяет роль/список ролей пользователя и токена; `core/security.py::abac` валидирует атрибуты (tenant/company/site/document/risk).【F:backend/app/core/security.py†L317-L548】
- Основные маршруты с RBAC/ABAC: `/api/v1/documents/{id}/status`, `/api/v1/tasks`, `/api/v1/contracts`, `/api/v1/invoices`, `/api/v1/risk/risks`. 

## Слои приложения
### API (`backend/app/api`)
- `backend/app/api/app.py` создаёт FastAPI-приложение, настраивает middleware (`TrustedHost`, `CORSMiddleware`, `TenantMiddleware`, `SlowAPIMiddleware`, `ObservabilityMiddleware`) и endpoint `/metrics` (Prometheus).
- `backend/app/api/v1/router.py` агрегирует все доменные роутеры (`auth`, `audit`, `files`, `packs`, `npa`, `risk`, `documents`, `tasks`) и содержит дополнительные эндпоинты (`/companies`, `/employees`, `/templates`).
- `backend/app/api/routes/*.py` содержат изолированные доменные контроллеры. Общие зависимости (`AsyncSession`, `Tenant`, `AccessContext`) — в `backend/app/api/dependencies.py` и `backend/app/api/deps/*`.

### Сервисный слой (`backend/app/services`, `backend/app/domains`)
...
### Модели и доступ к БД (`backend/app/models`, `backend/app/db`)
...
- `backend/app/db/session.py` и `backend/app/db/base.py` создают engine/Session, учитывают tenant при подключении. Репозитории (`backend/app/repository.py`) агрегируют сложные SELECT’ы.

### Конфигурация и инфраструктура (`backend/app/core`, `backend/app/middleware`)
...
1. **Новые домены** — создавайте модели/схемы, сервисы, отдельный роутер и подключайте его в `backend/app/api/v1/router.py`.
