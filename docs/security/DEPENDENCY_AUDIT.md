...
**Очередь исключений со сроком 30.09.2026 закрыта целиком (срезы 218–221):**
`black` убран (218), `pytest` 9 (219), `vite` 8 и `vitest` 4 (220),
`typescript-eslint` 8 без смены eslint (221). Срез-222 снял мёртвые инструменты
без исключений — `isort`, `rollup`, `ts-node`, `tsconfig-paths`: их никто не
звал (ни гейт, ни скрипт, ни конфиг), а поднимать «потому что уязвимость»
однажды пришлось бы; сторож `tests/test_dead_dev_tools.py` не даёт им
вернуться. Остались три записи с поздними сроками: `ecdsa` (31.10, апстрим-фикса
нет — решение владельца), `setuptools` и `msgpack` в образе (30.11).
...
- **pip-audit — БЛОКИРУЮЩИЙ** (шаги 1, 3, 4 плана ниже выполнены): база находок
  ...
  (сейчас одна — ecdsa, апстрим-фикса нет; записи pytest и black сняты срезами
  218–219; блок starlette ×7 СНЯТ 2026-07-31 подъёмом fastapi 0.141.1 /
...
4. Убрать `continue-on-error`: у `pip-audit` — **сделано** (шаг «pip-audit gate»
   ...
   бампы из п. 2 (pytest 9, vite 8, vitest 4, typescript-eslint 8 — сделаны срезами 219–221; black убран;
