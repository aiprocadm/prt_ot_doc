...
## Таблицы без модели (срезы 135 → 138 → 139, 2026-09-10)
...
мёртвые: `inspection_prep_packages.source_inspection_id` (в коде его больше
нет) и `document.job_id` (в коде остался как простой столбец без связи — он
есть в открытом контракте `DocumentRead.job_id` и в продукте всегда NULL;
снимается вместе с `/api/v2` и миграцией; внешний ключ на
`documentgenerationjob` в базе остался и уйдёт вместе с таблицей).

**История `documentgenerationjob`** — пример того, чем опасен неточный
вердикт. Срез-135 удалил её как «мёртвую», и полный прогон дал 18 падений:
ручки списка/детали документов отдавали 500 — модель ЧИТАЛИ фильтр списка
«генерируется»/«ошибка», статус карточки и служба готовности. Срез-138 вернул
её с честным вердиктом «читают, но пусто всегда». Срез-139 разобрался, ПОЧЕМУ
пусто: живая генерация (`PipelineRun`) создаёт строку документа только по
завершении, а упавший прогон строки не создаёт — состояния «генерируется» у
документа не бывает в принципе. Ветки сняты, опция «Генерация» убрана с
экрана, и только после этого модель удалена — «сначала снять зависимость,
потом удалять».
