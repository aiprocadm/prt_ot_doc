...
- **`modules/analytics/api.py`**: ABAC-зависимость на ВСЕ GET-роуты. Read-роли: **объединение ролей, имеющих `DASHBOARD_VIEW` в `frontend/src/permissions/permissions.ts`** (пин точного списка — в плане, по фактической матрице; минимум `_SUMMARY_ROLES` из `routes/dashboard.py` = admin/owner/line_manager/hr/ot_pb_lead + ot_specialist). `POST /analytics/recompute` — только admin/owner. Паттерн — `abac(_tenant_resource_id, required_roles=..., action=...)` как в `routes/reports.py:38-41`.
...
- **Wiring**: `pageRegistry` lazy + `routeGroups` (группа `DASHBOARD_VIEW`, путь `/analytics`) + `navigationConfig` пункт «Управленческая аналитика» в «Бизнес и аналитика». ВНИМАНИЕ: путь `/analytics` не должен перехватить `/analytics/trends` (существующий) — Route точный, не wildcard.
