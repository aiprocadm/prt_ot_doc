...
| `npa_id` → `npa_act.id` (общий реестр; в ORM простой столбец, внешний ключ держит PostgreSQL — миграция `20260910_b18_npabinding_npa_act`), `template_version_id` (nullable), `entity_type` (`template_version`,`document`,`pack`), `entity_id`, `context` (json), `ref` (varchar), `reviewed_revision_id` → `npa_revision.id` (nullable; срез-144, разд. 19.4 — редакция, по которой связь сверяли в последний раз; в ORM простой столбец, миграция `20260911_b18_npabinding_reviewed_rev`). Уникальность `tenant_id + npa_id + entity_type + entity_id`.
...
- **Кто читает:** `GET /compliance/requirements` (фильтры `npa_id`/`status`/`overdue`; в ответе `site_name`, `role_label` — срез-147), оценка влияния акта (`linked.requirements`, `summary.requirements` — без автозадач), Центр внимания (`compliance_requirement`: просрочено → critical/high, срок ≤ 3 дней → medium; `summary.overdue_requirements`).
- **Справочник формы (срез-147):** `GET /compliance/requirements/options` — роли записи получают кандидатов в ответственные (активные пользователи арендатора: id, имя, роль), живые площадки с компанией и роли словами. `role_code` — закрытый словарь `RoleEnum` (422 вне словаря); подписи ролей — `backend/app/core/role_labels.py::ROLE_LABELS` (повторяет таблицу ролей выше, полноту стережёт `backend/tests/test_role_labels.py`).
