...
фактами и в отчёте названы. С среза-56 — центр внимания, разрез по
дисциплинам и отчёт директору: они считают ФАКТЫ (просрочки, открытые
происшествия), поэтому убирается только ПУСТАЯ строка скрытой дисциплины,
строка с фактами остаётся, а фраза (`describe_hidden`) называет обе группы.
