...
| Ослабление проверки JWT tenant vs `X-Tenant` (slug или UUID) | Доступ к чужой аренде при подмене заголовка | `TenantMiddleware` + `tests/test_jwt_xtenant_uuid_scope_mismatch.py`, `tests/test_tenant_security.py` |
