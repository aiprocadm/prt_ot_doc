...
| client/client_user | same tenant, file owned by another company | file details/download | File row may be discoverable only by id, but action buttons should not imply access | files API enforces tenant row + company ABAC (`ensure_company_access`) | Deny (403) | Deny should emit file action deny audit events (for example `file.download_url.denied`/`file.delete.denied`/`file.link.denied`) and no file payload leak. |
| any role | same tenant, file is not AV-clean | privileged file usage (`download-url`, guarded link roles `output/signature/receipt`) | UI should hide/disable download/signature controls until status is clean | files service enforces clean-only guard (`file_not_clean`) before issuing privileged actions | Deny (403) | Deny should be auditable through file action deny events with reason `file_not_clean`. |
| token with stale high privilege, DB role downgraded | same tenant | privileged write API reuse | UI should require re-login/reduced privileges after downgrade | server must derive effective roles from persisted user role assignment (not stale token claims alone); files write handlers gate on effective `AccessContext.role` | Deny | Denial should be explicit (`AUTHZ_DENIED`/`insufficient_role`) and covered by route-guard deny tests for stale-claim tokens. |
