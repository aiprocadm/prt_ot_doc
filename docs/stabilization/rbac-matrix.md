...
| token with stale high privilege, DB role downgraded | same tenant | privileged write API reuse | UI should require re-login/reduced privileges after downgrade | server must derive effective roles from persisted user role assignment (not stale token claims alone); files write handlers gate on effective `AccessContext.role` | Deny | Denial should be explicit (`AUTHZ_DENIED`/`insufficient_role`) and auditable (see file action deny audit events). |
