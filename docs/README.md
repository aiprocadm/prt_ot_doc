# Документация

## Главный документ (source of truth)
- **Единое полное ТЗ:** [spec/TZ_FULL_UNIFIED.md](spec/TZ_FULL_UNIFIED.md)
- **Главный repo README:** [../README.md](../README.md)

## Аудит и соответствие
- Матрица покрытия ТЗ: [audit/TZ_COVERAGE_MATRIX.md](audit/TZ_COVERAGE_MATRIX.md)
- Репозитарный аудит и canonical layout: [audit/REPOSITORY_AUDIT.md](audit/REPOSITORY_AUDIT.md)
- Baseline verification (fail-first): [audit/BASELINE_VERIFICATION.md](audit/BASELINE_VERIFICATION.md)
- Compliance (historical): [audit/TZ_COMPLIANCE.md](audit/TZ_COMPLIANCE.md)

## Запуск и тестирование
- Codespaces runbook: [runbook-codespaces.md](runbook-codespaces.md)
- Setup guide: [SETUP.md](SETUP.md)
- Testing guide: [TESTING.md](TESTING.md)
- Repo structure: [PROJECT_STRUCTURE.md](PROJECT_STRUCTURE.md)

## Архитектура и API
- Архитектура: [ARCHITECTURE.md](ARCHITECTURE.md)
- Backend: [BACKEND.md](BACKEND.md)
- Frontend: [FRONTEND.md](FRONTEND.md)
- Modules: [MODULES.md](MODULES.md)
- Workflows and events: [WORKFLOWS_AND_EVENTS.md](WORKFLOWS_AND_EVENTS.md)
- OpenAPI контракт: [openapi.yaml](openapi.yaml)
- API overview: [API_OVERVIEW.md](API_OVERVIEW.md)
- Domain model: [DOMAIN_MODEL.md](DOMAIN_MODEL.md)
- Document core: [DOCUMENT_CORE.md](DOCUMENT_CORE.md)
- Integrations: [INTEGRATIONS.md](INTEGRATIONS.md)
- Security: [SECURITY.md](SECURITY.md)
- Observability: [OBSERVABILITY.md](OBSERVABILITY.md)

- [Analytics + Search + Export + Client Portal flows](analytics_export_search_client_portal_flows.md)
