...
2. Проверьте текущий handoff в `../../AI_IMPLEMENTATION_REPORT.md` (последний блок «Last Agent Handoff»). «Следующий точный шаг» — гипотеза прошлой волны, а не приказ.
3. **Сверьте заявленное с фактом** (обязательная сверка; протокол — §0.2 канона): статусы в `../audit/TZ_COVERAGE_MATRIX.md` (MVP) и `../roadmap/PLATFORM_VNEXT_IMPLEMENTATION_PLAN.md` (vNext-фазы) — заявления, не факты. Для целевых строк откройте evidence-пути, прогоните тесты строки, закройте хвост прошлого handoff; вердикты «сделано / не сделано / переделать»; расхождения — сначала в матрицу/отчёт, потом новая работа.
