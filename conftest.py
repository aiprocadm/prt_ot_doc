...
# ---------------------------------------------------------------------------
# СРЕЗ-232: накопитель обращений к устаревшим ручкам не протекает между тестами.
# ---------------------------------------------------------------------------
#
# ЧТО БЫЛО. ``services/api_deprecation_recorder`` копит обращения к устаревшим
# версиям API в словаре УРОВНЯ МОДУЛЯ — один на процесс. Это верно для боевой
# работы: буфер сливают пачкой, чтобы не писать в базу на каждый запрос. Но в
# прогоне тестов процесс один на множество тестов, а база у каждого теста СВОЯ.
# Значит, обращение, записанное одним тестом, оставалось в памяти и при первом
# же вызове слива попадало в базу СОВСЕМ ДРУГОГО теста.
#
# Поймалось это на параллельном прогоне: проверка уведомлений об устаревших
# ручках ждала одно событие, а находила два — второе прилетало от соседа по
# процессу. Последовательный прогон это прятал: там соседи выстраивались иначе,
# и пары «кто записал — кто слил» не складывалось.
#
# Сброс между тестами существовал (``reset_for_tests``), но звал его ОДИН файл
# тестов — тот, что про сам накопитель. Остальные о нём не знали.
#
# Здесь сброс делается для ВСЕХ тестов: буфер процесса обнуляется до и после
# каждого. Импорт внутри фикстуры намеренный — корневой conftest выполняется
# до настройки окружения приложения, и тянуть сюда модуль приложения на уровне
# файла нельзя.
import pytest as _pytest  # noqa: E402


@_pytest.fixture(autouse=True)
def _reset_api_deprecation_buffer():
    """Буфер обращений к устаревшим ручкам — общий на процесс; чистим его."""

    try:
        from app.services import api_deprecation_recorder as _recorder
    except Exception:  # pragma: no cover — тест не поднимает приложение
        yield
        return
    _recorder.reset_for_tests()
    yield
    _recorder.reset_for_tests()
