...
const ArchiveSearch = () => {
  ...
  return (
    <div className="space-y-4">
      ...
      <SavedViewsBar storageKey="archive-search:views" currentParams={params} onApply={applySavedView} />
      <div className="flex flex-wrap gap-2">
        <label className="sr-only" htmlFor="archive-query">Поиск</label>
        <Input id="archive-query" value={q} onChange={(e) => patchParams({ q: e.target.value })} placeholder="Поиск по метаданным и содержимому" />
      ...
      {!loading && error ? <ErrorState error={error} onRetry={runSearch} /> : null}
      {!loading && !error && !items.length ? <EmptyState title="Ничего не найдено" description="Измените параметры поиска или сбросьте фильтры" /> : null}
