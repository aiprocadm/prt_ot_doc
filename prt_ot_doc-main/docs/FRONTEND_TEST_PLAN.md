...
## Update: test stabilization for routing defaults

### Добавлены тесты
- `landingRoute.test.ts`:
  - выбор landing-route для стандартной роли (dashboard);
  - fallback на клиентский кабинет при portal-only разрешениях;
  - fallback на `/no-access`, когда разрешения отсутствуют.

### Что это стабилизирует
- Исключает регрессии «битого» дефолтного редиректа для ролей без `COMPANY_VIEW`/`DASHBOARD_VIEW`.
- Защищает от повторного появления нецелевого входа в корпоративную часть интерфейса для portal-only пользователей.
