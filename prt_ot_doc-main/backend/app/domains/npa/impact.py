...
class NpaImpactService:
    ...
    async def detail(self, act_id: str, revision_id: str | None = None) -> dict[str, Any] | None:
        ...
        linked = {
            ...
            "packages": sorted({row.entity_id for row in binding_rows if str(row.entity_type.value if hasattr(row.entity_type, 'value') else row.entity_type) == 'pack'}),
            "documents": sorted({row.entity_id for row in binding_rows if str(row.entity_type.value if hasattr(row.entity_type, 'value') else row.entity_type) == 'document'}),
            "risks": sorted({str((row.context or {}).get('risk_id')) for row in binding_rows if (row.context or {}).get('risk_id')}),
            "checklists": sorted({str((row.context or {}).get('checklist_id')) for row in binding_rows if (row.context or {}).get('checklist_id')}),
            "workflows": sorted({str((row.context or {}).get('workflow_definition_id')) for row in binding_rows if (row.context or {}).get('workflow_definition_id')}),
            "roles": sorted({str((row.context or {}).get('role_code')) for row in binding_rows if (row.context or {}).get('role_code')}),
            "sites": sorted({str((row.context or {}).get('site_id')) for row in binding_rows if (row.context or {}).get('site_id')}),
        }
        summary = {key: len(value) for key, value in linked.items()}
        return {
            "act": {"id": act.id, "code": act.code, "title": act.title, "edition": act.edition, "valid_from": act.valid_from.isoformat() if act.valid_from else None, "valid_to": act.valid_to.isoformat() if act.valid_to else None},
            "revisions": [
                {"id": r.id, "revision_code": r.revision_code, "title": r.title, "effective_from": r.effective_from.isoformat() if r.effective_from else None, "effective_to": r.effective_to.isoformat() if r.effective_to else None, "change_summary": r.change_summary}
                for r in revisions
            ],
            ...
            "summary": summary,
            "tasks_to_create": [
                {"code": f"npa-update-{key}", "title": f"Актуализировать зависимости НПА: {key}", "count": count}
                for key, count in summary.items() if count > 0
            ],
        }
