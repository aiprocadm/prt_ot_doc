...
def add_tenant_validation(content: str) -> str:
    ...
    def replacer(match):
        ...
        if "TenantContextValidator.ensure_tenant_context" in content[match.start():match.end()+200]:
            return match.group(0)
        
        # Extract function name for context
        func_name = re.search(r"def (\w+)\(", func_sig).group(1) if re.search(r"def (\w+)\(", func_sig) else "operation"
