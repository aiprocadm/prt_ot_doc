    ...
    monkeypatch.setattr(
        ...
        AsyncMock(
            return_value=RenderedDoc(
                content=b"DOCX", filename="sout-declaration.docx", media_type=_DOCX
            )
        ),
    )
...
async def test_declaration_print_503_when_pdf_unavailable(monkeypatch):
    monkeypatch.setattr(routes, "_require_sout_enabled", AsyncMock())
    monkeypatch.setattr(
        routes, "render_declaration", AsyncMock(side_effect=PdfRendererUnavailable("no soffice"))
    )
    with pytest.raises(Exception) as exc:
        await routes.print_declaration(
            cid="c1",
    ...
    monkeypatch.setattr(
        ...
        lambda **k: [
            ...
            DeclarationRow(
                ...
                "N1",
                False,
                "класс 3.1 — вредные/опасные условия",
            ),
        ],
    )
