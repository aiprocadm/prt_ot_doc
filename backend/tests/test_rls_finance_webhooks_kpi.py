"""SEC-65 RLS — finance/webhooks/KPI/automation slice — db-marked (PostgreSQL only).

Confirms the ``20260726_sec65_rls_finance_webhooks_kpi`` migration armed all 16
finance/billing-telemetry/webhook/read-model/automation tables with RLS + FORCE +
the ``tenant_isolation`` policy. No backfill to test: unlike ``edo_messages``,
both webhook tables carry an enforced tenant FK, so legacy slug rows cannot
exist. Policy semantics are proven generically in ``test_rls_committees.py``.
"""
...
@pytest.mark.skipif(
    not ADMIN_URL, reason="set TEST_PG_ADMIN_URL to run the RLS finance/webhooks/KPI guard"
)
def test_migration_arms_all_domain_tables() -> None:
    ...
    dbname = f"rls_finweb_{uuid.uuid4().hex[:12]}"
