...
async def test_preview_ok(monkeypatch):
    ...
    out = await routes.import_preview(
        cid="c1", tenant=_tenant(), session=AsyncMock(), access=None, file=_file()
    )
...
async def test_preview_404_when_campaign_missing(monkeypatch):
    ...
    with pytest.raises(Exception) as exc:
        await routes.import_preview(
            cid="nope", tenant=_tenant(), session=AsyncMock(), access=None, file=_file()
        )
...
async def test_preview_422_on_unsupported_format(monkeypatch):
    monkeypatch.setattr(routes, "_require_sout_enabled", AsyncMock())
    monkeypatch.setattr(
        routes, "preview_import", AsyncMock(side_effect=UnsupportedImportFormat("pdf"))
    )
    with pytest.raises(Exception) as exc:
        await routes.import_preview(
            cid="c1", tenant=_tenant(), session=AsyncMock(), access=None, file=_file("r.pdf")
        )
...
async def test_apply_ok(monkeypatch):
    monkeypatch.setattr(routes, "_require_sout_enabled", AsyncMock())
    monkeypatch.setattr(
        routes, "_get_campaign", AsyncMock(return_value=SimpleNamespace(id="c1", status="planned"))
    )
...
async def test_apply_422_on_validation_error(monkeypatch):
    ...
    monkeypatch.setattr(
        routes,
        "apply_import",
        AsyncMock(side_effect=ImportValidationError(["строка 1: пустая должность"])),
    )
    with pytest.raises(Exception) as exc:
        await routes.import_apply(
            cid="c1", tenant=_tenant(), session=AsyncMock(), access=None, file=_file()
        )
    assert getattr(exc.value, "status_code", None) == 422
