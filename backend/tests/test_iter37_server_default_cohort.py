...
def test_migration_revision_chains_to_iter32() -> None:
    ...
    assert (
        isinstance(rev, ast.Constant) and rev.value == "20260529_iter37_server_default_ab"
    ), f"revision = {getattr(rev, 'value', None)!r}"
    assert (
        isinstance(down, ast.Constant) and down.value == "20260528_iter32_business_drift"
...
@pytest.mark.parametrize(("table", "column", "_sa_type", "_arg", "_default"), _COHORT_AB)
...
@pytest.mark.parametrize(("table", "column", "sa_type", "_arg", "_default"), _COHORT_AB)
...
@pytest.mark.parametrize(("table", "column", "_sa_type", "_arg", "default"), _COHORT_AB)
