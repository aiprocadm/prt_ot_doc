"""SEC-65 RLS — notifications/calendar/portal/NPA slice — db-marked (PostgreSQL only).

Confirms the ``20260726_sec65_rls_notifications_calendar_portal`` migration armed
all 14 notifications/calendar/client-portal/NPA tables with RLS + FORCE + the
...
@pytest.mark.skipif(
    not ADMIN_URL, reason="set TEST_PG_ADMIN_URL to run the RLS notifications/calendar/portal guard"
)
def test_migration_arms_all_domain_tables() -> None:
    ...
    dbname = f"rls_notcal_{uuid.uuid4().hex[:12]}"
