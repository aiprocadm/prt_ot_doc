"""SEC-65 RLS — org-structure registry slice — db-marked (PostgreSQL only).

Confirms the ``20260724_sec65_rls_org_registry`` migration armed all 9 backbone tables
with RLS + FORCE + the ``tenant_isolation`` policy. Policy semantics are proven
...
@pytest.mark.skipif(not ADMIN_URL, reason="set TEST_PG_ADMIN_URL to run the RLS org-registry guard")
def test_migration_arms_all_org_tables() -> None:
    async def _run() -> None:
        import asyncpg
    ...
    dbname = f"rls_org_{uuid.uuid4().hex[:12]}"
