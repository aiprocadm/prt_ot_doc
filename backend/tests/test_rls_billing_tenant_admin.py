"""SEC-65 RLS — billing-gate & tenant-admin slice — db-marked (PostgreSQL only).

Confirms the ``20260726_sec65_rls_billing_tenant_admin`` migration armed the 6
previously deferred billing/tenant-admin tables with RLS + FORCE + the
``tenant_isolation`` policy. The enabling rework (tenant-pinned billing-guard
session, tenant-pinned TenantMiddleware settings lookup, ``rls_bypass`` fleet
and provisioning sessions) ships in the same change. Policy semantics are proven
generically in ``test_rls_committees.py``.
"""
...
@pytest.mark.skipif(
    not ADMIN_URL, reason="set TEST_PG_ADMIN_URL to run the RLS billing/tenant-admin guard"
)
def test_migration_arms_all_domain_tables() -> None:
    ...
    dbname = f"rls_billadm_{uuid.uuid4().hex[:12]}"
