...
def test_is_expired():
    ...
    assert lc.is_expired("active", date(2026, 6, 15), today) is False
    assert lc.is_expired("active", None, today) is False
    assert lc.is_expired("revoked", date(2026, 1, 1), today) is False
