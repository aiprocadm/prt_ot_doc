"""SEC-65 RLS — medical domain slice — db-marked (PostgreSQL only).

Confirms the ``20260722_sec65_rls_medical`` migration armed all 7 medical tables
with RLS + FORCE + the ``tenant_isolation`` policy. Policy *semantics* (isolation,
fail-closed, bypass, WITH CHECK) are already proven generically in
``test_rls_committees.py`` and are identical here (same predicate), so this slice
only asserts the migration reached the medical tables.
"""
...
@pytest.mark.skipif(not ADMIN_URL, reason="set TEST_PG_ADMIN_URL to run the RLS medical guard")
def test_migration_arms_all_medical_tables() -> None:
    async def _run() -> None:
        import asyncpg
    ...
    dbname = f"rls_med_{uuid.uuid4().hex[:12]}"
