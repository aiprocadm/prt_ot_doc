...
**Срез-135: модель ``ReminderRule`` удалена из кода** — её не читал и не писал
никто. Таблица, RLS и миграции ОСТАЛИСЬ: данные арендаторов трогать нельзя, а
вернуть механизм (уже с ручками) проще на существующей схеме. Список таблиц
под снос — в ``docs/CLEANUP_CANDIDATES.md``.
"""
