...
def _raise_quota(
    kind: str, *, limit: int, used: int, billing_gate: bool, correlation_id: str | None = None
) -> None:
    ...
    raise HTTPException(
        ...
        detail={
            ...
            "meta": {"kind": kind, "limit": limit, "used": used},
        },
    )


async def check_concurrent_jobs(
    session: AsyncSession, *, tenant: Tenant, quota: TenantQuota
) -> None:
    ...
    if used + 1 > limit:
        _raise_quota(
            ...
            limit=limit,
            used=used,
            billing_gate=bool(quota.enforce_billing_gate),
        )


async def check_monthly_counters(
    session: AsyncSession, *, tenant: Tenant, quota: TenantQuota, delta: int = 1
) -> None:
    ...
    counter = (
        await session.execute(
            select(TenantCounter).where(
                TenantCounter.tenant_id == tenant.id, TenantCounter.yyyymm == period_counter
            )
        )
    ).scalar_one_or_none()
...
async def check_storage(
    session: AsyncSession, *, tenant: Tenant, quota: TenantQuota, delta: int = 0
) -> None:
    used = await session.scalar(
        select(func.coalesce(func.sum(File.size), 0)).where(File.tenant_id == tenant.id)
    )
    used = int(used or 0)
    limit = int(quota.max_storage_mb) * 1024 * 1024
    if used + delta > limit:
        _raise_quota(
            "max_storage_mb", limit=limit, used=used, billing_gate=bool(quota.enforce_billing_gate)
        )
