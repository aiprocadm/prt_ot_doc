...
class Settings(BaseSettings):
    ...
    # BIZ-53 срез-3 (разд. 53.2): самостоятельная регистрация арендатора.
    # ВЫКЛЮЧЕНА ПО УМОЛЧАНИЮ ОСОЗНАННО: включённая публичная ручка создаёт схему
    # в базе на каждый запрос из интернета — это и расход, и вектор атаки.
    # Владелец включает её тогда, когда готов принимать поток, а не «внезапно
    # после обновления».
    self_service_signup_enabled: bool = Field(False, alias="SELF_SERVICE_SIGNUP_ENABLED")
    # Отдельный, гораздо более строгий счётчик, чем общий портальный: создание
    # арендатора несопоставимо дороже обычного запроса.
    self_service_signup_per_ip: str = Field("3/hour", alias="SELF_SERVICE_SIGNUP_PER_IP")
    # SEC-68 (разд. 68.2): «CAPTCHA/anti-bot на публичных формах». Лимит по
    # адресу не мешает боту с сотней адресов — именно так делают
    # регистрационный спам. Служба проверки подключается НАСТРОЙКОЙ: пусто —
    # проверки нет (и об этом пишется предупреждение в журнал при включённой
    # регистрации), "turnstile"/"recaptcha" — есть.
    signup_antibot_provider: str = Field("", alias="SIGNUP_ANTIBOT_PROVIDER")
    # ИМЯ переменной окружения с секретом, а не сам секрет (приём среза-204):
    # секрет в настройках пережил бы их резервную копию и любую выгрузку.
    signup_antibot_secret_env: str = Field(
        "SIGNUP_ANTIBOT_SECRET", alias="SIGNUP_ANTIBOT_SECRET_ENV"
    )
    ...
    # тяжёлых подпроцессов (LibreOffice-конвертация). Память — щедрая: soffice
    # легитимно ест сотни мегабайт на больших документах.
