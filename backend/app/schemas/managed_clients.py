...
class ClientContourSession(BaseSchema):
    """Вход в СОБСТВЕННЫЙ контур Dedicated-клиента (разд. 49.3).

    У Lightweight-клиента данные лежат в пространстве аутсорсера, и работать с
    ними можно тем же токеном. У Dedicated они в другом арендаторе, поэтому
    выдаётся отдельный токен — к контуру клиента, на личность специалиста,
    заведённую там платформой.

    Обновления (refresh) у этого токена НЕТ намеренно: обновление пережило бы
    и срок контекста, и отзыв согласия. Продлить работу можно только входом в
    контекст заново — а он каждый раз перепроверяет основание.
    """
...
class ClientContextRead(BaseSchema):
    ...
    #: Вход в контур Dedicated-клиента. ``None`` у Lightweight — там контура
    #: нет, и пустое поле честнее выдуманного токена «в никуда».
    contour: ClientContourSession | None = None
    #: Почему контура нет, если его нет. Пусто, когда он выдан или не нужен.
