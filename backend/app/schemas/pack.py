...
class PackScenarioField(BaseSchema):
    ...
    required: bool
    #: ПОДСКАЗКА ИЗ РЕЕСТРА, а НЕ ответ (решение владельца, handoff #960).
    #:
    #: Значение НЕ подставляется в документ само собой: отчёт подписывает
    #: специалист, и он отвечает за каждое число в нём. Подставь платформа
    #: число молча — человек подписал бы то, чего не проверял.
    suggested: str | None = None
    #: Откуда взята подсказка («ДТП за 365 дн. по учёту ДТП»). ОБЯЗАТЕЛЕН
    #: рядом со значением: число без источника проверить нельзя, а значит
...
class PackScenarioFieldsResponse(BaseSchema):
    ...
    #: Пояснение про подсказки — словами и на экране, а не только в коде:
