...
class AnalyticsAggregationService:
    ...
    async def detailed_counters(self, filters: DashboardFilters) -> dict[str, int]:
        ...
        workflow_open = int(await self.session.scalar(select(func.count()).select_from(WorkflowTask).where(WorkflowTask.tenant_id == self.tenant_id, WorkflowTask.status == WorkflowTaskStatus.OPEN, WorkflowTask.deleted_at.is_(None))) or 0)
        workflow_sla_breached = int(await self.session.scalar(select(func.count()).select_from(WorkflowTask).where(WorkflowTask.tenant_id == self.tenant_id, WorkflowTask.status == WorkflowTaskStatus.OPEN, WorkflowTask.due_at.is_not(None), WorkflowTask.due_at < func.now(), WorkflowTask.deleted_at.is_(None))) or 0)
        plan_overdue = int(await self.session.scalar(select(func.count()).select_from(PlanTask).where(PlanTask.tenant_id == self.tenant_id, PlanTask.status.in_([PlanTaskStatus.OPEN, PlanTaskStatus.OVERDUE]), PlanTask.due_at.is_not(None), PlanTask.due_at < func.now(), PlanTask.deleted_at.is_(None))) or 0)
        notifications_integration_errors = int(await self.session.scalar(select(func.count()).select_from(Notification).where(Notification.tenant_id == self.tenant_id, Notification.type == NotificationType.INTEGRATION_ERROR, Notification.deleted_at.is_(None))) or 0)
        notifications_edo_changes = int(await self.session.scalar(select(func.count()).select_from(Notification).where(Notification.tenant_id == self.tenant_id, Notification.type == NotificationType.EDO_STATUS_CHANGED, Notification.deleted_at.is_(None))) or 0)
        notifications_failed = int(await self.session.scalar(select(func.count()).select_from(Notification).where(Notification.tenant_id == self.tenant_id, Notification.status == NotificationStatus.FAILED, Notification.deleted_at.is_(None))) or 0)
        return {
...
class KpiDashboardService:
    ...
    async def incidents(self, filters: DashboardFilters) -> dict[str, Any]:
        ...
        return {"name": "incidents", "widgets": {k: detailed[k] for k in ["incidents_open", "workflow_open", "integration_errors", "failed_notifications"]} | {"packages_total": counters["packages_total"]}}

    async def inspections(self, filters: DashboardFilters) -> dict[str, Any]:
        counters = await self.agg.base_counters(filters)
        detailed = await self.agg.detailed_counters(filters)
        return {"name": "inspections", "widgets": {k: detailed[k] for k in ["inspections_open", "plan_tasks_overdue", "workflow_sla_breached"]} | {"open_incidents": counters["open_incidents"]}}

    async def prescriptions(self, filters: DashboardFilters) -> dict[str, Any]:
        detailed = await self.agg.detailed_counters(filters)
        return {"name": "prescriptions", "widgets": {k: detailed[k] for k in ["prescriptions_overdue", "workflow_open", "workflow_sla_breached", "plan_tasks_overdue"]}}

    async def overdue(self, filters: DashboardFilters) -> dict[str, Any]:
        counters = await self.agg.base_counters(filters)
        detailed = await self.agg.detailed_counters(filters)
        return {"name": "overdue", "widgets": {
