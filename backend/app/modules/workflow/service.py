...
class WorkflowService:
    ...
    async def create_definition(
        ...
        *,
        ...
        current_max = await self.session.scalar(
            select(WorkflowDefinitionVersion.version_no)
            .where(
                WorkflowDefinitionVersion.tenant_id == self.tenant_id,
        ...
        await self.session.flush()
        await self._timeline_like_audit(
            "workflow.definition.create",
            actor_user_id,
            definition.id,
            {"code": code, "version_no": version.version_no},
        )
        return version
    ...
    def validate_graph(self, graph: dict[str, Any]) -> None:
        ...
        if not nodes:
            raise HTTPException(
                status.HTTP_422_UNPROCESSABLE_ENTITY, "workflow graph requires nodes"
            )
        node_ids = set()
        start_count = 0
        end_count = 0
        ...
        for node in nodes:
            ...
            if not node_id or node_id in node_ids:
                raise HTTPException(
                    status.HTTP_422_UNPROCESSABLE_ENTITY, "workflow node ids must be unique"
                )
        ...
        if start_count != 1:
            raise HTTPException(
                status.HTTP_422_UNPROCESSABLE_ENTITY,
                "workflow graph requires exactly one start node",
            )
        if end_count < 1:
            raise HTTPException(
                status.HTTP_422_UNPROCESSABLE_ENTITY,
                "workflow graph requires at least one end node",
            )
        for transition in transitions:
            if transition.get("from") not in node_ids or transition.get("to") not in node_ids:
                raise HTTPException(
                    status.HTTP_422_UNPROCESSABLE_ENTITY,
                    "workflow transition references unknown node",
                )
        if not any(transition.get("from") == start_node_id for transition in transitions):
            raise HTTPException(
                status.HTTP_422_UNPROCESSABLE_ENTITY,
                "workflow start node must have at least one outgoing transition",
            )
        ...
        if unreachable:
            raise HTTPException(
                status.HTTP_422_UNPROCESSABLE_ENTITY,
                f"workflow graph contains unreachable nodes: {', '.join(sorted(unreachable))}",
            )
        ...
        if not any(end_node in visited for end_node in terminal_nodes):
            raise HTTPException(
                status.HTTP_422_UNPROCESSABLE_ENTITY, "workflow graph must reach an end node"
            )

    async def publish_version(
        self, version_id: str, actor_user_id: str | None
    ) -> WorkflowDefinitionVersion:
            ...
            (
                await self.session.execute(
                    select(WorkflowDefinitionVersion).where(
                        ...
                        WorkflowDefinitionVersion.status == WorkflowDefinitionStatus.PUBLISHED,
                    )
                )
            )
            .scalars()
    ...
    async def list_instances(
        self, *, status_filter: WorkflowInstanceStatus | None = None, entity_type: str | None = None
    ) -> list[tuple[WorkflowInstance, int]]:
        stmt = (
            ...
            .where(
                WorkflowInstance.tenant_id == self.tenant_id, WorkflowInstance.deleted_at.is_(None)
            )
            ...
            (
                await self.session.execute(
                    ...
                    .where(
                        ...
                        WorkflowDefinitionVersion.deleted_at.is_(None),
                    )
                    .order_by(WorkflowDefinitionVersion.version_no.desc())
                )
            )
    ...
    async def start_instance(
        ...
        *,
        ...
        if version_id:
            ...
            if definition and definition.current_version_id:
                version = await self.session.get(
                    WorkflowDefinitionVersion, definition.current_version_id
                )
        if version is None or version.tenant_id != self.tenant_id:
        ...
        if entity_type != definition.entity_type:
            raise HTTPException(
                status.HTTP_422_UNPROCESSABLE_ENTITY,
                "workflow entity_type does not match process definition",
            )
    ...
    async def list_tasks(
        self, *, user_id: str | None, role_codes: list[str] | None = None
    ) -> list[WorkflowTask]:
        ...
        if user_id:
            ...
            stmt = stmt.where(
                or_(
                    ...
                    WorkflowTask.assignee_role_code.in_(role_codes),
                )
            )
        return (
            (
                await self.session.execute(
                    stmt.order_by(
                        WorkflowTask.due_at.asc().nullslast(), WorkflowTask.created_at.desc()
                    )
                )
            )
            .scalars()
    ...
    async def complete_task(
        ...
        *,
        task_id: str,
        actor_user_id: str | None,
        actor_role_codes: list[str] | None,
        decision: str | None,
        ...
        self._ensure_task_actor_allowed(
            task=task, actor_user_id=actor_user_id, actor_role_codes=actor_role_codes
        )
        ...
        await self._append_event(
            ...
            actor_user_id,
            {"task_id": task.id, "decision": decision},
        )
        await self._timeline_like_audit(
            "workflow.task.complete",
            actor_user_id,
            task.id,
            {"instance_id": task.instance_id, "node_id": task.node_id, "decision": decision},
        )
    ...
    ) -> tuple[WorkflowInstance, list[WorkflowTimelineEvent], list[WorkflowTask]]:
        ...
        events = (
            (
                await self.session.execute(
                    ...
                    .where(
                        WorkflowTimelineEvent.tenant_id == self.tenant_id,
                        WorkflowTimelineEvent.instance_id == instance_id,
                    )
                    .order_by(WorkflowTimelineEvent.created_at.asc())
                )
            )
            .scalars()
            .all()
            ...
            (
                await self.session.execute(
                    select(WorkflowInstance).where(
                        ...
                        WorkflowInstance.current_node_id.is_not(None),
                    )
                )
            )
            .scalars()
        ...
        for instance in rows:
            ...
            if node and node.get("type") == "timer":
                ...
                if due_at_raw and datetime.fromisoformat(str(due_at_raw)) <= now:
                    await self._append_event(
                        instance.id, node["id"], "timer_fired", None, {"due_at": due_at_raw}
                    )
    ...
    async def _advance(
        ...
        *,
        ...
        transitions = graph.get("transitions") or []
        start_node_id = from_node_id or next(
            node["id"] for node in graph.get("nodes") or [] if node.get("type") == "start"
        )
        ...
        while next_node_id:
            ...
            await self._append_event(
                ...
                "node_entered",
                actor_user_id,
                {"node_type": node_type, "name": node.get("name")},
            )
            ...
            if node_type == "condition":
                next_node_id = self._next_node_id(
                    next_node_id, transitions, instance.context_json or {}
                )
                continue
            if node_type == "timer":
                ...
                instance.context_json = {
                    **(instance.context_json or {}),
                    f"timer_due::{next_node_id}": due_at.isoformat(),
                }
                instance.status = WorkflowInstanceStatus.WAITING
                await self.session.flush()
                return
            ...
            if node_type == "end":
                ...
                await self._append_event(
                    instance.id, next_node_id, "instance_completed", actor_user_id, {}
                )
                await self.session.flush()
                return
            next_node_id = self._next_node_id(
    ...
    def _next_node_id(
        self, current_node_id: str, transitions: list[dict[str, Any]], context: dict[str, Any]
    ) -> str | None:
    ...
    async def _timeline_like_audit(
        self, action: str, actor_user_id: str | None, entity_id: str, details: dict[str, Any]
    ) -> None:
    ...
    def _ensure_task_actor_allowed(
        *,
        ...
        if actor_user_id is None:
            raise HTTPException(
                status.HTTP_403_FORBIDDEN, "workflow task requires authenticated actor"
            )
        ...
        if not task.assignee_user_id and task.assignee_role_code:
            ...
            if task.assignee_role_code not in actor_role_codes and not allow_unassigned:
                raise HTTPException(
                    status.HTTP_403_FORBIDDEN, "workflow task requires matching role assignee"
                )
    ...
    async def sweep_task_sla(self, *, now: datetime | None = None) -> int:
        ...
        for task in rows:
            ...
            await self._append_event(
                ...
                "task_sla_escalated",
                None,
                {"task_id": task.id, "due_at": task.due_at.isoformat() if task.due_at else None},
            )
            await self.outbox.add_event(
                tenant_id=self.tenant_id,
                event_type="WorkflowTaskEscalated",
