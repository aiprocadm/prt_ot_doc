...
MODULE_EVENT_TYPES: dict[str, frozenset[str]] = {
    ...
    # Доп. №1 разд. 56.2 срез-1: реестр ТС своих событий не испускает — пустая
    # запись обязательна по тому же правилу (урок #948: без неё падает смена
    # тарифа у ВСЕХ арендаторов).
    "road_safety": frozenset(),
    "warehouse": frozenset(),
}
