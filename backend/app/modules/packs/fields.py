...
SCENARIO_FIELDS: dict[str, tuple[PackField, ...]] = {
    ...
    # Формирования, личный состав и категория объекта с срезом-43 ПОДСКАЗЫВАЮТСЯ
    # из реестров контура (prefill.py), но ответом не становятся.
    ...
    # Числа берутся из реестров контура и вносятся по мере готовности: с
    # решением #960 платформа их ПОДСКАЗЫВАЕТ (prefill.py), но ответом они
    # становятся только рукой специалиста (то же у экологии, ГО-ЧС и ПромБеза).
