...
def ensure_embedded_fonts(pdf_path: Path) -> None:
    ...
    if proc.returncode != 0:
        raise PdfFontsValidationError(
            "PDF_FONTS_CHECK_FAILED", proc.stderr.strip() or "pdffonts_failed"
        )

    lines = [line.strip() for line in proc.stdout.splitlines() if line.strip()]
    data_lines = [
        line for line in lines if not line.startswith("name") and not line.startswith("-")
    ]
