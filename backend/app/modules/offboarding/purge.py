...
# Срез-224: здесь стояла запись ``"incident_persons": {"fio_text": "NULL"}`` —
# «свободный текст с ФИО пострадавшего». Колонки ``fio_text`` нет ни у одной
# ЖИВОЙ таблицы: она существует только в двух таблицах-дубликатах прежних волн
# (``incident_persons``, ``training_protocol_items``), у которых нет моделей и
# которые новый код не пишет. У настоящей ``incident_person`` поле ``person_id``
# обязательное, то есть свободного ФИО там не бывает вовсе.
#
# Запись была не просто лишней, а вредной: обезличивание применяется ТОЛЬКО к
# удерживаемым таблицам, а после починки карты процессов (срез-224) мёртвый
# дубликат больше не удерживается — он удаляется ЦЕЛИКОМ, вместе со всем, что
# могло остаться в нём от прежних волн. Удаление строже обезличивания, поэтому
# запись убрана, а не переписана: иначе список обещал бы защиту, которой нет.
