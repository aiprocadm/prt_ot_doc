            ...
            ImportWorkplaceRow(
                ...
                factors=[
                    ImportFactorRow(
                        code=f.code,
                        name=f.name,
                        parsed_class=f.measured_class,
                        class_unparsed=f.class_unparsed,
                    )
                    for f in wp.factors
                ],
                errors=ri.errors,
    ...
    for d in diff:
        if d.change == "removed":
            rows.append(
                ImportWorkplaceRow(
                    row_index=-1,
                    workplace_code=d.workplace_code,
                    position_name="",
                    parsed_class=None,
                    current_class=d.current_class,
                    change="removed",
    ...
    for wp in parsed:
        ...
        if change == "new":
            ...
            for f in wp.factors:
                session.add(
                    SoutFactor(
                        ...
                        code=f.code,
                        name=f.name,
                        measured_class=_to_class(f.measured_class),
                    )
                )
            hist = build_class_history_row(
                ...
                workplace_id=row.id,
                old_class=None,
                new_class=_to_class(wp.assessed_class),
            )
            if hist is not None:
                session.add(hist)
            created += 1
