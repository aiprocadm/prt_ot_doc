...
class BootstrapTenantService:
    ...
    async def run(
        ...
        *,
        ...
        await self._log_bootstrap_event(tenant_id=tenant_id, tenant_slug=tenant_slug, dry_run=dry_run, summary=summary)
    ...
    async def _ensure_quota(self, *, tenant_id: str, dry_run: bool, summary: BootstrapTenantSummary) -> None:
        existing = (await self.session.execute(select(TenantQuota).where(TenantQuota.tenant_id == tenant_id))).scalar_one_or_none()
        if existing:
        ...
        if dry_run:
            ...
            return
        self.session.add(TenantQuota(tenant_id=tenant_id, max_parallel_jobs=4, max_doc_generations_per_month=2500, max_storage_mb=5120, enforce_billing_gate=False))
        summary.mark(entity="tenant_quota", created=True)

    async def _ensure_owner(self, *, tenant_id: str, owner_email: str, owner_password: str, dry_run: bool, summary: BootstrapTenantSummary) -> None:
        existing = (await self.session.execute(select(User).where(User.tenant_id == tenant_id, func.lower(User.email) == owner_email.lower()))).scalar_one_or_none()
        if existing:
        ...
        user = User(tenant_id=tenant_id, email=owner_email.lower(), full_name="Tenant Owner", role=RoleEnum.OWNER, hashed_password=hash_password(owner_password), is_active=True)
    ...
    async def _ensure_company_profile(self, *, tenant_id: str, tenant_name: str, dry_run: bool, summary: BootstrapTenantSummary) -> None:
        existing = (await self.session.execute(select(Company).where(Company.tenant_id == tenant_id))).scalars().first()
        if existing:
    ...
    async def _seed_starter_pack(self, *, tenant_id: str, tenant_slug: str, dry_run: bool, demo: bool, summary: BootstrapTenantSummary) -> None:
        ...
        existing = (await self.session.execute(select(Tenant).where(Tenant.id == tenant_id))).scalar_one_or_none()
        if existing and isinstance(existing.settings, dict) and existing.settings.get("starter_pack"):
        ...
        if existing is not None:
            existing.settings = {**(existing.settings or {}), "starter_pack": payload, "bootstrap_state": {"configured": False, "remaining_actions": ["setup_sites", "import_employees", "configure_integrations"]}, "storage_prefixes": [f"tenants/{tenant_slug}/exports", f"tenants/{tenant_slug}/uploads", f"tenants/{tenant_slug}/archives"]}
        summary.mark(entity="starter_pack", created=True)

    async def _seed_package_presets(self, *, tenant_id: str, dry_run: bool, summary: BootstrapTenantSummary) -> None:
        defaults = [
        ...
        for profile_name, preset_name, code in defaults:
            profile = (
                await self.session.execute(
                    select(PackageProfile).where(PackageProfile.tenant_id == tenant_id, PackageProfile.name == profile_name)
                )
            ).scalar_one_or_none()
            if profile is None and not dry_run:
                profile = PackageProfile(tenant_id=tenant_id, name=profile_name, description=f"Стартовый профиль: {profile_name}", config={"code": code})
                self.session.add(profile)
                await self.session.flush()
                created_any = True
            existing = (await self.session.execute(select(PackagePreset).where(PackagePreset.tenant_id == tenant_id, PackagePreset.name == preset_name))).scalar_one_or_none()
            if existing:
                continue
            if dry_run:
                continue
    ...
    async def _log_bootstrap_event(self, *, tenant_id: str, tenant_slug: str, dry_run: bool, summary: BootstrapTenantSummary) -> None:
        if dry_run:
        ...
        await AuditService(self.session).log_event(
            ...
            details={"tenant_slug": tenant_slug, "created": summary.created, "reused": summary.reused},
        )
        summary.mark(entity="audit_event", created=True)
