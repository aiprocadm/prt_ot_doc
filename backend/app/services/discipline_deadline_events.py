...
* **Ядро — только по названной причине.** У медосмотров и СИЗ события
  просрочки свои, второе дало бы две задачи. У срока удостоверения по
  обучению события нет (срез-76), поэтому он в обходе — единственный из
  ядра, список ``CORE_DEADLINE_SOURCES`` закрытый.
...
#: даёт. Второй задачи на ту же просрочку взяться неоткуда. Список закрытый:
#: новый источник ядра сюда попадает только с названной причиной.
CORE_DEADLINE_SOURCES: frozenset[str] = frozenset({"training_certificate"})
