...
celery_app.conf.beat_schedule = {
    ...
    # Срез-170: сроки и таймеры согласований. До этого обе работы были написаны,
    # но принимали арендатора параметром и потому в расписание не попадали —
    # эскалация просроченных задач и созревшие таймеры не срабатывали никогда.
    "workflow-sweep": {
        "task": "workflow.sweep.tick",
    ...
    # Срез-231: уборка ключей идемпотентности. Работа написана давно и настройка
    # срока хранения есть (`IDEMPOTENCY_TTL_DAYS`, по умолчанию 30 дней), но в
    # расписании задачи НЕ БЫЛО и не звал её никто — единственный путь удаления
    # записей не выполнялся ни разу. Ключи копились вечно у каждого арендатора,
    # а настроенный срок не значил ничего.
    #
    # Раз в сутки и в ночной полосе: удаление идёт по всем арендаторам подряд и
    # трогает только завершённые записи старше срока.
