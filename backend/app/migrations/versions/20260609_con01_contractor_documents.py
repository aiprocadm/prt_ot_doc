...
_ACCESS_STATUS = postgresql.ENUM(
    *_COMPLIANCE_VALUES, name="contractor_access_status", create_type=False
)
_TRAINING_STATUS = postgresql.ENUM(
    *_COMPLIANCE_VALUES, name="contractor_training_status", create_type=False
)
_MEDICAL_STATUS = postgresql.ENUM(
    *_COMPLIANCE_VALUES, name="contractor_medical_status", create_type=False
)
_INCIDENT_SEVERITY = postgresql.ENUM(
    *_SEVERITY_VALUES, name="contractor_incident_severity", create_type=False
)
...
def upgrade() -> None:
    ...
    if not _has_table("contractor_registry"):
        ...
        op.create_index(
            "ix_contractor_registry_tenant_status", "contractor_registry", ["tenant_id", "status"]
        )
        op.create_index(
            "ix_contractor_registry_tenant_company",
            "contractor_registry",
    ...
    if not _has_table("contractor_employees"):
        op.create_table(
            ...
            sa.Column(
                "contractor_id",
                sa.String(length=36),
                sa.ForeignKey("contractor_registry.id", ondelete="CASCADE"),
                nullable=False,
            ),
            ...
            sa.Column(
                "training_status", _TRAINING_STATUS, nullable=False, server_default="PENDING"
            ),
        ...
        op.create_index(
            "ix_contractor_employees_contractor_id", "contractor_employees", ["contractor_id"]
        )
        op.create_index(
            "ix_contractor_employees_tenant_contractor",
            "contractor_employees",
            ["tenant_id", "contractor_id"],
        )
        op.create_index(
            "ix_contractor_employees_tenant_training",
            "contractor_employees",
            ["tenant_id", "training_status"],
        )
        op.create_index(
            "ix_contractor_employees_tenant_medical",
            "contractor_employees",
            ["tenant_id", "medical_status"],
        )
    ...
    if not _has_table("contractor_incidents"):
        op.create_table(
            ...
            sa.Column(
                "employee_id",
                sa.String(length=36),
                sa.ForeignKey("contractor_employees.id", ondelete="SET NULL"),
                nullable=True,
            ),
        ...
        op.create_index(
            "ix_contractor_incidents_contractor_id", "contractor_incidents", ["contractor_id"]
        )
        op.create_index(
            "ix_contractor_incidents_employee_id", "contractor_incidents", ["employee_id"]
        )
        op.create_index(
            "ix_contractor_incidents_tenant_contractor",
            "contractor_incidents",
            ["tenant_id", "contractor_id"],
        )
        op.create_index(
            "ix_contractor_incidents_tenant_status", "contractor_incidents", ["tenant_id", "status"]
        )
    ...
    if not _has_table("contractor_documents"):
        ...
        op.create_index(
            "ix_contractor_documents_tenant_contractor",
            "contractor_documents",
            ["tenant_id", "contractor_id"],
        )
        op.create_index(
            "ix_contractor_documents_tenant_employee",
            "contractor_documents",
            ["tenant_id", "employee_id"],
        )
        op.create_index(
            "ix_contractor_documents_tenant_valid",
            "contractor_documents",
            ["tenant_id", "valid_until"],
        )
        op.create_index(
            "ix_contractor_documents_tenant_type", "contractor_documents", ["tenant_id", "doc_type"]
        )
