...
_TASK_STATUS = sa.Enum("open", "in_progress", "done", name="decisiontaskstatus")
...
def upgrade() -> None:
    ...
    op.create_table(
        "committee_member",
        *_common(
            sa.Column("committee_id", sa.String(length=36), sa.ForeignKey("committee.id", ondelete="CASCADE"), nullable=False, index=True),
            sa.Column("person_id", sa.String(length=36), sa.ForeignKey("person.id", ondelete="RESTRICT"), nullable=False, index=True),
            sa.Column("role", _MEMBER_ROLE, nullable=False),
        ),
        sa.UniqueConstraint("tenant_id", "committee_id", "person_id", "role", name="uq_committee_member"),
    )
    op.create_table(
        "committee_meeting",
        *_common(
    ...
    op.create_table(
        ...
        *_common(
            sa.Column("meeting_id", sa.String(length=36), sa.ForeignKey("committee_meeting.id", ondelete="CASCADE"), nullable=False, index=True),
            sa.Column("seq", sa.Integer(), nullable=False, server_default="1"),
            sa.Column("title", sa.String(length=500), nullable=False),
            sa.Column("presenter_person_id", sa.String(length=36), sa.ForeignKey("person.id", ondelete="SET NULL"), nullable=True),
        ),
    )
    op.create_table(
        "committee_decision",
        *_common(
            sa.Column("meeting_id", sa.String(length=36), sa.ForeignKey("committee_meeting.id", ondelete="CASCADE"), nullable=False, index=True),
            sa.Column("agenda_item_id", sa.String(length=36), sa.ForeignKey("committee_agenda_item.id", ondelete="SET NULL"), nullable=True),
    ...
    op.create_table(
        ...
        *_common(
            sa.Column("decision_id", sa.String(length=36), sa.ForeignKey("committee_decision.id", ondelete="CASCADE"), nullable=False, index=True),
            sa.Column("assignee_person_id", sa.String(length=36), sa.ForeignKey("person.id", ondelete="SET NULL"), nullable=True),
