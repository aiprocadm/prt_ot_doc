...
def upgrade() -> None:
    with op.batch_alter_table('company') as batch:
        batch.add_column(sa.Column('branding_payload', sa.JSON(), nullable=False, server_default=sa.text("'{}'")))
        batch.add_column(sa.Column('preferred_header_preset_code', sa.String(length=64), nullable=True))
