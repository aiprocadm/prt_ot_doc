...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("issued_by", sa.String(length=36), sa.ForeignKey("user.id", ondelete="SET NULL"), nullable=True),
        sa.Column("result_exam_id", sa.String(length=36), sa.ForeignKey("medical_exam.id", ondelete="SET NULL"), nullable=True),
    )
    ...
    op.create_table(
        ...
        sa.Column("source_exam_id", sa.String(length=36), sa.ForeignKey("medical_exam.id", ondelete="SET NULL"), nullable=True),
        sa.Column("started_at", sa.DateTime(timezone=True), nullable=False),
        sa.Column("lifted_at", sa.DateTime(timezone=True), nullable=True),
        sa.Column("status", _SUSP_STATUS, nullable=False, server_default="active"),
        sa.Column("lifted_by", sa.String(length=36), sa.ForeignKey("user.id", ondelete="SET NULL"), nullable=True),
    )
...
def downgrade() -> None:
    ...
    if bind.dialect.name == "postgresql":
        for type_name in ("medicalsuspensionreason", "medicalsuspensionstatus", "medicalreferralstatus", "medicalfitness", "medicalexamkind"):
