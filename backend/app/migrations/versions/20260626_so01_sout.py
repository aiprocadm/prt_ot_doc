...
_GUARANTEE_KIND = sa.Enum(
    "additional_leave", "extra_pay", "reduced_hours", "milk", "early_pension",
    "medical_exam",
...
def upgrade() -> None:
    ...
    sout_class_ref = sa.Enum(
        "optimal", "acceptable", "harmful_3_1", "harmful_3_2", "harmful_3_3",
        "harmful_3_4", "dangerous",
        name="soutclass", create_type=False,
    )
    ...
    op.create_table(
        ...
        *_common(
            sa.Column("deleted_at", sa.DateTime(timezone=True), nullable=True),
            sa.Column("campaign_id", sa.String(length=36), sa.ForeignKey("sout_campaign.id", ondelete="CASCADE"), nullable=False, index=True),
    ...
    op.create_table(
        "sout_factor",
        *_common(
            sa.Column("workplace_id", sa.String(length=36), sa.ForeignKey("sout_workplace.id", ondelete="CASCADE"), nullable=False, index=True),
