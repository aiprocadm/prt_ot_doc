...
#: Вид срока экологической отчётности (Доп. №1 разд. 55.3 «сроки сдачи
#: отчётности, платежей»; разд. 57.2 «приближается срок сдачи 2-ТП»). ЗАКРЫТЫЙ
#: словарь: «отчёт» и «платёж» — разные задачи эколога, и по виду календарь
#: и Центр внимания отличают одно от другого. Замеры и продление разрешений
#: сюда НЕ входят: у них свои источники (``ecology_measurement``,
#: ``ecology_permit``), и второй срок на то же событие дал бы две строки.
REPORTING_KINDS: dict[str, str] = {
    "report": "Отчётность",
    "payment": "Платёж",
}

#: Состояние срока — ВЫЧИСЛЯЕТСЯ при чтении из даты исполнения и сегодняшнего
#: дня, в таблице не хранится: сохранённое «предстоит» пережило бы дату.
REPORTING_STATUS_TITLES: dict[str, str] = {
    "planned": "Предстоит",
    "overdue": "Просрочено",
    "done": "Исполнено",
}
...
class EcologyReportingDeadline(TenantBaseModel, SoftDeleteMixin):
    """Срок сдачи экологической отчётности или платежа (разд. 55.3, срез-71).

    ПОЧЕМУ СВОЯ СУЩНОСТЬ. Решение среза-23 — «сроки отчётности и платежей идут
    через существующий контур сроков, а не через вшитые в код 20-е число и
    10 марта» — оказалось без точки входа: ``ComplianceDeadline`` строится
    только из удостоверений обучения, а свободное событие календаря без API.
    Пример ТЗ «приближается срок сдачи 2-ТП» до Центра внимания не доходил
    никогда.

    ДАТУ ВНОСИТ ЭКОЛОГ. Сроки установлены нормативными актами и меняются
    (перенос на рабочий день, новые формы), поэтому платформа их НЕ назначает
    и НЕ вычисляет — та же граница, что у периодичности ПЭК и ставок платы.

    Исполненный срок (``done_on``) в календарь и Центр внимания не попадает:
    сданный отчёт — уже не срок. В реестре он остаётся ради истории.
    """

    __tablename__ = "nvos_reporting_deadline"

    kind: Mapped[str] = mapped_column(String(16), nullable=False)
    #: что именно сдаётся или платится: «2-ТП (отходы) за 2025 год»,
    #: «Плата за НВОС, 1 квартал 2026»
    title: Mapped[str] = mapped_column(String(255), nullable=False)
    #: отчётный период словами — свободная строка: у форм он разный
    #: (год, квартал, месяц), а гадать по названию нельзя
    period: Mapped[str | None] = mapped_column(String(32), nullable=True)
    due_on: Mapped[date] = mapped_column(Date, nullable=False)
    #: дата сдачи/уплаты; пусто — ещё не исполнено
    done_on: Mapped[date | None] = mapped_column(Date, nullable=True)
    responsible: Mapped[str | None] = mapped_column(String(255))
    notes: Mapped[str | None] = mapped_column(Text)

    __table_args__ = (Index("ix_reporting_deadline_tenant_due", "tenant_id", "due_on"),)
