...
class IncidentCase(TenantBaseModel, SoftDeleteMixin):
    """Происшествие расширенного контура — таблица без точки входа (срез-117).

    Заполнить её нельзя: ручек нет, сида нет, миграций с данными нет. Живой
    контур происшествий работает на ядровой ``Incident`` (регистрация, журнал,
    расследование, связь с CAPA, «давление» на карте рисков — срез-74). Слой
    жизненного цикла для этой модели (``IncidentCaseService`` и соседи) удалён
    срезом-117: он не имел потребителей и создавал второе, недостижимое
    описание того же процесса.

    Модель и таблица оставлены намеренно: у них миграции, индексы и политики
    RLS, а данные арендаторов не удаляют ради чистоты кода. Возвращать контур —
    только ВМЕСТЕ с ручками, иначе история повторится.
    """
