...
#: Вид ДТП — ЗАКРЫТЫЙ словарь по форме учёта дорожно-транспортных
#: происшествий. Деление нужно для анализа аварийности: наезд на пешехода и
#: опрокидывание предупреждают разными мероприятиями, и свалить их в одну
#: свободную строку значит сделать анализ невозможным.
ACCIDENT_KINDS: dict[str, str] = {
...
#: Вина — ЗАКРЫТЫЙ словарь, и значение сюда вносит АРЕНДАТОР по документам
#: ГИБДД.
#:
#: ГРАНИЦА: виновность устанавливают ГИБДД и суд, а не платформа. Поэтому
#: значение по умолчанию — «не установлена», и оно не превращается в «наш
#: водитель» само по себе. Поля «виноват ли наш водитель» в смысле вердикта
#: системы здесь НЕТ: есть учёт того, что записано в документах.
ACCIDENT_FAULT: dict[str, str] = {
...
#: Последствия — СЧИТАЮТСЯ ПРИ ЧТЕНИИ из чисел пострадавших и погибших, полем
#: не хранятся: сохранённая «тяжесть» разойдётся с числами при первом же
#: уточнении, а какое из двух правда — будет непонятно.
ACCIDENT_CONSEQUENCE_TITLES: dict[str, str] = {
    "damage_only": "Только материальный ущерб",
    "injured": "Есть пострадавшие",
    "fatal": "Есть погибшие",
}

#: Состояние разбора — тоже СЧИТАЕТСЯ, и это главное решение среза: своего
#: жизненного цикла у ДТП НЕТ. Расследование ведётся в ядровом инциденте,
#: мероприятия — в ядровом CAPA, а состояние следует из этих связей. Заведи
#: ДТП собственный статус «разобрано» — и он разойдётся с мероприятиями в
#: первый же день: мероприятие закрыли, а галочку не переставили.
ACCIDENT_FOLLOWUP_TITLES: dict[str, str] = {
    "not_started": "Мероприятия не заведены",
    "open": "Есть незакрытые мероприятия",
    "closed": "Незакрытых мероприятий нет",
}

#: тип источника, которым ДТП адресуется в ядровом CAPA. Ядро принимает любую
#: строку — своей таблицы мероприятий контур БДД не заводит.
ACCIDENT_CAPA_SOURCE = "road_accident"


class RoadAccident(TenantBaseModel, SoftDeleteMixin):
    """ДТП: факт, участники со стороны арендатора и связи с разбором.

    ПОЧЕМУ НЕ ЯДРОВОЙ ``Incident``. ТЗ требует «связь с инцидентами и CAPA» —
    именно СВЯЗЬ, и она здесь есть. А вот сделать ДТП «видом инцидента» нельзя
    по трём причинам, и каждая проверена по коду ядра:

    1. **ДТП — не несчастный случай.** Помятый бампер это ДТП, но не
       происшествие по охране труда. Стань ДТП видом инцидента — каждая
       царапина заводила бы расследование с этапами и участниками.
    2. **Ядровой инцидент ТРЕБУЕТ площадку** (``incident.site_id`` NOT NULL).
       У ДТП на 45-м километре трассы площадки нет. Ослабить это в таблице,
       которой пользуются все дисциплины, ради одной — плохой размен.
    3. **Вид инцидента — перечисление в базе** (``incidenttype``). Добавление
       значения меняет тип, общий для всего продукта.

    СВЯЗЬ ВМЕСТО ПОГЛОЩЕНИЯ: ``incident_id`` НЕОБЯЗАТЕЛЕН. Пострадал человек —
    ДТП привязывается к ядровому инциденту, и расследование идёт там, где оно
    уже умеет идти. Пострадавших нет — инцидента не заводится, и пустая ссылка
    это законное состояние, а не пробел.

    МЕРОПРИЯТИЯ НЕ ДУБЛИРУЮТСЯ: ядровой ``corrective_actions`` адресуется
    парой «тип источника + идентификатор» и принимает любую строку. ДТП
    становится источником мероприятий без единой правки ядра.

    ГРАНИЦА: платформа НЕ устанавливает вину и НЕ оценивает, достаточны ли
    мероприятия. Вину устанавливают ГИБДД и суд; платформа хранит внесённое по
    их документам. Состояние разбора — это факт о незакрытых мероприятиях, а
    не вердикт «разобрано хорошо».
    """
    ...
    #: машина из реестра — ДТП без своего ТС арендатора не касается
    vehicle_id: Mapped[str] = mapped_column(
        ForeignKey("road_vehicle.id"), nullable=False, index=True
    )
    #: водитель НЕОБЯЗАТЕЛЕН: в стоящую машину въезжают и без водителя за
    #: рулём. Требовать его значило бы заставлять вписывать неправду
    ...
    vehicle: Mapped[Vehicle] = relationship(backref="accidents")
