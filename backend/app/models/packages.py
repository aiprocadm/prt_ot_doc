...
class ClientPortalToken(TenantBaseModel):
    ...
    # SEC-68 (срез-186): второй канал доставки кода. Отдельная колонка, а не
    # расширенное значение otp_email: адрес и телефон — разные вещи, и колонка
    # с именем «email», хранящая телефон, врала бы каждому, кто её читает.
    # Заполнена ровно одна из двух: канал выбирается тем, что указали.
    otp_phone: Mapped[str | None] = mapped_column(String(32), nullable=True)
