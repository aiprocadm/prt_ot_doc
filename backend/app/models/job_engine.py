...
class DocumentJobStep(TenantBaseModel):
    ...
    status: Mapped[JobStepStatus] = mapped_column(
        String(16), nullable=False, default=JobStepStatus.QUEUED.value
    )
    started_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    ended_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    attempts: Mapped[int] = mapped_column(Integer, nullable=False, default=0)
...
class PipelineStepLock(TenantBaseModel):
    __tablename__ = "pipeline_step_locks"

    running_count: Mapped[int] = mapped_column(Integer, nullable=False, default=0)

    __table_args__ = (
        UniqueConstraint("tenant_id", name="uq_pipeline_step_locks_tenant"),
        Index("ix_pipeline_step_locks_tenant_updated", "tenant_id", "updated_at"),
    )
