...
class Document(TenantBaseModel):
    ...
    # Наследие: ссылка на задание генерации из контура DocumentGenerationJob,
    # который удалён срезом-139 (таблица без единой записи). Столбец оставлен —
    # он есть в открытом контракте DocumentRead.job_id и в базе; в продукте
    # всегда NULL. Снять можно только вместе с /api/v2 и миграцией.
