...
## Пожарная безопасность — по объекту и по людям площадки (срез-82/83, разд. 54.1)

У площадки есть свои сроки ПБ: перезарядка и поверка средств защиты,
плановые тренировки, пересмотр документов — и противопожарные инструктажи
её людей (срез-83; люди — те же, что у медосмотров: через рабочее место).
Считаются они той же формулой, что сводка готовности модуля ПБ
(``app/services/discipline_fire_safety.py``), только по одной площадке и её
людям; красятся общим правилом (``core/discipline_status.fire_safety_status``):
просрочка — красный, срок в горизонте — жёлтый, порядок — «не измеряется»
с фактом, зелёного не бывает. Средство без площадки к площадке не относится —
как человек без рабочего места: его инструктажи видны в сводке модуля, а на
карточке — нет.
...
def build_site_overview(
    ...
    *,
    ...
    ``fire_safety`` — сроки ПБ самой площадки и инструктажи её людей
    (срез-82/83); без них строка ПБ остаётся с причиной словаря, как у
    светофора клиента.
    """
