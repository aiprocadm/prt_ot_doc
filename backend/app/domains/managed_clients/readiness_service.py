...
Отделено от чистых правил (``readiness.py``): здесь SQL и склейка пар
«сотрудник × норма», решения о цвете живут в правилах.
...
Данные клиента ограничены: выборки помещаются в память, и честная склейка пар
...
- **Экзамен без вида закрывает любую норму.** ``MedicalExam.exam_kind``
  ...
  объявило бы разрыв каждому клиенту со старой картотекой — ложно-красный
  светофор, которому перестают верить.
- **Норма СИЗ закрыта, когда активных выдач НЕ МЕНЬШЕ нормы.** Норма задаёт
  количество (две пары перчаток); одна активная выдача из двух — разрыв.
  Имена сравниваются без учёта регистра (выдачи заводились и руками).
...
from app.domains.managed_clients.readiness import DirectionCounts
...
class ClientReadinessNumbers:
    ...
    def __init__(
        self, medical: DirectionCounts, ppe: DirectionCounts, training_overdue: int
    ) -> None:
...
async def collect_client_numbers(
    ...
    *,
    ...
    if not people:
        return ClientReadinessNumbers(DirectionCounts(), DirectionCounts(), 0)
