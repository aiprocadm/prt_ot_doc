...
async def create_work_permit(
    ...
    *,
    ...
    wp = WorkPermit(
        ...
        measures_before_text=measures_before_text,
        measures_during_text=measures_during_text,
...
async def issue(
    session, *, tenant_id, work_permit_id, actor_user_id, photo_file_id=None, note=None
):
...
async def close(
    ...
    if wp is None:
        return None
    lc.validate_transition(
        str(wp.status), lc.STATUS_CLOSED
    )  # FSM до гейта: draft/closed/cancelled → WorkPermitTransitionError
...
async def record_completion(
    ...
    *,
    ...
    if await _has_signed_closing_signature(
...
async def create_briefing(
    ...
    *,
    ...
    br = WorkPermitBriefing(
        tenant_id=tenant_id,
        work_permit_id=work_permit_id,
        conducted_by_person_id=conducted_by_person_id,
...
async def get_briefing(
    ...
    *,
    ...
    stmt = select(WorkPermitBriefing).where(
        WorkPermitBriefing.id == briefing_id,
        WorkPermitBriefing.tenant_id == tenant_id,
    )
    return (await session.execute(stmt)).scalar_one_or_none()
...
async def create_admission(
    ...
    *,
    ...
    adm = WorkPermitDailyAdmission(
        ...
        start_at=start_at,
        end_at=end_at,
        admitted_by_person_id=admitted_by_person_id,
        note=note,
    )
    session.add(adm)
    await session.flush()
...
async def list_admissions(
    ...
    *,
    ...
    stmt = (
        ...
        .order_by(
            WorkPermitDailyAdmission.admission_date.asc(), WorkPermitDailyAdmission.created_at.asc()
        )
    )
    return list((await session.execute(stmt)).scalars().all())
