...
async def _norm_interval(
    ...
    stmt = (
        select(MedicalNorm.interval_days)
        .where(
...
async def record_exam(
    ...
    *,
    ...
    exam = MedicalExam(
        ...
        person_id=person_id,
        exam_kind=exam_kind,
        exam_type=exam_type or exam_kind.value,
        exam_date=exam_date,
        fitness=fitness,
    ...
    await outbox.enqueue(
        tenant_id=tenant_id,
        event_type=EventType.MEDICAL_EXAM_RECORDED.value,
    ...
    *,
    ...
    if action is lc.SuspensionAction.OPEN:
        susp = MedicalSuspension(
            tenant_id=tenant_id,
            person_id=person_id,
            reason=lc.reason_for(exam.contraindications),
        ...
        await outbox.enqueue(
            tenant_id=tenant_id,
            event_type=EventType.PERSON_SUSPENDED.value,
...
async def _load_factor_catalog(session: AsyncSession, *, tenant_id: str) -> list[lc.FactorTuple]:
    """29н factor catalog as ORM-free FactorTuples for the pure engine."""
    stmt = select(
        MedicalFactor.code,
        MedicalFactor.name,
        MedicalFactor.exam_kinds,
        MedicalFactor.periodicity_months,
    ...
    *,
    ...
    for person in people:
        ...
        required = lc.resolve_required_kinds(
            person.position_id,
            person.working_conditions_class,
            hazard_ids,
            norms,
        )
        required |= set(
        ...
        rows.append(
            {
                ...
                "factors": [
                    {"code": f[0], "name": f[1]} for f in sorted(factors, key=lambda f: f[0])
                ],
                "headcount": hc,
...
async def build_named_list(
    ...
    ld_stmt = (
        select(MedicalExam.person_id, func.max(MedicalExam.exam_date))
        .where(
            MedicalExam.tenant_id == tenant_id,
            MedicalExam.deleted_at.is_(None),
            MedicalExam.person_id.in_(person_ids),
    ...
    last_exam: dict[str, date] = {pid: d for pid, d in (await session.execute(ld_stmt)).all()}
...
async def generate_due_referrals(
    ...
    *,
    ...
    for it in items:
        ...
        open_kinds = await _open_referral_kinds(
            session, tenant_id=tenant_id, person_id=it["person_id"]
        )
        if kind in open_kinds:
            continue
        await issue_referral(
            session,
            tenant_id=tenant_id,
