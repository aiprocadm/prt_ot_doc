...
DISCIPLINES_WITHOUT_RULES: dict[Discipline, str] = {
    # СВЕРКА 2026-08-26: прежняя причина утверждала, что «объектов НВОС,
    # отходов и отчётности по ПЭК в продукте пока нет». Это перестало быть
    # правдой ещё на срезе-1 экологии: сущности есть, а причина осталась
    # прежней. Правильный довод — про СОБЫТИЯ: правило подписывается на
    # событие, и пока контур их не испускает, подписываться не на что.
    Discipline.ECOLOGY: (
        "предметные сущности экологии заведены (объекты НВОС, отходы, "
        "источники выбросов, нормативы, план-график ПЭК и замеры), но событий "
        "контур пока не испускает: правилу не на что подписаться, а правило "
        "на свободный текст названия проверки было бы угадайкой"
    ),
    # СВЕРКА 2026-08-28: обе причины ПЕРЕЧИСЛЯЛИ отсутствующие сущности («ни
    # формирований, ни учений, ни планов»), и это протухло — формирования,
    # учения и планы заведены срезами 56.1, реестр ТС появился в 56.2. Сторож,
    # поставленный срезом экологии, требовал лишь упоминания СОБЫТИЙ и
    # перечисление пропускал. Теперь причина говорит ТОЛЬКО о событиях: это
    # свойство кода, а не перечень таблиц, и оно не устаревает от появления
    # нового реестра.
    Discipline.CIVIL_DEFENSE: (
        "контур ГО и ЧС не испускает событий: реестры дисциплины ведутся, но "
        "подписаться правилу не на что"
    ),
    Discipline.ROAD_SAFETY: (
        "контур БДД не испускает событий: подписаться правилу не на что"
    ),
}
