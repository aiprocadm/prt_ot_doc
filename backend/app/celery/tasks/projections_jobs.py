...
def _run_projection_job(
    tenant_id: str, runner: Callable[[ProjectionOrchestrator], Coroutine[Any, Any, Any]]
) -> dict[str, Any]:
...
def rebuild_dashboard_snapshots_job(*, tenant_id: str, snapshot_date: str | None = None, **kwargs):
    date_value = (
        datetime.fromisoformat(snapshot_date).date() if snapshot_date else datetime.now(UTC).date()
    )
    return _run_projection_job(
        tenant_id, lambda orchestrator: orchestrator.rebuild_dashboard_snapshot(date_value)
    )
...
def rebuild_package_projection_job(*, tenant_id: str, **kwargs):
    return _run_projection_job(
        tenant_id, lambda orchestrator: orchestrator.rebuild_package_projection()
    )
...
def rebuild_person_compliance_projection_job(*, tenant_id: str, **kwargs):
    return _run_projection_job(
        tenant_id, lambda orchestrator: orchestrator.rebuild_person_compliance_projection()
    )
...
def rebuild_site_safety_projection_job(*, tenant_id: str, **kwargs):
    return _run_projection_job(
        tenant_id, lambda orchestrator: orchestrator.rebuild_site_safety_projection()
    )
...
def rebuild_contractor_readiness_projection_job(*, tenant_id: str, **kwargs):
    return _run_projection_job(
        tenant_id, lambda orchestrator: orchestrator.rebuild_contractor_readiness_projection()
    )
...
def rebuild_client_portal_projection_job(*, tenant_id: str, **kwargs):
    return _run_projection_job(
        tenant_id, lambda orchestrator: orchestrator.rebuild_client_portal_projection()
    )
