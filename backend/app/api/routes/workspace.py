...
#: Кому сводка роли показывает происшествия, проверки и просроченное обучение
#: по арендатору (срез-151).
#:
#: До среза здесь стояли `safety_lead` и `safety_manager` — кодов, которых нет
#: в ``RoleEnum``, а значит, нет ни у одного пользователя. Разрез «для
#: специалистов по охране труда» не срабатывал НИ РАЗУ ни для одной реальной
#: роли охраны труда: специалист ОТ, руководитель ОТиПБ, начальник отдела ОТ и
#: инженер ПБ получали в своей сводке нули по происшествиям и проверкам, а
#: работал он только для admin/owner. Права ручки при этом не менялись — она
#: открыта всем ролям арендатора (`rbac()` без списка), и те же числа эти роли
#: видят на дашборде.
_SAFETY_ROLES = {
    "admin",
    "owner",
...
#: Здесь по той же причине был мёртвый `hr_manager`; живая роль — `hr`.
_HR_ROLES = {"hr", "admin", "owner"}
