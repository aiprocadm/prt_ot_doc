...
from app.schemas.work_permit import (
    ...
    WorkPermitBriefingCreate,
    WorkPermitBriefingRead,
    WorkPermitBriefingUpdate,
    WorkPermitClosingRecordCreate,
    WorkPermitClosingSummary,
    WorkPermitCreate,
    WorkPermitDailyAdmissionCreate,
    WorkPermitDailyAdmissionRead,
    WorkPermitDailyAdmissionUpdate,
    WorkPermitEventRead,
    WorkPermitExtendRequest,
    WorkPermitMemberCreate,
    WorkPermitMemberRead,
    WorkPermitPage,
    WorkPermitRead,
    WorkPermitSignatureCreate,
...
_READ_ROLES = list(screen_roles("work_permit.view"))
_WRITE_ROLES = list(screen_roles("work_permit.manage"))
ReaderAccess = Annotated[
    AccessContext, Depends(abac(_tenant_resource_id, required_roles=_READ_ROLES))
]
WriterAccess = Annotated[
    AccessContext, Depends(abac(_tenant_resource_id, required_roles=_WRITE_ROLES))
]
...
def _briefing_read(br) -> WorkPermitBriefingRead:
    return WorkPermitBriefingRead(
        id=str(br.id),
        work_permit_id=str(br.work_permit_id),
        conducted_by_person_id=(
            str(br.conducted_by_person_id) if br.conducted_by_person_id else None
        ),
        conducted_at=br.conducted_at,
...
def _pep_to_http(exc: Exception) -> HTTPException:
    if isinstance(exc, PepNotFound):
        return HTTPException(
            ...
            detail=api_problem_detail(
                code="PEP_NOT_FOUND", message=str(exc), error_type="work_permit"
            ),
        )
    return HTTPException(
        status_code=status.HTTP_409_CONFLICT,
...
def _signature_read(
    row: SignatureRequest, stream: str, *, confirm_code: str | None = None
) -> WorkPermitSignatureRead:
...
async def _closing_summary(
    session: AsyncSession, tenant: Tenant, wp: WorkPermit
) -> WorkPermitClosingSummary:
    rows = (
        (
    ...
    signed = await signed_closing_kinds(
        session, tenant_id=str(tenant.id), work_permit_id=str(wp.id)
    )
...
async def _permit_read(session: AsyncSession, tenant: Tenant, wp: WorkPermit) -> WorkPermitRead:
    ...
    return WorkPermitRead(
        ...
        safety_systems=wp.safety_systems,
        measures_before_text=wp.measures_before_text,
        measures_during_text=wp.measures_during_text,
        special_conditions_text=wp.special_conditions_text,
...
async def list_work_permits(
    ...
    count_stmt = (
        select(func.count()).select_from(WorkPermit).where(WorkPermit.tenant_id == tenant.id)
    )
...
async def create_work_permit_endpoint(
    ...
    wp = await create_work_permit(
        ...
        equipment_text=payload.equipment_text,
        hazards_text=payload.hazards_text,
        measures_text=payload.measures_text,
        planned_start=payload.planned_start,
        planned_end=payload.planned_end,
        subdivision_text=payload.subdivision_text,
        content_text=payload.content_text,
        conditions_text=payload.conditions_text,
        safety_systems=payload.safety_systems,
        measures_before_text=payload.measures_before_text,
        measures_during_text=payload.measures_during_text,
        special_conditions_text=payload.special_conditions_text,
        ppe_text=payload.ppe_text,
...
async def update_work_permit_endpoint(
    ...
    if (
        "work_type" in fields
        and "type_specific" not in fields
...
async def delete_work_permit_endpoint(
    wp_id: str, tenant: TenantDep, session: SessionDep, access: WriterAccess
):
    TenantContextValidator.ensure_tenant_context(tenant)
...
@router.post(
    "/{wp_id}/members", response_model=WorkPermitMemberRead, status_code=status.HTTP_201_CREATED
)
@audit_operation("update", "work_permit")
async def add_member_endpoint(
    wp_id: str,
    payload: WorkPermitMemberCreate,
    tenant: TenantDep,
    session: SessionDep,
    access: WriterAccess,
) -> WorkPermitMemberRead:
...
@router.delete(
    "/{wp_id}/members/{member_id}", status_code=status.HTTP_204_NO_CONTENT, response_model=None
)
@audit_operation("update", "work_permit")
...
async def readiness(
    ...
    return ReadinessReportRead(
        ok=report.ok,
        violations=[
            ViolationRead(person_id=v.person_id, role=v.role, code=v.code, severity=v.severity)
...
async def issue_endpoint(
    wp_id: str,
    payload: WorkPermitActionRequest,
    tenant: TenantDep,
    session: SessionDep,
    access: WriterAccess,
) -> WorkPermitRead:
...
async def extend_endpoint(
    wp_id: str,
    payload: WorkPermitExtendRequest,
    tenant: TenantDep,
    session: SessionDep,
    access: WriterAccess,
) -> WorkPermitRead:
...
@router.post(
    "/{wp_id}/briefing", response_model=WorkPermitBriefingRead, status_code=status.HTTP_201_CREATED
)
@audit_operation("update", "work_permit")
async def create_briefing_endpoint(
    wp_id: str,
    payload: WorkPermitBriefingCreate,
    tenant: TenantDep,
    session: SessionDep,
    access: WriterAccess,
) -> WorkPermitBriefingRead:
...
async def update_briefing_endpoint(
    wp_id: str,
    briefing_id: str,
    payload: WorkPermitBriefingUpdate,
    tenant: TenantDep,
    session: SessionDep,
    access: WriterAccess,
) -> WorkPermitBriefingRead:
...
def _admission_read(a) -> WorkPermitDailyAdmissionRead:
    return WorkPermitDailyAdmissionRead(
        id=str(a.id),
        work_permit_id=str(a.work_permit_id),
...
@router.post(
    "/{wp_id}/admissions",
    response_model=WorkPermitDailyAdmissionRead,
    status_code=status.HTTP_201_CREATED,
)
@audit_operation("update", "work_permit")
async def create_admission_endpoint(
    wp_id: str,
    payload: WorkPermitDailyAdmissionCreate,
    tenant: TenantDep,
    session: SessionDep,
    access: WriterAccess,
) -> WorkPermitDailyAdmissionRead:
    ...
    try:
        adm = await create_admission(
            ...
            admitted_by_person_id=payload.admitted_by_person_id,
            note=payload.note,
            actor_user_id=access.user.id if access else None,
        )
    except lc.WorkPermitTransitionError as exc:
...
async def update_admission_endpoint(
    wp_id: str,
    admission_id: str,
    payload: WorkPermitDailyAdmissionUpdate,
    tenant: TenantDep,
    session: SessionDep,
    access: WriterAccess,
) -> WorkPermitDailyAdmissionRead:
...
@router.post(
    "/{wp_id}/signatures",
    response_model=WorkPermitSignatureRead,
    status_code=status.HTTP_201_CREATED,
)
@audit_operation("update", "work_permit")
async def create_permit_signature_endpoint(
    wp_id: str,
    payload: WorkPermitSignatureCreate,
    tenant: TenantDep,
    session: SessionDep,
    access: WriterAccess,
) -> WorkPermitSignatureRead:
...
async def list_signatures_endpoint(
    ...
    return [_signature_read(r, "permit") for r in permit_rows] + [
...
async def record_closing_endpoint(
    wp_id: str,
    payload: WorkPermitClosingRecordCreate,
    tenant: TenantDep,
    session: SessionDep,
    access: WriterAccess,
) -> WorkPermitClosingSummary:
