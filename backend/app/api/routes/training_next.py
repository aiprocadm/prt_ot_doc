    ...
    rows = (
        (
            await session.execute(
                ...
                .where(TrainingProgram.tenant_id == tenant.id, TrainingProgram.deleted_at.is_(None))
...
async def list_tests(
    ...
    rows = (
        (
            await session.execute(
                select(TrainingTest).where(
                    TrainingTest.tenant_id == tenant.id, TrainingTest.deleted_at.is_(None)
                )
            )
        )
        .scalars()
        ...
        (
            await session.execute(
                select(TrainingGroup).where(
                    TrainingGroup.tenant_id == tenant.id, TrainingGroup.deleted_at.is_(None)
                )
            )
        )
        .scalars()
...
async def list_enrollments(
    ...
    stmt = select(TrainingEnrollment).where(
        TrainingEnrollment.tenant_id == tenant.id, TrainingEnrollment.deleted_at.is_(None)
    )
    if person_id:
...
async def create_enrollment(
    ...
    e = TrainingEnrollment(
        tenant_id=tenant.id, assigned_at=datetime.now(tz=timezone.utc), **_safe(payload)
    )
    session.add(e)
    await session.flush()
    return e
...
async def list_protocols(
    ...
    rows = (
        (
            await session.execute(
                select(TrainingProtocol).where(
                    TrainingProtocol.tenant_id == tenant.id, TrainingProtocol.deleted_at.is_(None)
                )
            )
        )
        .scalars()
        ...
        (
            await session.execute(
                select(TrainingCertificate).where(
                    TrainingCertificate.tenant_id == tenant.id,
...
async def create_certificate(
    ...
    c = TrainingCertificate(
        tenant_id=tenant.id, issued_at=date.today(), status="active", **_safe(payload)
    )
    session.add(c)
    await session.flush()
    return c
...
async def send_certificate_registry(
    item_id: str,
    tenant: Tenant = Depends(get_tenant_record),
    session: AsyncSession = Depends(get_session),
    __: Any = _PermWriteDep,
...
async def send_protocol_registry(
    item_id: str,
    tenant: Tenant = Depends(get_tenant_record),
    session: AsyncSession = Depends(get_session),
    __: Any = _PermWriteDep,
...
async def teacher_dashboard(
    ...
    group_stmt = select(TrainingGroup).where(
        TrainingGroup.tenant_id == tenant.id, TrainingGroup.deleted_at.is_(None)
    )
    if teacher_user_id:
        ...
        (
            await session.execute(
                ...
                .where(
                    ...
                    TrainingEnrollment.training_group_id.in_(group_ids or ["__none__"]),
        ...
        (
            await session.execute(
                ...
                .where(
                    TrainingEnrollment.tenant_id == tenant.id,
                    TrainingEnrollment.training_group_id.in_(group_ids or ["__none__"]),
                    TrainingEnrollment.completion_status.in_(["completed", "confirmed"]),
                    TrainingEnrollment.deleted_at.is_(None),
                )
            )
        ).scalar_one()
    ...
    avg_progress = float(
        (
            await session.execute(
                select(func.avg(TrainingEnrollment.progress_percent)).where(
                    TrainingEnrollment.tenant_id == tenant.id,
                    TrainingEnrollment.training_group_id.in_(group_ids or ["__none__"]),
                    TrainingEnrollment.deleted_at.is_(None),
                )
...
async def learner_dashboard(
    ...
    if person_id != "me":
        stmt = stmt.where(TrainingEnrollment.person_id == person_id)
    enrollments = (
        (await session.execute(stmt.order_by(TrainingEnrollment.updated_at.desc()))).scalars().all()
    )
    completed = sum(
        1 for item in enrollments if item.completion_status in {"completed", "confirmed"}
    )
    overdue = sum(
        1
        for item in enrollments
        if item.due_at
        and item.due_at < datetime.now(tz=timezone.utc)
        and item.completion_status not in {"completed", "confirmed"}
    )
    return {
        "person_id": person_id,
...
async def confirm_enrollment_completion(
    item_id: str,
    payload: dict,
    tenant: Tenant = Depends(get_tenant_record),
    session: AsyncSession = Depends(get_session),
...
async def assign_enrollment_retake(
    item_id: str,
    payload: dict,
    tenant: Tenant = Depends(get_tenant_record),
    session: AsyncSession = Depends(get_session),
...
async def ingest_runtime_result(
    item_id: str,
    payload: dict,
    tenant: Tenant = Depends(get_tenant_record),
    session: AsyncSession = Depends(get_session),
...
async def training_analytics_overview(
    tenant: Tenant = Depends(get_tenant_record),
    session: AsyncSession = Depends(get_session),
    __: Any = _PermReadDep,
):
