...
- **Adjacent drifts surfaced while building the live-PG write smoke (separate from enum parity, not fixed here):** `tenant.code` / `tenant.schema_name` / `tenant.s3_prefix` are declared nullable on the ORM `Tenant` model but are NOT NULL on the migrated PG schema; and configuring all mappers in an isolated session raises `InvalidRequestError: Multiple classes found for path "Inspection"` (an ambiguous string-keyed `relationship`). Both warrant their own ORM↔migration-drift cleanup.
