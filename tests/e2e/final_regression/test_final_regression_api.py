...
@pytest.mark.anyio
async def test_pipeline_idempotency_same_key_same_result(app_fixture, make_auth_headers, sessionmaker, data_factory) -> None:
...
@pytest.mark.anyio
async def test_template_selection_by_code_version_not_found_returns_conflict(async_client, make_auth_headers) -> None:
    headers = await make_auth_headers()
