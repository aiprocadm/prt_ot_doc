...
#:
#: ЧЕМ ОНИ ОПАСНЫ НА САМОМ ДЕЛЕ (срез-224). Не тем, что занимают место, а тем,
#: что их имена ПРАВДОПОДОБНЫ и отличаются от живых одной буквой:
#: ``incident_persons`` против ``incident_person``, ``training_plans`` против
#: ``training_plan``. Попав в карту решений, такое имя выглядит верным, проходит
#: проверки «таблица существует» — и молча ничего не делает. Так офбординг
#: удерживал пустой дубликат вместо материалов расследования и удалял то, что
#: ст. 230.1 ТК РФ велит хранить 45 лет. Поэтому любой список ТАБЛИЦ в боевом
#: коде обязан проверяться по МОДЕЛЯМ, а не по схеме базы и не по списку RLS:
#: и схема, и список RLS знают про мёртвые таблицы тоже.
...
def test_новых_таблиц_без_модели_не_появилось(
    ...
    assert not gone, (
        "таблица из списка «без модели» больше не сирота — уберите её из "
        "списка: " + ", ".join(gone)
    )
