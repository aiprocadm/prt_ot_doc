...
async def _build_issued_permit(session, *, tenant_id, foreman_id, supervisor_id):
    ...
    wp = await svc.create_work_permit(
        session, tenant_id=tenant_id, work_type="height", zone_text="z"
    )
    await svc.add_member(
        session, tenant_id=tenant_id, work_permit_id=wp.id, person_id=foreman_id, role="foreman"
    )
    await svc.add_member(
        session,
        tenant_id=tenant_id,
...
async def test_record_completion_upsert(sessionmaker, data_factory):
    ...
    async with sessionmaker() as session:
        wp = await _build_issued_permit(
            session, tenant_id=tid, foreman_id=foreman.id, supervisor_id=supervisor.id
        )
        out = await svc.record_completion(
            session,
            tenant_id=tid,
            work_permit_id=wp.id,
            completion_text="место сдано",
            actor_user_id="u1",
        )
    ...
    async with sessionmaker() as session:
        ...
        with pytest.raises(lc.WorkPermitCompletionLocked):
            await svc.record_completion(
                ...
                tenant_id=tid,
                work_permit_id=wp.id,
                completion_text="изменённый текст",
                actor_user_id="u1",
            )
...
async def test_cancel_not_gated(sessionmaker, data_factory):
    ...
    async with sessionmaker() as session:
        ...
        cancelled = await svc.cancel(
            session, tenant_id=tid, work_permit_id=wp.id, actor_user_id="u1"
        )
...
async def test_sign_closing_rejects_wrong_role(sessionmaker, data_factory):
    ...
    async with sessionmaker() as session:
        ...
        await svc.add_member(
            session, tenant_id=tid, work_permit_id=wp.id, person_id=observer.id, role="observer"
        )
        with pytest.raises(WorkPermitSignerError):
            await sign_closing(
                session,
