...
ТРЕТИЙ СОРТ (срез-143): «сегодня», ЗАМОРОЖЕННОЕ НА СБОРЕ. ``TODAY =
date.today()`` на уровне модуля вычисляется, когда pytest собирает файлы —
за часы до самого теста; продукт же берёт «сейчас» в момент вызова. Полный
прогон на ``main`` 2026-09-11 (23:23–01:46 UTC) переехал через полночь UTC,
и deadline-события дисциплин дали пять ``11 == 10``: сроки посеяны от
вчерашнего «сегодня», просрочка посчитана от сегодняшнего. Лечится не
переносом даты внутрь теста (это лишь сужает окно до секунд), а ОДНОЙ
меткой времени для семян и для продукта: службе, принимающей ``now=``,
передать ту же метку, от которой посеяны сроки. Машиной это не проверить:
15 других модулей замораживают «сегодня» так же, но сверяют не с часами
продукта, а с тем, что сами посеяли, — и не ломаются.
"""
