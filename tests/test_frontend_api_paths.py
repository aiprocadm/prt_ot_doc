...
DEFECTS_IN_QUEUE: dict[str, str] = {
    "POST /packs": (
        "карточка компании (`features/companies/CompanyDetails.tsx`) запускает "
        "комплект этой ручкой — у сервера её нет (есть `/packs/run`, "
        "`/packs/generate`, `/packs/preview` с другими телами). Кнопка "
        "кончается 404; перевод на живой контракт — отдельный срез"
    ),
