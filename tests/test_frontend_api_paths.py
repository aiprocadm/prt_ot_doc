...
#: Срез-157 убрал отсюда `POST /files`: мёртвая функция хранилища удалена.
NOT_A_DEFECT: dict[str, str] = {
    # Срез-233: строка ушла — вместе с самим вызовом. Решение среза-152
    # («ручки нет, витрина шлёт best-effort; появится ручка — строка уйдёт»)
    # принималось без одного факта: у собранных событий НЕТ И ЧИТАТЕЛЯ —
    # они копятся в хранилище браузера, которое никто не читает. То есть
    # механизм не доставлял ничего никуда, а запрос уходил при КАЖДОМ переходе
    # между экранами и всегда получал 404. Отправку убрали, сбор оставили и
    # честно назвали его состояние в `frontend/src/utils/uxMetrics.ts`.
