...
@pytest.mark.anyio
async def test_pipeline_run_happy_path_and_actions(client, tenant_headers):
    profile_payload = {
        "code": "doc_happy_v1",
        "name": "Doc Happy",
        "steps": [
    ...
    details = await client.get(f"/api/v1/pipelines/runs/{run_id}", headers=tenant_headers)
    assert details.status_code == 200
    assert details.json()["run_id"] == run_id
    assert isinstance(details.json()["step_runs"], list)

    canceled = await client.post(f"/api/v1/pipelines/runs/{run_id}:cancel", headers=tenant_headers)
