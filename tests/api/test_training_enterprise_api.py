...
async def test_training_teacher_and_runtime_flow(
    ...
    lesson = await async_client.post(
        f"/api/v1/training/modules/{module.id}/lessons",
        json={"title": "Lesson", "lesson_order": 1},
        headers={**headers, "X-Tenant": "test"},
    )
    assert lesson.status_code == 201

    progress = await async_client.post(
        f"/api/v1/training/enrollments/{enrollment.id}/progress",
        json={
            "completed_lesson_ids": [lesson.json()["id"]],
            "provider": "xapi",
            "event": "progressed",
        },
        headers={**headers, "X-Tenant": "test"},
    ...
    runtime = await async_client.post(
        f"/api/v1/training/enrollments/{enrollment.id}/runtime-results",
        json={"content_format": "scorm", "session_ref": "sess-1", "score": 92, "passed": True},
        headers={**headers, "X-Tenant": "test"},
    )
    assert runtime.status_code == 200
    assert runtime.json()["enrollment"]["completion_status"] == "completed"

    teacher = await async_client.get(
        "/api/v1/training/teacher/dashboard", headers={**headers, "X-Tenant": "test"}
    )
    assert teacher.status_code == 200
    assert teacher.json()["enrollments_total"] == 1

    learner = await async_client.get(
        f"/api/v1/training/learner/dashboard?person_id={person.id}",
        headers={**headers, "X-Tenant": "test"},
    )
    assert learner.status_code == 200
    assert learner.json()["completed_total"] == 1

    program_detail = await async_client.get(
        f"/api/v1/training/programs/{program.id}/detail", headers={**headers, "X-Tenant": "test"}
    )
    assert program_detail.status_code == 200
    assert program_detail.json()["lessons_total"] == 1

    enrollment_detail = await async_client.get(
        f"/api/v1/training/enrollments/{enrollment.id}/detail",
        headers={**headers, "X-Tenant": "test"},
    )
    ...
    analytics = await async_client.get(
        "/api/v1/training/analytics/overview", headers={**headers, "X-Tenant": "test"}
    )
    ...
    retake = await async_client.post(
        f"/api/v1/training/enrollments/{enrollment.id}/retake",
