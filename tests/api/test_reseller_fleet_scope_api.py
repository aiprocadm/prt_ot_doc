...
@pytest.mark.anyio
async def test_тариф_партнёру_ограничен_его_набором(
    async_client: AsyncClient, make_auth_headers
) -> None:
    """BIZ-52 срез-8 снял запрет среза-2 и заменил его ПОТОЛКОМ.

    Раньше тариф был закрыт партнёру целиком (`FLEET_COMMERCIALS_PLATFORM_ONLY`)
    с явной пометкой «пока нет потолка». Потолок появился: отказ теперь
    по СОДЕРЖАНИЮ — партнёр без своего набора не выдаёт модули, которых у него
    нет. Подробности суб-биллинга — в `test_reseller_subbilling_api.py`.
    """
...
async def test_права_проверяются_раньше_области(
    ...
    """На ЗАКРЫТОЙ партнёру ручке отказ обязан быть 403 ещё до поиска арендатора.

    Иначе перебором id партнёр вычислил бы чужой контур по разнице между 403
    и 404. Проверяется на квотах: они остались за владельцем платформы
    (потолок по квотам — продуктовое решение, срез-8 его не выдумывал).

    На тарифе это правило больше не действует и не должно: партнёр теперь
    ПОЛЬЗУЕТСЯ этой ручкой, и она ведёт себя как остальные — чужой арендатор
    отвечает 404, ровно как в `/status`.
    """

    await _shape_tree()

    response = await async_client.patch(
        f"{BASE}/несуществующий-id/quotas",
        headers=await _partner_headers(make_auth_headers),
        json={"max_storage_mb": 1024},
    )
