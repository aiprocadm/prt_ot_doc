...
@pytest.mark.asyncio
async def test_batch_create_stores_and_returns_supplier(
...
@pytest.mark.asyncio
async def test_batch_create_unknown_supplier_404(
...
@pytest.mark.asyncio
async def test_transfer_into_existing_dest_keeps_dest_supplier(
    ...
    async with sessionmaker() as session:
        ...
        dest = PPEStockBatch(
            ...
            quantity=2,
            location="B",
            supplier_id=dst_sup.id,
        )
        session.add_all([source, dest])
        await session.flush()

        result = await transfer_stock(
            session, tenant_id=tenant.id, source_batch_id=source.id, to_location="B", quantity=3
        )
        merged = (
            await session.execute(
                select(PPEStockBatch).where(PPEStockBatch.id == result.dest_batch_id)
        ...
        assert (
            merged.supplier_id == dst_sup.id
        )  # existing provenance preserved, NOT overwritten by source


@pytest.mark.asyncio
async def test_batch_update_clears_supplier(
    async_client, make_auth_headers, sessionmaker, data_factory: TestDataFactory
):
    async with sessionmaker() as session:
        await data_factory.ensure_tenant(session=session)
...
@pytest.mark.asyncio
async def test_item_preferred_supplier_roundtrip(
    async_client, make_auth_headers, sessionmaker, data_factory: TestDataFactory
):
    async with sessionmaker() as session:
        await data_factory.ensure_tenant(session=session)
...
@pytest.mark.asyncio
async def test_item_create_unknown_preferred_supplier_404(
    async_client, make_auth_headers, sessionmaker, data_factory: TestDataFactory
):
    async with sessionmaker() as session:
        await data_factory.ensure_tenant(session=session)
