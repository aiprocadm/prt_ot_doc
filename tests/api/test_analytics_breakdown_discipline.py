...
async def test_просрочка_не_считается_это_none_а_не_ноль(
    ...
    # (ЭПБ, учения ГО, документы ТС); срез-58: пожарная безопасность считается
    # по виду инструктажа (противопожарные и ПТМ). Просрочка считается у ВСЕХ
    # восьми — ветка «None» ниже жива только сторожем с monkeypatch.
    assert ATTENTION_DISCIPLINES == frozenset(Discipline)
