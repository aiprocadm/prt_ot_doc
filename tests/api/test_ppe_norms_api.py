...
@pytest.mark.asyncio
async def test_norm_crud_roundtrip(
...
@pytest.mark.asyncio
async def test_norm_foreign_refs_must_exist_in_tenant(
...
@pytest.mark.asyncio
async def test_norms_are_tenant_isolated(
    async_client, make_auth_headers, sessionmaker, data_factory
):
    ...
    f_position_id, f_hazard_id, f_item_id = await _seed_refs(
        sessionmaker, data_factory, slug="other"
    )
    ...
    async with sessionmaker() as session:
        ...
        norm = PPENorm(
            ...
            hazard_id=f_hazard_id,
            item_id=f_item_id,
            item_name="Чужая норма",
            quantity=1,
            interval_days=365,
        )
        session.add(norm)
    ...
    assert (
        await async_client.get(f"{BASE}/{foreign_norm_id}", headers=headers)
    ).status_code == status.HTTP_404_NOT_FOUND
    assert (
        await async_client.patch(f"{BASE}/{foreign_norm_id}", headers=headers, json={"quantity": 5})
    ).status_code == status.HTTP_404_NOT_FOUND
    assert (
        await async_client.delete(f"{BASE}/{foreign_norm_id}", headers=headers)
    ).status_code == status.HTTP_404_NOT_FOUND
...
@pytest.mark.asyncio
async def test_same_item_norm_duplicate_caught_by_item_id(
    async_client, make_auth_headers, sessionmaker, data_factory
):
