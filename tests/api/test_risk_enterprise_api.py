...
@pytest.mark.anyio
async def test_advanced_risk_methodology_and_map(
        ...
        json={
            ...
            "formula_json": {
                "ranges": [
                    ...
                    {"min": 201, "max": 10000, "level": "critical"},
                ]
            },
        },
    ...
    recalc = await async_client.post(
        f"/api/v1/risk/advanced/maps/{risk_map.json()['id']}/recalculate",
        headers={**headers, "X-Tenant": "test"},
    )
    assert recalc.status_code == 200

    summary = await async_client.get(
        f"/api/v1/risk/advanced/maps/{risk_map.json()['id']}/summary",
        headers={**headers, "X-Tenant": "test"},
    )
    ...
    custom = await async_client.post(
        ...
        json={
            ...
            "formula_json": {
                ...
                "coefficients": {"probability": 2, "severity": 3, "exposure": 1},
            },
            "scale_json": {
                "level_rules": [
                    {"min": 0, "max": 10, "level": "low"},
                    {"min": 10.01, "max": 20, "level": "medium"},
                    {"min": 20.01, "max": 1000, "level": "high"},
                ]
            },
        },
        headers={**headers, "X-Tenant": "test"},
