...
async def test_extend_revoked_permit_409(async_client, make_auth_headers, data_factory):
    ...
    permit_id = r.json()["id"]
    assert (
        await async_client.post(f"{BASE}/{permit_id}/revoke", headers=headers)
...
@pytest.mark.asyncio
async def test_permits_are_tenant_isolated(
    ...
    assert (
        await async_client.get(f"{BASE}/{foreign_permit_id}", headers=headers)
    ).status_code == status.HTTP_404_NOT_FOUND
    assert (
        await async_client.patch(
            f"{BASE}/{foreign_permit_id}", headers=headers, json={"permit_type": "z"}
        )
    ).status_code == status.HTTP_404_NOT_FOUND
    assert (
        await async_client.post(f"{BASE}/{foreign_permit_id}/revoke", headers=headers)
    ).status_code == status.HTTP_404_NOT_FOUND
