...
@pytest.mark.anyio
async def test_machine_keys_and_public_api(
    ...
    async with sessionmaker() as session:  # type: AsyncSession
        ...
        await data_factory.create_person(
            session=session, tenant=tenant, company=company, first_name="Ivan", last_name="Petrov"
        )
    ...
    created = await async_client.post(
        "/api/v1/machine-keys",
        json={"name": "bot", "scopes": ["employees:read", "documents:read", "integrations:write"]},
    ...
    auth = await async_client.get(
        "/api/v1/public/auth/machine", headers={"X-Tenant": "test", "X-API-Key": token}
    )
    ...
    docs = await async_client.get(
        "/api/v1/public/documents", headers={"X-Tenant": "test", "X-API-Key": token}
    )
    ...
    webhook = await async_client.post(
        ...
        json={
            "name": "BI callback",
            "url": "https://example.test/public-hook",
            "subscribed_events": ["ExportDelivered"],
        },
        headers={"X-Tenant": "test", "X-API-Key": token},
    )
    ...
    rotated = await async_client.post(
        f"/api/v1/machine-keys/{created.json()['id']}/rotate",
        headers={**headers, "X-Tenant": "test"},
    )
    ...
    new_auth = await async_client.get(
        "/api/v1/public/auth/machine", headers={"X-Tenant": "test", "X-API-Key": new_token}
    )
