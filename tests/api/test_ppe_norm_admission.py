...
async def _world(session, data_factory, *, tag: str):
    ...
    position = Position(
        tenant_id=tenant.id, company_id=company.id, name=f"PPE-Gate-{tag}"
    )
    hazard = RiskHazard(
        tenant_id=tenant.id, code=f"ppegate-{tag}", title=f"Фактор {tag}"
    )
...
def _norm(tenant, position, hazard, item, *, quantity: int = 1) -> PPENorm:
    return PPENorm(
        ...
        item_id=item.id, item_name=item.name, quantity=quantity, interval_days=365,
    )


def _issue(tenant, person, *, item_id=None, item_name: str, quantity: int = 1,
           expires_in_days: int | None = 300) -> PPEIssue:
    expires = NOW + timedelta(days=expires_in_days) if expires_in_days is not None else None
    return PPEIssue(
        tenant_id=tenant.id, person_id=person.id, item_id=item_id,
        item_name=item_name, quantity=quantity,
        issued_at=NOW - timedelta(days=1), expires_at=expires, status="issued",
    )
...
@pytest.mark.asyncio
async def test_norm_overdue_issue_blocks_even_with_other_active_issue(
    ...
    async with sessionmaker() as session:
        ...
        session.add(_issue(tenant, person, item_id=helmet.id,
                           item_name=helmet.name, expires_in_days=-5))
...
async def test_all_norm_lines_covered_passes(sessionmaker, data_factory):
    ...
    async with sessionmaker() as session:
        ...
        session.add(_issue(tenant, person, item_id=helmet.id,
                           item_name=helmet.name, expires_in_days=300))  # ok
        session.add(_issue(tenant, person, item_id=belt.id,
                           item_name=belt.name, expires_in_days=10))  # due_soon
        await session.commit()
...
async def test_partial_quantity_blocks(sessionmaker, data_factory):
    ...
    async with sessionmaker() as session:
        ...
        session.add(_issue(tenant, person, item_id=gloves.id,
                           item_name=gloves.name, quantity=1))
        await session.commit()
...
@pytest.mark.asyncio
async def test_legacy_item_name_only_norm_matches_catalog_issue(
    sessionmaker, data_factory
):
    ...
    async with sessionmaker() as session:
        ...
        session.add(item)
        await session.flush()
        session.add(PPENorm(  # legacy: no catalog link
            tenant_id=tenant.id, position_id=position.id, hazard_id=hazard.id,
            item_id=None, item_name=item.name, quantity=1, interval_days=180,
        ))
