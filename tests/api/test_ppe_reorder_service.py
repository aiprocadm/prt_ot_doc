...
async def _item(session, tenant_id, *, name, min_stock, preferred=None):
    it = PPEItem(
        tenant_id=tenant_id, name=name, min_stock=min_stock, preferred_supplier_id=preferred
    )
    session.add(it)
    await session.flush()
    return it
...
async def test_explicit_supplier_wins(sessionmaker, data_factory: TestDataFactory):
    async with sessionmaker() as session:
        ...
        await _batch(
            session, tenant.id, item.id, qty=1, supplier_id=hist.id, received=date(2026, 6, 1)
        )
...
@pytest.mark.asyncio
async def test_explicit_soft_deleted_falls_back_to_history(
    ...
    async with sessionmaker() as session:
        ...
        item = await _item(session, tenant.id, name="Каска", min_stock=10, preferred=dead.id)
        await _batch(
            session, tenant.id, item.id, qty=1, supplier_id=live.id, received=date(2026, 6, 1)
        )
        rows = await compute_shortages(session, tenant.id, now=NOW)
        row = next(r for r in rows if r.item_id == item.id)
...
def _row(item_id, deficit, below, sid, sname):
    return ShortageRow(
        ...
        supplier_source="explicit" if sid else None,
    )
