...
* ядровые ``Asset``/``Equipment`` (``backend/app/models/assets.py``, весь файл
  48 строк) — это ДВЕ и ТРИ колонки (имя, категория; серийный номер, статус),
  **без единой ручки API** и без единого поля срока. Учитывать по ним ЭПБ
  нечем, и никто ими не пользуется;
