...
def test_else_inside_if_block_is_valid() -> None:
    report = lint_template(
        _docx_with_text(
            "{% if person.active %}{{ person.name }}{% else %}Inactive{% endif %}"
        )
    )
...
def test_elif_with_unsafe_expression_is_error() -> None:
    report = lint_template(
        _docx_with_text("{% if x %}{% elif y ; danger %}bad{% endif %}")
    )
...
def test_unbalanced_double_brace_is_error() -> None:
    ...
    assert any(
        "Unbalanced placeholder delimiters" in err for err in report["errors"]
    )
...
def test_balanced_delims_no_error() -> None:
    ...
    delim_errs = [
        e for e in report["errors"] if "Unbalanced" in e or "delimiters" in e
    ]
    assert delim_errs == []
...
def test_available_fields_not_set_skips_warning() -> None:
    ...
    assert all(
        "not declared in available_fields" not in w for w in report["warnings"]
    )
...
def test_available_fields_flags_unknown_in_condition() -> None:
    report = lint_template(
        _docx_with_text(
            "{% if mystery.flag %}A{% endif %}{{ person.name }}"
        ),
        available_fields=["person"],
    )
    msgs = [w for w in report["warnings"] if "not declared in available_fields" in w]
...
def test_available_fields_flags_unknown_in_loop_iter() -> None:
    report = lint_template(
        _docx_with_text(
            "{% for x in unknown_list %}{{ x }}{% endfor %}"
        ),
        available_fields=["person"],
    )
    msgs = [w for w in report["warnings"] if "not declared in available_fields" in w]
...
def test_parse_returns_delimiter_counts() -> None:
    parsed = parse_docx_placeholders(
        _docx_with_text("{{ a }} {{ b }} {% if c %}x{% endif %}")
    )
...
def test_legacy_syntactic_errors_still_caught(
