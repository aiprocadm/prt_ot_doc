...
async def test_contingent_includes_factor_driven_person_without_norm(sessionmaker, data_factory):
    ...
    assert any(
        it["person_id"] == person.id and it["exam_kind"] == "periodic" and it["status"] == "missing"
        for it in items
    )
...
async def test_unknown_exam_kind_label_is_skipped_not_crashing(sessionmaker, data_factory):
    ...
    async with sessionmaker() as session:
        ...
        company = await data_factory.create_company(tenant=tenant, session=session)
        hazard = RiskHazard(
            tenant_id=tenant.id, code="noise2", title="Шум2", medical_factor_code="4.5"
        )
        session.add(hazard)
        await session.flush()
        ...
        session.add(
            MedicalFactor(
                ...
                code="4.5",
                name="Шум2",
                exam_kinds=["bogus_kind", "periodic"],
                periodicity_months=12,
            )
        )
        await session.commit()
