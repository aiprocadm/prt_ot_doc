...
WITHOUT_WRITERS: dict[str, str] = {
    ...
    # Срез-138: сюда ПЕРЕЕХАЛА DocumentGenerationJob — прежний вердикт «мёртвая»
    # был неверен, и срез-135 на нём удалил модель: ручки документов отдали 500.
    "DocumentGenerationJob": (
        "читают ручки документов: фильтр списка «генерируется»/«ошибка» смотрит на "
        "Document.job, поле job_id есть в DocumentRead (открытый контракт); заданий "
        "не заводит никто — фильтр пуст всегда, живая генерация идёт через DocumentJob"
    ),
