...
Exercises `render_docx` end-to-end through a real DOCX zip plus the internal
helpers (`_f_upper`, `_f_lower`, `_f_date`, `_render_jinja`) directly. Goal:
30+ focused cases for null values, loops, conditions, filters, split runs,
...
def test_split_runs_render_correctly() -> None:
    out = render_docx(_docx_split(["{{ na", "me }}"]), {"name": "Alice"}, PASSPORT)
...
def test_cyrillic_payload_round_trips() -> None:
    out = render_docx(
        _docx("Сотрудник: {{ name }}"), {"name": "Анна Сидорова"}, PASSPORT
    )
...
def test_if_truthy_renders_body() -> None:
    out = render_docx(
        _docx("{% if active %}YES{% endif %}"), {"active": True}, PASSPORT
    )
...
def test_if_falsy_skips_body() -> None:
    out = render_docx(
        _docx("[{% if active %}YES{% endif %}]"), {"active": False}, PASSPORT
    )
...
def test_loop_over_empty_list_yields_empty_text() -> None:
    out = render_docx(
        _docx("[{% for x in xs %}{{ x }}{% endfor %}]"), {"xs": []}, PASSPORT
    )
    assert "[]" in _all_text(out)
...
def test_loop_iterates_dict_items() -> None:
    out = render_docx(
        _docx(
            "{% for item in items %}{{ item.label }}:{{ item.qty }};{% endfor %}"
        ),
...
# ---------- passport injection sanity ----------


def test_passport_marker_present_in_output() -> None:
    out = render_docx(_docx("Hello"), {}, PASSPORT)
    with zipfile.ZipFile(BytesIO(out)) as zin:
        document_xml = zin.read("word/document.xml")
    assert b"PTD-PASSPORT:" in document_xml
